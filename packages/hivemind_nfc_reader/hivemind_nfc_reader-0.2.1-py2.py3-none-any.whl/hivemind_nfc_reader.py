...
def set_lights():
    ...
    while True:
        ...
        for pin in filter(lambda i: i.get("light"), settings["pin_config"]):
            if mode == "blink":
                value = int(time.monotonic() * 8) % 2 == 1
                ...
                idx = PIN_ORDER.get(pin["player_id"], 0)
                value = frame in frames_on_by_idx.get(idx, {})
                ...
                value = math.sin(elapsed.total_seconds() * 4) / 2 + 0.5
            else:
                value = state["lights_on"].get(pin["light"], False)
