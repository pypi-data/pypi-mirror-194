...
#        'ObservatoryID': 'spase://SMWG/Observatory/ACE',
    ...
    result = hdp.get_spase(types, query, time_range)
    if result['HttpStatus'] == 200:
        #print('HDP Spase:')
        #print(ET.tostring(result['Result']))
        print('Result ResourceIDs:')
        for r_id in result['Result'].findall('.//' + ET_SPASE_NS +
                                             'ResourceID'):
            print(r_id.text)
    else:
    ...
    if result['HttpStatus'] == 200:
        ...
        print_error('hdp.get_observatory_ids', result)
    ...
    result = hdp.get_spase(types, query)
    if result['HttpStatus'] == 200:
        print('HDP Spase:')
        print(result['Result'])
    else:
