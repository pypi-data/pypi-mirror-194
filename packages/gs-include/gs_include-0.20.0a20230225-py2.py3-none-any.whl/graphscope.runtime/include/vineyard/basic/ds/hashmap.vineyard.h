...
class __attribute__((annotate("vineyard"))) Hashmap : public Registered<Hashmap<K, V, H, E>>,
  ...
  public:
    void Construct(const ObjectMeta& meta) override {
        ...
        this->data_buffer_mmaped_ = std::dynamic_pointer_cast<Blob>(meta.GetMember("data_buffer_mmaped_"));
...
public:
  ...
  void PostConstruct(const ObjectMeta& meta) override {
    ...
    if (data_buffer_mmaped_ != nullptr) {
      // value in orignal string_view: k1 = data_buffer + offset
      // value in mmap-ed string_view: k2 = data_buffer_mmaped_ + offset
      //
      // -> k2 = k1 + diff
      diff_ = reinterpret_cast<uintptr_t>(data_buffer_mmaped_->data()) -
              data_buffer_;
    }
  }
 ...
 private:
  ...
  __attribute__((annotate("shared"))) std::shared_ptr<Blob> data_buffer_mmaped_;
  ptrdiff_t diff_ = 0;
...
class HashmapBaseBuilder: public ObjectBuilder {
  public:
    ...
    explicit HashmapBaseBuilder(
        ...
        this->set_data_buffer_mmaped_(__value.data_buffer_mmaped_);
    }
    ...
    std::shared_ptr<Object> _Seal(Client &client, std::shared_ptr<Hashmap<K, V, H, E>> &__value) {
        ...
        // using __data_buffer_mmaped__value_type = typename std::shared_ptr<Blob>::element_type;
        using __data_buffer_mmaped__value_type = typename decltype(__value->data_buffer_mmaped_)::element_type;
        auto __value_data_buffer_mmaped_ = std::dynamic_pointer_cast<__data_buffer_mmaped__value_type>(
            data_buffer_mmaped_->_Seal(client));
        __value->data_buffer_mmaped_ = __value_data_buffer_mmaped_;
        __value->meta_.AddMember("data_buffer_mmaped_", __value->data_buffer_mmaped_);
        __value_nbytes += __value_data_buffer_mmaped_->nbytes();
  ...
  protected:
    ...
    std::shared_ptr<ObjectBase> data_buffer_mmaped_;
    ...
    void set_data_buffer_mmaped_(std::shared_ptr<ObjectBase> const & data_buffer_mmaped__) {
        this->data_buffer_mmaped_ = data_buffer_mmaped__;
    }
