...
def downloadGOSAT(outdir, nowdate, username, password, prod='CO2'):
    '''
    ...
    if prod in ['CO2', 'co2'] :
        for version in CO2_Info :
            if nowdate >= CO2_Info[version][0] and nowdate <= CO2_Info[version][1] :
                break

    if prod in ['CH4', 'ch4'] :
        for version in CH4_Info :
            if nowdate >= CH4_Info[version][0] and nowdate <= CH4_Info[version][1] :
                break

    url = r'https://data2.gosat.nies.go.jp/wgetdata/GU/SWIRL2{prod}/{YYYY}/SWIRL2{prod}_{YYYY}{MM}_{version}.tar'.format(
        prod=prod,
        version=version,
