...
from coremltools.converters.mil.frontend.paddle.test.testing_utils import \
    _copy_input_data
...
class TestPaddlePaddleConverterExamples:
    @staticmethod
    def test_convert_paddle_vision_mobilenet_v2(tmpdir):
        """
        In this example, we'll instantiate a PaddlePaddle classification model and convert
        it to Core ML.
        """
        ...
        """
        The next thing we need to do is generate PaddleScript for the model. The easiest
        ...
        """
        Now we actually trace the model. This will produce the PaddleScript that the
        CoreML converter needs.
        """
        traced_model = paddle.jit.trace(model, example_input)

        """
        Now with a PaddleScript representation of the model, we can call the CoreML
    ...
    @staticmethod
    def test_convert_paddle_traced_model_to_milinternal(tmpdir):
        ...
        paddle_model = Network()
        paddle_model.eval()
        example_input = paddle.rand(1, 100)
        traced_model = paddle.jit.trace(paddle_model, example_input)
        model = ct.convert(
            traced_model,
            inputs=[ct.TensorType(name="input", shape=example_input.shape)],
    ...
    @staticmethod
    def test_paddle_classifier():
        class Net(paddle.nn.Module):
            def __init__(self):
                super(Net, self).__init__()
                self.linear1 = paddle.nn.Linear(28 * 28, 100)
                self.linear2 = paddle.nn.Linear(100, 50)
                self.final = paddle.nn.Linear(50, 10)
        ...
        model = Net()
        model.eval()
        example_input = paddle.rand(1, 28 * 28, 1)
        traced_model = paddle.jit.trace(model, example_input)
        traced_model.eval()
    ...
    def test_convert_to_argument_with_paddle_model(tmpdir, convert_to):
        class Network(paddle.nn.Module):
            def __init__(self):
                super(Network, self).__init__()
                self.hidden = paddle.nn.Linear(30, 5)
                self.relu = paddle.nn.ReLU()
        ...
        paddle_model = Network()
        paddle_model.eval()
        example_input = paddle.rand(1, 30)
        traced_model = paddle.jit.trace(paddle_model, example_input)
        model = ct.convert(
            traced_model,
            inputs=[ct.TensorType(name="input", shape=example_input.shape)],
    ...
    @staticmethod
    def test_deployment_target_argument_with_paddle_model():
        class Network(paddle.nn.Module):
            def __init__(self):
                super(Network, self).__init__()
                self.hidden = paddle.nn.Linear(30, 5)
    ...
    @staticmethod
    def test_get_milprogram_method_with_paddle_model():
        class Network(paddle.nn.Module):
            def __init__(self):
                super(Network, self).__init__()
                self.hidden = paddle.nn.Linear(100, 10)
                self.relu = paddle.nn.ReLU()
    ...
    def test_classifier_from_paddle_model(convert_to, provide_prob_output_argument):
        paddle_model = paddle.nn.ReLU().eval()
        traced_model = paddle.jit.trace(paddle_model, paddle.rand(3,))
        variable_name = "var_2"
...
###############################################################################
# Note: Stress tests for PaddlePaddle input / output types
###############################################################################

@pytest.mark.skipif(ct.utils._macos_version() < (10, 15), reason='Model produces specification 4.')
@pytest.mark.skipif(not _HAS_PADDLE, reason=MSG_PADDLE_NOT_FOUND)
class TestPaddleInputs(_TestInputs):
    @staticmethod
    @pytest.mark.skipif(not ct.utils._is_macos(), reason="test needs predictions")
    def test_paddle_predict_input():
        TestPaddleInputs._test_variant_input_type_prediction(paddle.tensor)
    ...
    def test_int64_inputs():
        ...
        class TestModule(paddle.nn.Module):
            def __init__(self):
                ...
                self.embedding = paddle.nn.Embedding(num_tokens,
                    embedding_size)
        ...
        example_input = paddle.randint(high=num_tokens, size=(2,), dtype=paddle.int64)
        traced_model = paddle.jit.trace(model, example_input)
        mlmodel = ct.convert(
            traced_model,
            inputs=[
    ...
    def test_fully_dynamic_inputs():
        ...
        model = Model(paddle.tensor(3))
        scripted_model = paddle.jit.script(model)
        ...
        # running predict() is supported on macOS
        if ct.utils._is_macos():
            x, y = paddle.rand(2, 4), paddle.rand(1, 2)
            paddle_input = _copy_input_data([x, y])
            paddle_res = model(*paddle_input)
            results = mlmodel.predict({"x": x.cpu().detach().numpy(),
              "y": y.cpu().detach().numpy()})
            for i, name in enumerate(mlmodel.output_description):
                np.testing.assert_allclose(paddle_res[i], results[name])

            x, y = paddle.rand(1, 6), paddle.rand(2, 3)
            paddle_input = _copy_input_data([x, y])
            paddle_res = model(*paddle_input)
            results = mlmodel.predict({"x": x.cpu().detach().numpy(),
              "y": y.cpu().detach().numpy()})
    ...
    @staticmethod
    def test_rank0_inputs_paddle():
        """Similar to TestPaddlePaddleConverterExamples::test_int64_inputs but
    ...
    def test_paddle_range_dim_lstm(variable_length):
        """
        ...
        class TestNet(paddle.nn.Module):
            def __init__(self):
              super(TestNet, self).__init__()
              self.lstm = paddle.nn.LSTM(input_size, hidden_size, 1)
        ...
        rand_input = paddle.rand(*input_shape)
        h_shape = (1, batch, hidden_size)
        rand_h0 = paddle.rand(*h_shape)
        rand_c0 = paddle.rand(*h_shape)

        traced_model = paddle.jit.trace(model, (rand_input, rand_h0, rand_c0))
    ...
    def test_paddle_outofbound_range_dim(use_symbol):
        ...
        class TestModule(paddle.nn.Module):
            def __init__(self):
                super(TestModule, self).__init__()
                self.embedding = paddle.nn.Embedding(num_tokens, embedding_size)
        ...
        example_input = paddle.randint(high=num_tokens, size=(3,),
                dtype=paddle.int64)
        traced_model = paddle.jit.trace(model, example_input)
        ...
        if ct.utils._is_macos():
            ...
            with pytest.raises(RuntimeError,
                ...
                example_input2 = paddle.randint(high=num_tokens, size=(99,),
                        dtype=paddle.int64)
                result = mlmodel.predict(
            ...
            with pytest.raises(RuntimeError,
                ...
                example_input2 = paddle.randint(high=num_tokens, size=(2,),
                        dtype=paddle.int64)
                result = mlmodel.predict(
                    {"input": example_input2.detach().numpy().astype(np.float32)}
                )
    ...
    @staticmethod
    def test_paddle_enumerated_shapes():
        ...
        example_input = paddle.randn(1, 3, 28, 28)
        traced_model = paddle.jit.trace(model, example_input)
        ...
        if ct.utils._is_macos():
            ...
            # Test (1, 3, 56, 56) shape (can't verify numerical parity with Paddle
    ...
    @staticmethod
    def test_paddle_image_enumerated_shapes():
        import paddlevision
        paddle_model = paddlevision.models.mobilenet_v2().features
        paddle_model.eval()
        example_input = paddle.rand(1, 3, 256, 256)
        traced_model = paddle.jit.trace(paddle_model, example_input)
    ...
    @staticmethod
    def test_paddle_optional_input():
        ...
        example_input = [
            paddle.randint(high=num_tokens, size=(2,), dtype=paddle.int64),
            paddle.rand(1),
            ]
        traced_model = paddle.jit.trace(model, example_input)
        ...
        for compute_units in ct.ComputeUnit:
            ...
            if ct.utils._is_macos():
                ...
                # Verify outputs
                paddle_default_value = paddle.tensor([3])
                expected = model(example_input[0].detach(), paddle_default_value)
                name = list(result.keys())[0]
                np.testing.assert_allclose(result[name], expected.detach().numpy())
...
def int32_input_model():
    class Model(paddle.nn.Module):
        def forward(self, x):
            return x + 5
    example_input = paddle.randint(0, 100, (10, 20), dtype=paddle.int32)
    return paddle.jit.trace(Model().eval(), example_input)
...
def int64_input_model():
    class Model(paddle.nn.Module):
        def forward(self, x):
            return x + 5
    example_input = paddle.randint(0, 100, (10, 20), dtype=paddle.int64)
    return paddle.jit.trace(Model().eval(), example_input)
...
def float32_input_model_add_op():
    class Model(paddle.nn.Module):
        def forward(self, x):
            return x + 5.5
    example_input = paddle.randint(0, 100, (10, 20), dtype=paddle.float32)
    return paddle.jit.trace(Model().eval(), example_input)
...
def float32_two_input_model():
    class Model(paddle.nn.Module):
        def forward(self, x, y):
            return x + y
    example_input = paddle.randint(0, 100, (10, 20), dtype=paddle.float32)
    return paddle.jit.trace(Model().eval(), [example_input, example_input])
...
def float32_two_output_model():
    class Model(paddle.nn.Module):
        def forward(self, x):
            y = paddle.nn.ReLU()(x)
            out1 = paddle.nn.ReLU()(y)
            out2 = paddle.nn.ReLU6()(x)
            return out1, out2
    example_input = paddle.randint(0, 100, (10, 20), dtype=paddle.float32)
    return paddle.jit.trace(Model().eval(), example_input)
...
def rank3_input_model():
    class Model(paddle.nn.Module):
        def forward(self, x):
            return x + 5.5
    example_input = paddle.randint(0, 100, (1, 10, 20), dtype=paddle.float32)
    return paddle.jit.trace(Model().eval(), example_input)
...
def rank4_input_model():
    class Model(paddle.nn.Module):
        def forward(self, x):
            return x + 5.5
    example_input = paddle.randint(0, 100, (1, 3, 10, 20), dtype=paddle.float32)
    return paddle.jit.trace(Model().eval(), example_input)
...
def rank4_grayscale_input_model():
    class Model(paddle.nn.Module):
        def forward(self, x):
            return x + 10
    example_input = paddle.randint(0, 100, (1, 1, 10, 20), dtype=paddle.float32)
    return paddle.jit.trace(Model().eval(), example_input)
...
def linear_model():
    ...
    class Model(paddle.nn.Module):
        def __init__(self):
            super().__init__()
            self.linear = paddle.nn.Linear(10, 15, bias=False)
            self.constant_tensor = paddle.ones((15,), dtype=paddle.float32)
        ...
        def forward(self, x):
            ...
            x = paddle.nn.ReLU()(x)
            return x
    example_input = paddle.randint(0, 10, (1, 10), dtype=paddle.float32)
    return paddle.jit.trace(Model().eval(), example_input)
...
class TestInputOutputConversionAPI:
    ...
    def test_input_shape_missing_error(self, float32_input_model_add_op):
        with pytest.raises(ValueError,
                           match="'shape' must be provided in the 'inputs' argument for paddle conversion"):
            mlmodel = ct.convert(float32_input_model_add_op,
                                 inputs=[ct.TensorType(dtype=np.int32)],
                                 minimum_deployment_target=ct.target.macOS12)

    def test_unsupported_input_dtype_in_paddle_model(self, int64_input_model):
        # test that no error is raised when no dtype is provided by the user,
        # and the Paddle model's input dtype is not supported.
    ...
    def test_prediction_with_fp16_io(self):
        paddle_model = paddle.nn.Linear(30, 5).eval()
        traced_model = paddle.jit.trace(paddle_model, paddle.rand(1, 30))
        ...
        reference_output = traced_model(paddle.from_numpy(sample_input)).detach().numpy()
...
class TestGrayscaleImagePredictions:
    ...
    def test_grayscale_input_image(self, rank4_grayscale_input_model):
        ...
        reference_output = rank4_grayscale_input_model(paddle.from_numpy(sample_input.astype(np.float32))).detach().numpy()
        np.testing.assert_allclose(reference_output, model_output, rtol=1e-2, atol=1e-2)
    ...
    def test_grayscale_output_image(self, rank4_grayscale_input_model):
        ...
        reference_output = rank4_grayscale_input_model(paddle.from_numpy(sample_input)).detach().numpy()
