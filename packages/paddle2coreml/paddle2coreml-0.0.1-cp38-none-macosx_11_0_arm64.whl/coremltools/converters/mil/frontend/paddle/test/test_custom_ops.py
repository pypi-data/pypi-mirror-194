...
from coremltools.converters.mil.frontend.paddle.ops import _get_inputs
from coremltools.converters.mil.frontend.paddle.ops import \
    cosine_similarity as cosine_similarity_main
from coremltools.converters.mil.frontend.paddle.paddle_op_registry import \
    _PADDLE_OPS_REGISTRY as _PADDLE_OPS_REG
from coremltools.converters.mil.frontend.paddle.paddle_op_registry import \
    register_paddle_op
...
from .testing_utils import PaddleBaseTest, convert_to_mlmodel
...
default_cosine_similarity = _PADDLE_OPS_REG.get("cosine_similarity", None)


@register_paddle_op(override=True)
def cosine_similarity(context, node):
    cosine_similarity_main(context, node)


# Log custom Cosine Similarity conversion function
custom_cosine_similarity = _PADDLE_OPS_REG["cosine_similarity"]


def _set_paddle_reg_op(op_type, op_func):
    _PADDLE_OPS_REG[op_type] = op_func


class TestCompositeOp(PaddleBaseTest):

    @pytest.mark.parametrize("input_shape", [(100, 180), (56, 123)])
    def test_composite_op(self, input_shape):
        _set_paddle_reg_op("cosine_similarity", custom_cosine_similarity)
        model = nn.CosineSimilarity(dim=1, eps=1e-6)
        self.run_compare_paddle([input_shape, input_shape], model)
        _set_paddle_reg_op("cosine_similarity", default_cosine_similarity)
...
class TestCustomOp:
    ...
    class custom_paddle_sparse_matmul(Operation):
    ...
    def _sparse_mm(context, node):
        ...
        x = mb.custom_paddle_sparse_matmul(
    ...
    def test_custom_sparse_mm_op(self, input_shape=(4, 4)):
        class TestLayer(nn.Module):
            ...
            def forward(self, x, y):
                x = paddle.sparse.mm(x, y)
                return x

        model = TestLayer()
        input_data_x = paddle.ones(input_shape)
        input_data_y = paddle.ones(input_shape)
        input_data = [input_data_x, input_data_y]
        model.eval()
        paddle_model = paddle.jit.trace(model, (input_data_x, input_data_y))
        mlmodel = convert_to_mlmodel(paddle_model, input_data)
