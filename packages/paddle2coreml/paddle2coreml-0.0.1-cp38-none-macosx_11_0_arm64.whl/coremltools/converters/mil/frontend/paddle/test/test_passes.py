...
from ..internal_graph import (InternalPaddleIRBlock, InternalPaddleIRGraph,
                              InternalPaddleIRNode)
from ..paddleir_passes import (flatten_graph_input_values,
...
class TestPaddlePasses:
    """Class containing tests for InternalPaddleIR optimization passes.
    """
    ...
    def test_transform_inplace_ops_loop(self):
        ...
        graph_inputs = OrderedDict()
        graph_inputs["x"] = paddle.rand(1)
        loop_block = InternalPaddleIRBlock(
            inputs=["i.1"],
            outputs=["1"],
            nodes=[
                InternalPaddleIRNode(
        ...
        loop_node = InternalPaddleIRNode(
            inputs=["3", "1"],
            outputs=[],
            kind="loop",
            blocks=[loop_block],
    ...
    def test_transform_inplace_ops_if(self):
        ...
        graph_inputs = OrderedDict()
        graph_inputs["x"] = paddle.rand(1)
        if_true_block = InternalPaddleIRBlock(
            inputs=[],
            outputs=[],
            nodes=[
                InternalPaddleIRNode(
        ...
        if_false_block = InternalPaddleIRBlock(
            inputs=[],
            outputs=[],
            nodes=[
                InternalPaddleIRNode(
