...
class CBKafkaConsumer:
    def __init__(self,
                 ...
                 start_from: type[str, int] = 'earliest'):
        ...
        if cursor > 0:
            partitions = self.__kafka_consumer.partitions_for_topic(topic)
            [self.__commit_since_offset(Message(cursor, topic, p, None)) for p in partitions]
