...
class CndScaffold:
    ...
    def _replace_jinja(self, target, content):
        file_name = self._replace_content(target, self._data_to_replace)
        file_content = self._replace_content(content, self._data_to_replace)
    ...
    def _load_file(self, item):
        ...
        if self.engine == 'jinja2':
            file = self._replace_jinja(target, content)
        else:
            file = self._replace_default(item, target, content)
