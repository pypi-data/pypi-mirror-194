...
class SuperSet(object):
    ...
    def get_slice_fields(self, itr: float, full_field: bool = True):
        slice_field = []

        for mode in self.supermodes:
            slice_field.append(
                mode.field.get_value_from_itr(itr=itr, full_field=full_field)
            )
    ...
    def get_propagation_field(self,
        ...
        :type       kwargs:             dictionary
        """
        slice_structure = self.get_slice_fields(itr=1.0, full_field=True)

        import pyvista as pv

        f = slice_structure[0].field
        x, y = numpy.mgrid[0: f.shape[0], 0: f.shape[1]]
        grid = pv.StructuredGrid(x, y, slice_structure[0].field)

        plotter = pv.Plotter(notebook=False, off_screen=True)

        plotter.add_mesh(
            grid,
            scalars=slice_structure[0].field,
            lighting=False,
            show_edges=True,
        )
        ...
        for z, amplitudes, itr in zip(z_list, amplitudes_list, itr_list):
            slice_structure = self.get_slice_fields(itr=itr, full_field=True)

            total_field = slice_structure[0].field * 0

            for amp, mode in zip(amplitudes, slice_structure):
                total_field += numpy.abs(amp)**2 * mode.field

            pts[:, -1] = total_field.ravel()
            plotter.update_coordinates(pts, render=False)
            plotter.update_scalars(total_field.ravel(), render=False)

            plotter.write_frame()
            plotter.update()
            plotter.set_scale(xscale=None, yscale=None, zscale=20)
