...
class Field(InheritFromSuperMode):
    ...
    def get_value_from_itr(self, itr: float, full_field: bool = True):
        slice_number = self.itr_to_slice(itr)
        if full_field:
            x, y, mesh = self.apply_boundary_symmetries(mesh=self._data[slice_number])
        else:
            x, y, mesh = self.axes.x_mesh, self.axes.y_mesh, self._data[slice_number]

        return NameSpace(x=x, y=y, field=mesh)
