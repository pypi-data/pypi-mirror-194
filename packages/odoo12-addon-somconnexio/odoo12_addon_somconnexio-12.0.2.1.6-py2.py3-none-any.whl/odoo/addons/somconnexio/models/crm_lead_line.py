...
class CRMLeadLine(models.Model):
    ...
    def _onchange_icc(self):
        ...
        if self.id.__class__ == models.NewId:
            # onchange behaviour
    ...
    def add_activation_notes(self):
        ...
        for lead_line in self:
            ...
            # Other lead lines from different partner's leads
            partner_other_leads = self.env['crm.lead'].search([
                ('id', '!=', lead_line.id),
                ('partner_id', '=', lead_line.lead_id.partner_id.id),
                ('stage_id', '!=', self.env.ref('somconnexio.stage_lead5').id),
                ('date_open', '>=', create_date - timedelta(days=2)),
                ('date_open', '<=', create_date + timedelta(days=2))
            ])

            partner_other_lead_lines = []
            for lead in partner_other_leads:
                for line in lead.lead_line_ids:
                    partner_other_lead_lines.append(line)

            # Lead lines from same lead
            partner_same_lead_lines = lead_line.lead_id.lead_line_ids.filtered(
                lambda line: line != lead_line)
            partner_other_lead_lines.extend(
                line for line in partner_same_lead_lines)
            ...
            if lead_line.is_mobile:
                phones_from_other_leads.extend([
                    ...
                    for other_lead_line in partner_other_lead_lines
