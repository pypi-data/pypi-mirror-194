...
class SalesInvoicesStream(ExactOnlineStream):
    ...
    def get_path(self, context: Optional[dict]) -> str:
        ...
        replication_key_value = self.get_starting_timestamp(context).strftime('%Y-%m-%dT%H:%M')
        
        path = "/salesinvoice/SalesInvoices?" \
            f"$filter={self.replication_key}%20ge%20datetime%20%27{replication_key_value}%27&" \
