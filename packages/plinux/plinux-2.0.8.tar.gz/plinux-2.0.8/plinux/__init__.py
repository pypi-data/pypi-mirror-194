...
class SSHResponse:
    ...
    def stdout(self):
        ...
        try:
            out = json.loads(out)
            self.logger.debug('JSON file detected. Dict wil be returned.')
            return out
        except JSONDecodeError:
            pass

        # Try to convert into ConfigParser
        try:
            buffer = io.StringIO(out)
            config = configparser.ConfigParser()
            config.read_file(buffer)
            self.logger.debug('Config (INI) file detected. ConfigParser wil be returned.')
            return config
...
class Plinux:
    ...
    def cat_ini(self, path: str, sudo: bool = False) -> configparser.ConfigParser:
        """Get ini file and return Parser
        ...
        ini = self.cat(path, sudo=sudo)
        buffer = io.StringIO(ini)
        config = configparser.ConfigParser()
        config.read_file(buffer)
