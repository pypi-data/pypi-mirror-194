    ...
    for capture_info in dcli:
        ...
        if capture_info.get("metadata"):
            for metadata in capture_info["metadata"]:
                metadata_obj = label_set.get(metadata["name"])
                ...
                if metadata_obj is None:
                    ...
                    metadata_obj.insert()
                    label_set[metadata_obj.name] = metadata_obj
                    label_value_set[metadata_obj.name] = {}

                metadata_value_obj = label_value_set[metadata_obj.name].get(
                    metadata["value"]
                )
        ...
        if capture_info.get("sessions"):
            for session in capture_info["sessions"]:
                ...
                if session["session_name"] not in session_set:
                    ...
                    session_set[session["session_name"]] = new_segmenter
                ...
                for segment in session["segments"]:
                    ...
                    if label_obj is None:
                        ...
                        label_obj.insert()
                        label_set[segment["name"]] = label_obj
