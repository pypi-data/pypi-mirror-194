...
def validate_pastureGrass_key_units(data: dict, list_key: str = 'practices'):
    ...
    def validate(values: tuple):
        ...
        return term_id != validate_term_id or not key_units or key_units == validate_key_units or {
            'level': 'error',
            'dataPath': f".{list_key}[{index}].key",
            'message': f"{validate_term_id} key units must be 'ha'",
            'params': {
                'value': key_units,
