...
class BaseRole(Named):
    identity_provider = FieldProperty(schemas.IBaseRole["identity_provider"])
    ...
    assume_role_policies = FieldProperty(schemas.IBaseRole["assume_role_policies"])
...
class AssumeRolePolicy(Parent):
    ...
    condition = FieldProperty(schemas.IAssumeRolePolicy["condition"])

@implementer(schemas.IAssumeRolePolicies)
class AssumeRolePolicies(Parent):
    statement = FieldProperty(schemas.IAssumeRolePolicies["statement"])
...
class Principal(Named):
    ...
    federated = FieldProperty(schemas.IPrincipal["federated"])
