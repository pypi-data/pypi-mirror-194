...
from paco.models.base import Enablable, Parent, Named, CFNExport, Deployable, HasStack, \
     Resource, ApplicationResource, AccountRegions, Notifiable
from paco.models.metrics import Monitorable
from paco.models.iam import Role, ManagedPolicy
...
@implementer(schemas.IEC2FlowLogDestinationOptions)
class EC2FlowLogDestinationOptions(Resource):
    file_format = FieldProperty(schemas.IEC2FlowLogDestinationOptions['file_format'])
    hive_compatible_partitions = FieldProperty(schemas.IEC2FlowLogDestinationOptions['hive_compatible_partitions'])
    per_hour_partition = FieldProperty(schemas.IEC2FlowLogDestinationOptions['per_hour_partition'])

@implementer(schemas.IEC2FlowLog)
class EC2FlowLog(Resource):
    destination_options = FieldProperty(schemas.IEC2FlowLog['destination_options'])
    log_destination_type = FieldProperty(schemas.IEC2FlowLog['log_destination_type'])
    log_destination = FieldProperty(schemas.IEC2FlowLog['log_destination'])
    log_format = FieldProperty(schemas.IEC2FlowLog['log_format'])
    max_aggregation_interval = FieldProperty(schemas.IEC2FlowLog['max_aggregation_interval'])
    resource_id = FieldProperty(schemas.IEC2FlowLog['resource_id'])
    resource_type = FieldProperty(schemas.IEC2FlowLog['resource_type'])
    traffic_type = FieldProperty(schemas.IEC2FlowLog['traffic_type'])
...
class EC2Resource(Named):
    eips = FieldProperty(schemas.IEC2Resource['eips'])
    keypairs = FieldProperty(schemas.IEC2Resource['keypairs'])
    users = FieldProperty(schemas.IEC2Resource['users'])
    groups = FieldProperty(schemas.IEC2Resource['groups'])
    ...
    def resolve_ref(self, ref):
        if ref.parts[2] == 'keypairs':
            ...
            return value
        if ref.parts[2] == 'eips':
            eip_account = ref.parts[3]
            eip_region = ref.parts[4]
            eip_name = ref.parts[5]
            eip = self.eips[eip_account][eip_region][eip_name]
            if ref.last_part == 'allocation_id' and eip.allocation_id != None:
                return eip.allocation_id
            if ref.last_part == 'ip_address' and eip.ip_address != None:
                return eip.ip_address

        return self.resolve_ref_obj.resolve_ref(ref)

# Guard Duty
@implementer(schemas.IGuardDutyMalwareProtectionScanEC2)
class GuardDutyMalwareProtectionScanEC2(Named, dict):
    ebs_volumes = FieldProperty(schemas.IGuardDutyMalwareProtectionScanEC2['ebs_volumes'])

@implementer(schemas.IGuardDutyDataSourceMalwareProtection)
class GuardDutyDataSourceMalwareProtection(Named, dict):
    scan_ec2_instance_with_findings = FieldProperty(schemas.IGuardDutyDataSourceMalwareProtection['scan_ec2_instance_with_findings'])

@implementer(schemas.IGuardDutyDataSourceS3Logs)
class GuardDutyDataSourceS3Logs(Named, Deployable):
    pass

@implementer(schemas.IGuardDutyDetectorDataSources)
class GuardDutyDetectorDataSources(Named, dict):
    malware_protection = FieldProperty(schemas.IGuardDutyDetectorDataSources['malware_protection'])
    s3_logs = FieldProperty(schemas.IGuardDutyDetectorDataSources['s3_logs'])

@implementer(schemas.IGuardDutyDetectorRegion)
class GuardDutyDetectorRegion(Named, Deployable, Monitorable):
    data_sources = FieldProperty(schemas.IGuardDutyDetectorRegion['data_sources'])

    def __init__(self, name, __parent__):
        super().__init__(name, __parent__)
        self.type = 'GuardDutyDetector'

@implementer(schemas.IGuardDutyDetectorAccounts)
class GuardDutyDetectorAccounts(Named, dict):
    pass

@implementer(schemas.IGuardDutyDetectorAccountRegionSets)
class GuardDutyDetectorAccountRegionSets(Named, dict):
    pass

@implementer(schemas.IGuardDutyResource)
class GuardDutyResource(Named):
    detectors = FieldProperty(schemas.IGuardDutyResource['detectors'])


# Amazon Inspector
@implementer(schemas.IInspectorRegion)
class InspectorRegion(Named, Deployable, Monitorable):
    resource_types = FieldProperty(schemas.IInspectorRegion['resource_types'])

    def __init__(self, name, __parent__):
        super().__init__(name, __parent__)
        self.type = 'Inspector'

@implementer(schemas.IInspectorAccounts)
class InspectorAccounts(Named, dict):
    pass

@implementer(schemas.IInspectorAccountRegionSets)
class InspectorAccountRegionSets(Named, dict):
    pass

@implementer(schemas.IInspectorResource)
class InspectorResource(Named):
    inspectors = FieldProperty(schemas.IInspectorResource['inspectors'])


# CloudWatch Monitoring Account
@implementer(schemas.IMonitoringAccountRegion)
class MonitoringAccountRegion(Named, Deployable, Monitorable):
    data_types = FieldProperty(schemas.IMonitoringAccountRegion['data_types'])

    def __init__(self, name, __parent__):
        super().__init__(name, __parent__)
        self.type = 'MonitoringAccount'

@implementer(schemas.IMonitoringAccountAccounts)
class MonitoringAccountAccounts(Named, dict):
    pass

@implementer(schemas.IMonitoringAccountAccountRegionSets)
class MonitoringAccountAccountRegionSets(Named, dict):
    pass

@implementer(schemas.ICloudWatchResource)
class CloudWatchResource(Named):
    monitoring_accounts = FieldProperty(schemas.ICloudWatchResource['monitoring_accounts'])

    def resolve_ref(self, ref):
        return self.resolve_ref_obj.resolve_ref(ref)

# AppConfig
@implementer(schemas.IAppConfigApplicationEnvironment)
class AppConfigApplicationEnvironment(Named):
    pass

@implementer(schemas.IAppConfigApplicationProfile)
class AppConfigApplicationProfile(Named):
    pass

@implementer(schemas.IAppConfigApplicationEnvironments)
class AppConfigApplicationEnvironments(Named, dict):
    pass

@implementer(schemas.IAppConfigApplicationProfiles)
class AppConfigApplicationProfiles(Named, dict):
    pass

@implementer(schemas.IAppConfigApplication)
class AppConfigApplication(Named):
    profiles = FieldProperty(schemas.IAppConfigApplication['profiles'])
    environments = FieldProperty(schemas.IAppConfigApplication['environments'])

@implementer(schemas.IAppConfigApplications)
class AppConfigApplications(Named, dict):
    pass
@implementer(schemas.IAppConfigRegion)
class AppConfigRegion(Named, Deployable, Monitorable):
    applications = FieldProperty(schemas.IAppConfigRegion['applications'])

@implementer(schemas.IAppConfigAccounts)
class AppConfigAccounts(Named, dict):
    pass

@implementer(schemas.IAppConfigAccountRegionSets)
class AppConfigAccountRegionSets(Named, dict):
    pass

@implementer(schemas.IAppConfigResource)
class AppConfigResource(Named, dict):
    pass
...
class Route53HealthCheck(Resource):
    ...
    troposphere_props = troposphere.route53.HealthCheckConfig.props
    cfn_mapping = {
...
class IAMUserProgrammaticAccess(Enablable):
    externally_managed = FieldProperty(schemas.IIAMUserProgrammaticAccess['externally_managed'])
...
@implementer(schemas.IIAMUserPermissionAccessKeys)
class IAMUserPermissionAccessKeys(IAMUserPermission):
    pass
...
@implementer(schemas.IIAMPolicies)
class IAMPolicies(Named, dict):
    pass

@implementer(schemas.IIAMPolicy)
class IAMPolicy(Resource, ManagedPolicy):
    accounts = FieldProperty(schemas.IIAMRole['accounts'])

@implementer(schemas.IIAMRoles)
class IAMRoles(Named, dict):
    pass
...
@implementer(schemas.IIAMIdentityProvider)
class IAMIdentityProvider(Named, dict):
    provider_type = FieldProperty(schemas.IIAMIdentityProvider['provider_type'])
    provider = FieldProperty(schemas.IIAMIdentityProvider['provider'])
    accounts = FieldProperty(schemas.IIAMIdentityProvider['accounts'])
    audiences = FieldProperty(schemas.IIAMIdentityProvider['audiences'])

@implementer(schemas.IIAMIdentityProviders)
class IAMIdentityProviders(Named, dict):
    pass

@implementer(schemas.IIAMResource)
class IAMResource(Named):
    roles = FieldProperty(schemas.IIAMResource['roles'])
    users = FieldProperty(schemas.IIAMResource['users'])
    identity_providers = FieldProperty(schemas.IIAMResource['identity_providers'])
