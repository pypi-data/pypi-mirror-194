...
class HTTP(AbstractClient):
    ...
    def __print_req(self, url, files):
        ...
        if method == "get":
            res_record.place_data("method", "GET", "interfaces")
            if body is not None:
                Printer.print_request_info("GET", name, url, header, body, body_type, cookies=cookies, auth=auth)
                return
            if param is not None:
                Printer.print_request_info("GET", name, url, header, param, "key_value", cookies=cookies, auth=auth)
                return
            Printer.print_request_info("GET", name, url, header, None, None, cookies=cookies, auth=auth)
        else:
            # 打印关键信息
            res_record.place_data("method", method.upper(), "interfaces")

            Printer.print_request_info(method.upper(), name, url, header, body, body_type, files, param, cookies, auth)
    ...
    def __process_res(self, r):
        ...
        if not tr_base64 and content_disposition is not None:
            if "attachment" in content_disposition:
                tr_base64 = True
        if tr_base64:
            DebugPrinter.info("该响应不是文本数据，将会直接转为base64字符串，此行为由测试框架执行")
