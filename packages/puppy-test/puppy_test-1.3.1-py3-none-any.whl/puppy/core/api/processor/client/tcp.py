...
class TCP(AbstractClient):
    ...
    def __init__(self, interface_info: Interface, interface_data: dict, context: dict, _type):
        """
        HTTP 接口客户端
    ...
    def _request(self):
        ...
        if self.__type == "tcp":
            client = TcpClient(server, port, data)
        elif self.__type == "tcp_for_flow_bank":
            client = TcpClientForFB(server, port, data)
        else:
            raise MyException("不支持的TCP协议：{}!".format(self.__type))
        Printer.print_request_info("TCP", name, "{}:{}".format(
...
class TcpClient(object):
    ...
    def __init__(self, server, port, data):
        ...
        self.__encoding = config.get_config("tcp_encoding")
