...
class ProcessorData(object):
    ...
    @classmethod
    def is_usable(cls, if_param, context):
        """
        得到真正的if param
        :param if_param:
        :param context:
...
class PostProcessor(object):
    ...
    # 期望的实际值和期望值的长度
    __length_for_expect = config.get_config("length_for_expect", int)
    ...
    def __core(self, data, context: dict, db_info):
        ...
        for post in Utils.merge_and_sort(data, un):
            xml_track.push_sorted_data(post)
            ...
            DebugPrinter.info("执行{}标签".format(name))
            # 判断是if还是其他
            if name == "if":
                # 处理if
                is_if = True
    ...
    def __work_for_post(self, context: dict, data):
        ...
        if not ProcessorData.is_usable(data.if_param, context):
            DebugPrinter.debug("标签未执行，因为其if属性判定为 False")
            return
        DebugPrinter.debug("标签开始执行,它的描述为 {}".format(data.desc))
    ...
    def __process_req_header(self, context: dict, post: ProcessorData):
        ...
        key = Express.calculate_str(post.key, context)
        desc = post.desc
        ...
        if key == "$whole":
            ...
            if re_regex is Utils.ERROR and source_value is not None and act == "set":
                re_regex = expect
                DebugPrinter.debug("标签从 请求头 提取数据失败，使用默认值 {}".format(expect))
    ...
    def __process_res_header(self, context: dict, post: ProcessorData):
        ...
        if key == "$whole":
            ...
            if re_regex is Utils.ERROR and source_value is not None and act == "set":
                re_regex = expect
                DebugPrinter.debug("标签从 响应头 提取数据失败，使用默认值 {}".format(expect))
            self.__process(context, act, desc, re_regex, expect, names, var_type)
        else:
            if re_type in ["key_value"]:
                re_json = JsonParse(res).get_value(key)
    ...
    def __process_req_data(self, context: dict, post: ProcessorData):
        ...
        if key == "$whole":
            ...
            if re_regex is Utils.ERROR and source_value is not None and act == "set":
                re_regex = expect
                DebugPrinter.debug("标签从 请求报文 提取数据失败，使用默认值 {}".format(expect))
            self.__process(context, act, desc, re_regex, expect, names, var_type)
        else:
    ...
    def __process_res_data(self, context: dict, post: ProcessorData):
        ...
        if key == "$whole":
            ...
            if re_regex is Utils.ERROR and source_value is not None and act == "set":
                re_regex = expect
                DebugPrinter.debug("标签从 响应报文 提取数据失败，使用默认值 {}".format(expect))
            self.__process(context, act, desc, re_regex, expect, names, var_type)
        else:
    ...
    def __process_sql(self, context: dict, post: ProcessorData):
        ...
        if re_type == "table":
            if key == "$whole":
                ...
                if re_regex is Utils.ERROR and source_value is not None and act == "set":
                    re_regex = expect
                    DebugPrinter.debug("标签从 数据库 提取数据失败，使用默认值 {}".format(expect))
                self.__process(context, act, desc, re_regex, expect, names, var_type)
            else:
                # 如果是其他key
    ...
    def __process_replace_body(self, value):
        ...
        :return:
        """
        DebugPrinter.debug("标签执行成功，将把响应报文替换为 {}".format(str(value)))
    ...
    def __process_sleep(value):
        ...
        DebugPrinter.debug("标签执行成功，将睡眠 {} 秒".format(wait))
        time.sleep(wait)
    ...
    def __process_set(context: dict, names, value, var_type):
        ...
        if len(names) > 1:
            ...
            for i in range(len(names)):
                ...
                DebugPrinter.debug("标签存储了变量 {} 到局部变量池，其值为 {}".format(names[i], value[i]))
        else:
            context[names[0]] = PostProcessor.__get_specified_type_var(value, var_type)
            DebugPrinter.debug("标签存储了变量 {} 到局部变量池，其值为 {}".format(names[0], value))
    ...
    def __process_print(value, desc, var_type):
        ...
        """
        if desc == "打印响应报文":
            res_record.place_data("res_s_data", str(value), "interfaces")
        if not value:
    ...
    def __assert_sql(self, context: dict, ass: AssertData):
        ...
        DebugPrinter.debug("标签执行了sql语句 {}".format(sql))
        DebugPrinter.debug("其结果为 {}".format(res.res))
        key = Express.calculate_str(ass.key, context)
    ...
    def __throw_assert(assert_re, real, expect, reason, type_, sql, desc, error=None):
        ...
        :return:
        """
        if len(str(real)) > PostProcessor.__length_for_expect:
            real = str(real)[:PostProcessor.__length_for_expect] + "..."
        if len(str(expect)) > PostProcessor.__length_for_expect:
            expect = str(expect)[:PostProcessor.__length_for_expect] + "..."
        if type_ == "res_data":
            type_ = "响应数据"
        elif type_ == "status":
            type_ = "响应状态"
        ...
        if assert_re:
            DebugPrinter.debug("断言成功")
            res_record.place_data("asserts", "断言[{}]成功，描述：{}".format(type_, desc), "asserts")
            print("    断言[{}]成功，描述：{}".format(type_, desc))
        else:
            DebugPrinter.debug("断言失败")
            res_record.place_data("asserts", "断言[{}]失败，描述：{}".format(type_, desc), "asserts")
