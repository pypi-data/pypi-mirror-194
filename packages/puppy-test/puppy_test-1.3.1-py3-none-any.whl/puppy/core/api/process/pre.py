...
class PreProcessor(object):
    ...
    def __core(self, context: dict, data, db_info):
        ...
        for pre in Utils.merge_and_sort(data, un):
            xml_track.push_sorted_data(pre)
            name = pre.get("name")
            value = pre.get("value")
    ...
    def __work_for_pre(self, context: dict, pre):
        ...
        if not ProcessorData.is_usable(pre.if_param, context):
            DebugPrinter.debug("标签未执行，因为其if属性判定为 False")
            return
        DebugPrinter.debug("标签开始执行,它的描述为 {}".format(pre.desc))
    ...
    def __process_req_header(self, context: dict, pre: ProcessorData):
        ...
        """
        re_type = pre.re_type
        key = Express.calculate_str(pre.key, context)
        desc = pre.desc
    ...
    def __work_for_interface(self, context: dict, inf):
        ...
        if scope == "header":
            # 替换请求头
            DebugPrinter.debug("正在处理 {} 接口请求头的数据".format(self.__interface_info.name))
            ty = "json"
        elif scope == "body":
            # 替换请求报文
            DebugPrinter.debug("正在处理 {} 接口请求报文 的数据".format(self.__interface_info.name))
            ty = self.__interface_info.body_type
        elif scope == "files":
            # 替换文件
            DebugPrinter.debug("正在处理 {} 接口文件域 的数据".format(self.__interface_info.name))
            ty = "files"
        elif scope == "port":
            # 替换端口
            DebugPrinter.debug("正在处理 {} 接口的端口".format(self.__interface_info.name))
            ty = "port"
        elif scope == "server":
            # 替换地址
            DebugPrinter.debug("标签正在处理 {} 接口的服务器".format(self.__interface_info.name))
            ty = "server"
        elif scope == "param":
            # 替换请求参数
            DebugPrinter.debug("正在处理 {} 接口请求参数 的数据".format(self.__interface_info.name))
            ty = "key_value"
        elif scope == "method":
            DebugPrinter.debug("正在处理 {} 接口的请求方法".format(self.__interface_info.name))
            ty = "method"
        elif scope == "path":
            DebugPrinter.debug("正在处理 {} 接口的请求路径".format(self.__interface_info.name))
            ty = "path"
        elif scope == "type":
            DebugPrinter.debug("正在处理 {} 接口请求报文 的数据格式".format(self.__interface_info.name))
            ty = "type"
            scope = "body_type"
        else:
            raise UnexpectedTagsException()
    ...
    def __delete(self, context, ty: str, scope: str, param: InfoData):
        ...
        key = Express.calculate_str(param.name, context)
        if key == "$whole":
            # 整个删除
            setattr(self.__interface_info, scope, None)
            DebugPrinter.debug("将处理中数据整个清空")
        elif key == "$regex":
            # 正则删除
            ...
            if deleted_re is None:
                ...
                DebugPrinter.debug("使用正则表达式 {} ,删除了处理数据中的第 {} 个匹配".format(param.name_key, param.name_count))
        elif key == "#":
            # 字符符name
            ...
            if ty == "json" or ty == "key_value" or ty == "form_gb18030":
                ...
                DebugPrinter.debug("删除了处理中数据由 {} 指定的内容".format(key))
            elif ty == "xml":
    ...
    def __replace(self, context: dict, ty: str, scope: str, param: InfoData):
        # 取得要替换的key,$whole指整个替换
        replace_key = Express.calculate_str(param.name, context)
        # 取得要替换的值
        ...
        if replace_key == "$whole":
            ...
            DebugPrinter.debug("将处理中数据整体替换为 {}".format(value))
            ...
            if replace_re is None:
                ...
                setattr(self.__interface_info, scope, replace_re)
                DebugPrinter.debug("将处理中数据的 {} 部分替换为 {}".format(param.name_key, value))
        else:
            # 按类型替换
            # 取得要替换的源值
            attr = getattr(self.__interface_info, scope)
            ...
            if ty == "json" or ty == "key_value" or ty == "form_gb18030":
                ...
                DebugPrinter.debug("将处理中数据由 {} 指定的内容替换为 {}".format(replace_key, value))
            elif ty == "xml":
                if not XmlParse.is_xml(attr):
                    raise InfDataIsIncorrectException(scope,self.__interface_info.name)
    ...
    def __insert(self, context: dict, ty: str, scope: str, param: InfoData):
        ...
        insert_key = Express.calculate_str(param.name, context)
        # 取得要插入的值
        value = Express.calculate_str(param.value, context)
        if insert_key == "$whole":
            # 整个插入
            setattr(self.__interface_info, scope, value)
            DebugPrinter.debug("处理中数据被全覆盖，并插入了新的内容 {}".format(value))
            ...
            if ty == "json" or ty == "key_value" or ty == "form_gb18030":
                ...
                DebugPrinter.debug("向处理中数据插入了由 {} 指定的内容 {}".format(insert_key, value))
            elif ty == "files":
                if not Files.is_file_interface(attr):
                    raise InfIsNotFileException(self.__interface_info.name)
    ...
    def __replace_or_insert(self,context:dict,ty:str,scope:str,param:InfoData):
        ...
        if key=="$whole":
            #整个插入
            setattr(self.__interface_info,scope,value)
            DebugPrinter.debug("处理中的数据被全替换为 {}".format(value))
        elif key == "$regex":
            # 正则替换
            attr = getattr(self.__interface_info, scope)
            replace_re = Regex(param.name_key, param.name_count).replace_value(attr, value)
            ...
            if ty == "json" or ty == "key_value" or ty == "form_gb18030":
                ...
                if replace_re is None:
                    ...
                    if insert_re is None:
                        ...
                        setattr(self.__interface_info, scope, insert_re)
                        DebugPrinter.debug("向处理中数据插入了由 {} 指定的内容 {}".format(key, value))
                else:
                    setattr(self.__interface_info, scope, replace_re)
                    DebugPrinter.debug("将处理中数据由 {} 指定的内容替换为 {}".format(key, value))
            elif ty == "xml":
                if not XmlParse.is_xml(attr):
                    raise InfDataIsIncorrectException(scope, self.__interface_info.name)
