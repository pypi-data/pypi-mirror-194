...
class Express(object):
    ...
    def calculate_in_config(cls, text, *context):
        ...
        if len(m) == 1 and "${{{}}}".format(m[0]) == text:
            ...
            for o in m:
                ...
                DebugPrinter.debug("将执行表达式 ${{{}}}".format(o))
                ex_o = Express(o)
                express_r = ex_o.calculate(*context)
                DebugPrinter.debug("表达式 ${{{}}} 已执行，其运算结果为 {}".format(o, express_r))
                if express_r == Utils.ERROR:
                    raise MyException(" {} 中的表达式计算错误：{}".format(text_copy, ex_o.error))
                else:
                    old = "${{{}}}".format(o)
    ...
    def calculate_sql(cls, text, *context):
        ...
        if len(m) == 1 and "${{{}}}".format(m[0]) == text:
            # 存在1个表达式，且表达式就等于text，将直接返回计算后的结果
            from ...printer.debug_printer_ import DebugPrinter
            DebugPrinter.debug("将执行表达式 ${{{}}}".format(m[0]))
            ...
            if express_r == Utils.ERROR:
                ...
                text = express_r
            DebugPrinter.debug("表达式 ${{{}}} 已执行，其运算结果为 {}".format(m[0], express_r))
        else:
            for o in m:
                from ...printer.debug_printer_ import DebugPrinter
                DebugPrinter.debug("将执行表达式 ${{{}}}".format(o))
