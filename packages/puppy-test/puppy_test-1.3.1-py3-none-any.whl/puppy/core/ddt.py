...
def mk_test_name(name, value, index=0):
    ...
    if not is_trivial(value):
        if value and type(value) == dict and value.get("id") is not None and value.get("checkpoint") is not None:
            id=int(value.get("id"))
            id=id if id>9 else "0{}".format(id)
            return "test_{0}_{1}".format(id,value.get("checkpoint"))
        elif value and type(value) == dict and value.get("NO") is not None and value.get("checkpoint") is not None:
            no = int(value.get("NO"))
            no = no if no > 9 else "0{}".format(no)
            return "test_{0}_{1}".format(no, value.get("checkpoint"))
        elif value and type(value) == dict and value.get("用例编号") is not None and value.get("用例名称") is not None:
            no = int(value.get("用例编号"))
            no = no if no > 9 else "0{}".format(no)
            return "test_{0}_{1}".format(no, value.get("用例名称"))
        elif value and type(value) == dict and value.get("name") is not None and value.get("id") is not None:
            return "{0}_{1}_{2}".format(name, value.get("id"), value.get("name"))
