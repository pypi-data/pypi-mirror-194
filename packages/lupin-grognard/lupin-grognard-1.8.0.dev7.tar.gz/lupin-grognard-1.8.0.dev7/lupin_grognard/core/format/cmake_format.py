...
class CMakeFormatter:
    def format_cmake_files(self) -> None:
        ...
        if not self._check_clang_format_yaml_file_exist():
            die(
                msg="could not find .clang-format.yaml file, please create one in the root path"
            )
    ...
    def _check_clang_format_yaml_file_exist(self) -> bool:
        """Check if a .clang-format.yaml file is present in the root path"""
        clang_format_file_path = os.path.join(os.getcwd(), ".clang-format.yaml")
        return os.path.exists(clang_format_file_path)
