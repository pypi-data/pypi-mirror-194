...
def _ping_nano():
    return b"NanoVib" in subprocess.check_output("netsh wlan show interfaces", shell=True)


def _get_most_recent_measurement(folder_to_analyze: Path = (Path().home() / "Downloads")) -> Path:
...
def continuous_analyzing(folder_to_analyze: Path = (Path().home() / "Downloads")) -> None:
