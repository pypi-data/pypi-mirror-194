...
def run_systemctl_unit_cmd(cmd: str, unit_name: str, *args, verbose=True) -> bool:
    """Use to run systemctl unit commands
    :param cmd: start, enable, reload, kill, etc (see systemctl --help)
    :param unit_name: services, sockets, targets, timers
    :param args: additional arguments for systemctl command, eg. " --force"
    :param verbose: prints command output
    """
    cmd = f"systemctl {cmd} {unit_name}"
    if args:
        for arg in args:
            cmd = cmd + f" {arg}"
    ...
    if retcode != 0:
        if not verbose:
            _print(f"FAIL: Running {cmd}")
        run(f"systemctl status {unit_name} -l")
        run("journalctl -xn")
        return False
    return True


def service_start(service_name) -> bool:
    """Start service
    The arguments are:
    \tNone
    Returns:
    ...
    """
    run_systemctl_unit_cmd("start", service_name, verbose=True)
    if not is_service_running():
        return False
    return True


def service_stop(service_name) -> bool:
    """Stop service
    The arguments are:
    \tName of the service
    Returns:
    ...
    \tFalse: There was some problem
    """
    return run_systemctl_unit_cmd("stop", service_name, verbose=True)


def service_restart(service_name) -> bool:
    """Restart service
    The arguments are:
    \tName of the service
    Returns:
    ...
    if service_timestamp is not None:
        ...
        if time.mktime(actual_time) - time.mktime(timestamp_struct) < 5:
            ...
            time.sleep(5)
    return run_systemctl_unit_cmd("restart", service_name, verbose=True)


def service_enable(service_name, now=False) -> bool:
    return systemctl_enable(unit_name=service_name, now=now)


def systemctl_enable(unit_name, now=False, verbose=False) -> bool:
    """Enables systemd unit
    The arguments are:
    \tName of the unit
    Returns:
    \tTrue: Unit got enabled
    \tFalse: There was some problem
    """
    args = ["enable", f"unit_name={unit_name}", f"verbose={verbose}"]
    if now:
        args.insert(2, " --now")
    return run_systemctl_unit_cmd(*args)


def systemctl_disable(unit_name, verbose=False) -> bool:
    """Disables systemd unit
    The arguments are:
    \tName of the unit
    Returns:
    \tTrue: Unit got disabled
    \tFalse: There was some problem
    """
    return run_systemctl_unit_cmd("disable", unit_name, verbose=verbose)


def systemctl_is_enabled(unit_name, verbose=False) -> bool:
    """Runs systemctl is-enabled
    The arguments are:
    \tName of the unit
    Returns:
    \tTrue: systemctl is-enabled returned 0
    \tFalse: unit is not enabled
    """
    return run_systemctl_unit_cmd("is-enabled", unit_name, verbose=verbose)
