...
def get_registration_and_act_hi_var_from_dwh(appIds):
    ...
    print(appId)
    SQL = "SELECT * DWROBOWLBL.LOS_USER_REGISTRATION_DATA s WHERE s.REGISTRATION_ID in ('{}') OR s.EX_APPLICATION_ID in ('{}') AND ROWNUM<2 ORDER BY ID DESC"
    ...
    if not cursorRegis:
        ...
        SQL = "SELECT * DWROBOLENDING.LOS_USER_REGISTRATION_DATA s WHERE s.REGISTRATION_ID in ('{}') AND ROWNUM<2 ORDER BY ID DESC"
