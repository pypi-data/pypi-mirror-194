...
def get_registration_and_act_hi_var_from_dwh(appIds):
    appId = appIds.replace(',',"','")
    print('List appID: '+ appId)
    SQL = "SELECT * DWROBOWLBL.LOS_USER_REGISTRATION_DATA s WHERE s.REGISTRATION_ID in ('{}') OR s.EX_APPLICATION_ID in ('{}') AND ROWNUM<2 ORDER BY ID_ DESC"
    SQL = SQL.format(appId, appId)
    print(SQL)
    connection = cx_Oracle.connect(
        "PATTA_RAJESH", "Pa55w0rd_$U", "172.27.1.7:1521/dwproddc", encoding="utf-8", nencoding="UTF-8")
    cursorRegis = connection.cursor()
    cursorRegis.execute(SQL)
    # dataDWH = cursorRegis.fetchall()
    response = []
    responseRegis = []
    isWhitelabel = False
    procId = ''
    if not cursorRegis:
        isWhitelabel = True
        for row in cursorRegis:
            print('rowrow===> ' + row)
            responseRegis.append(row)
            procId = row
    else:
        SQL = "SELECT * DWROBOLENDING.LOS_USER_REGISTRATION_DATA s WHERE s.REGISTRATION_ID in ('{}') AND ROWNUM<2 ORDER BY ID_ DESC"
        SQL = SQL.format(appId)
        print(SQL)
        cursorRegis = connection.cursor()
        cursorRegis.execute(SQL)
        for row in cursorRegis:
    ...
    responseVar = [] 
    if isWhitelabel:
        SQL = "SELECT * FROM DWROBOWLBL.ACT_HI_VARINST as r WHERE r.PROC_INST_ID_  = '{}'"
        SQL = SQL.format(appId)
        print(SQL)
        cursorVar = connection.cursor()
        cursorVar.execute(SQL)
        for row in cursorVar:
            print('rowrow===> ' + row)
            responseVar.append(row)
    else:
        SQL = "SELECT * FROM DWROBOLENDING.ACT_HI_VARINST as r WHERE r.PROC_INST_ID_  = '{}'"
        SQL = SQL.format(appId)
        print(SQL)
        cursorVar = connection.cursor()
        cursorVar.execute(SQL)
        for row in cursorVar:
            ...
            responseVar.append(row)
    print(responseVar)
    response.append(responseRegis)
    response.append(responseVar)
    connection.close()
    return response
...
def checkAMLService(req):
    # PROD
    SQL_Order1 = "SELECT * FROM operation_risk.af_tbl_aml WHERE PERSON_ID = '{}' AND STATUS = 1"
    SQL_Order2 = "SELECT * FROM operation_risk.af_tbl_aml WHERE FULL_NAME = '{}' AND ID_NO = '{}' AND STATUS = 1"
    SQL_Order3 = "SELECT * FROM operation_risk.af_tbl_aml WHERE FULL_NAME = '{}' AND DOB = '{}' AND STATUS = 1"
    SQL_Order4 = "SELECT * FROM operation_risk.af_tbl_aml WHERE FULL_NAME = '{}' AND STATUS = 1"
