...
@LastEditTime: 2023-02-23 10:36:55
...
class SevenHelper:
    ...
    @classmethod
    def get_row_by_list(self, data, field_name, field_type=1):
        """
        :description: 获取列表中指定列的集合
        ...
        if isinstance(data,list) == False:
            return []
        if field_type == 1:
            return [int(item[field_name]) for item in data]
        else:
            return [str(item[field_name]) for item in data]
