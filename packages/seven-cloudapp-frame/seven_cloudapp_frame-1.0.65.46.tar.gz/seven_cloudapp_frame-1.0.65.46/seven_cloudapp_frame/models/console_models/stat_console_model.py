...
@LastEditTime: 2023-03-01 14:47:03
                        ...
                        try:
                            ...
                            if stat_orm.repeat_type > 0:
                                if stat_orm.repeat_type == 2:
                                    stat_log_dict = stat_log_model.get_dict("act_id=%s and module_id=%s and orm_id=%s and user_id=%s", field="id", params=[stat_queue.act_id, stat_queue.module_id, stat_orm.id, stat_queue.user_id])
                                else:
                                    stat_log_dict = stat_log_model.get_dict("act_id=%s and module_id=%s and orm_id=%s and user_id=%s and create_day=%s", field="id", params=[stat_queue.act_id, stat_queue.module_id, stat_orm.id, stat_queue.user_id, create_day_int])
                                if stat_log_dict:
                                    is_add = False
        ...
        while True:
            try:
                ...
                try:
                    ...
                    if stat_orm.repeat_type > 0:
                        if stat_orm.repeat_type == 2:
                            stat_log_dict = stat_log_model.get_dict("act_id=%s and module_id=%s and orm_id=%s and user_id=%s", field="id", params=[stat_queue_dict["act_id"], stat_queue_dict["module_id"], stat_orm.id, stat_queue_dict["user_id"]])
                        else:
                            stat_log_dict = stat_log_model.get_dict("act_id=%s and module_id=%s and orm_id=%s and user_id=%s and create_day=%s", field="id", params=[stat_queue_dict["act_id"], stat_queue_dict["module_id"], stat_orm.id, stat_queue_dict["user_id"], create_day_int])
                        if stat_log_dict:
                            is_add = False
...
class ClearDataWork(TimingWork):
    ...
    def process_only_table(self):
        ...
        page_index = 0
        while True:
            stat_orm_dict_list, is_next = stat_orm_model.get_dict_page_list(field="id,repeat_type", page_index=page_index, page_size=50, order_by="id asc", page_count_mode="next")
