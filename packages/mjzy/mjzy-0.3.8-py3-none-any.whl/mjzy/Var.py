...
def ar1_var_es(returns, size=10000, alpha=0.05):
    ...
    simulated_returns = np.empty(size)
    returns = returns.values
    last_return = returns[-1]
    # Compute simulated returns using AR(1) model
    for i in range(size):
        simulated_returns[i] = ar1_coeff * last_return + sigma * np.random.normal()
...
def normal_weighted(meta ,size=10000,lambda_=0.94, alpha=0.05):
    ...
    es_ew = -np.mean(simu_norm[simu_norm<=var_ew])
...
def Delta_Normal_VaR(portfolio_type,prices,lambda_=0.94,portfolio_='All'):
    current_price, assets_prices, hold = portfolio.Norm_price(portfolio_type,prices,portfolio_=portfolio_)
