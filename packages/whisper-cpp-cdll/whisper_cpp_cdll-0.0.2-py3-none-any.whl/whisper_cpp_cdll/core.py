...
# sr, d = scipy.io.wavfile.read(bo)
# d = d.astype('float32') / 32768.0
# result = run_whisper(data = d, libname = "/app/whisper.cpp/libwhisper.so", fname_model = "/app/whisper.cpp/models/ggml-tiny.bin")
# => [ {'start': 0, 'end': 17000, 'segment_id': 0, 'text': 'hello world', 'tokens': ... }, .... ]
def run_whisper(
    data,
    libname, 
    fname_model,
    ...
    for start in range(0, total_length, WINDOW_SIZE):
        res = _execute_whisper_full(data = data[start:start+WINDOW_SIZE].copy(), whisper = whisper, ctx = ctx)
        for segment in res:
            segment['start'] += start
            segment['end'] += start
        spokens.extend(res)
