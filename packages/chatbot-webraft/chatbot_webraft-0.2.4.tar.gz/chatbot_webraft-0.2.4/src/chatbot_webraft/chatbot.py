...
def nlpm(word,model,words_list1,words_list2,modelname):
    ...
    happy_synonyms = ['glad', 'joyful', 'elated', 'pleased', 'content', 'cheerful', 'ecstatic', 'overjoyed', 'thrilled',
                      'delighted', 'satisfied']
    angry_synonyms = ['upset', 'mad', 'irate', 'furious', 'enraged', 'incensed', 'outraged', 'livid', 'exasperated',
                      'aggravated', 'infuriated']
    run_synonyms = ['jog', 'sprint', 'dash', 'gallop', 'jogging', 'trot', 'race', 'jogged', 'running', 'sprinted',
                    'dashing']
    big_synonyms = ['large', 'huge', 'enormous', 'gigantic', 'massive', 'colossal', 'vast', 'immense', 'monstrous',
                    'tremendous', 'substantial']
    ...
    def run(modelname,word,model):
        x = model_load(modelname,word,model)
        output = rephrase_sentence(x, synonyms)
        return output
    return run(modelname,word,model)
def model_load(modeltype,input, model,modelname):
    global words_list1
    global words_list2
    global model_name
    input = input.lower()
