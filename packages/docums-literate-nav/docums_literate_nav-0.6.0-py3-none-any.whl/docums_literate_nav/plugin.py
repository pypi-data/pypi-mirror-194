...
import docums.config
import docums.config.config_options
import docums.plugins
import docums.structure.files
import docums.structure.nav
import docums.structure.pages

try:
    from docums.plugins import event_priority
except ImportError:
    event_priority = lambda priority: lambda f: f  # No-op fallback


from docums_literate_nav import parser

log = logging.getLogger(f"docums.plugins.{__name__}")


class _PluginConfig:
    nav_file = docums.config.config_options.Type(str, default="SUMMARY.md")
    implicit_index = docums.config.config_options.Type(bool, default=False)
    markdown_extensions = docums.config.config_options.MarkdownExtensions()
    tab_length = docums.config.config_options.Type(int, default=4)


class LiterateNavPlugin(docums.plugins.BasePlugin):
    config_scheme = tuple(
        (k, v)
        for k, v in _PluginConfig.__dict__.items()
        if isinstance(v, docums.config.config_options.BaseConfigOption)
    )

    @event_priority(-100)  # Run last
    def on_files(self, files: docums.structure.files.Files, config: docums.config.Config) -> None:
    ...
    def on_nav(
        self,
        nav: docums.structure.nav.Navigation,
        config: docums.config.Config,
        files: docums.structure.files.Files,
    ) -> None:
        if files != getattr(self, "_files", None):
            log.warning(
                "The literate-nav plugin created the nav based on files that were subsequently modified by another Docums plugin! "
                "Re-order `plugins` in docums.yml so that 'literate-nav' appears later."
            )
...
def resolve_directories_in_nav(
    ...
    """Walk through a standard Docums nav config and replace `directory/` references.
...
class DocumsGlobber:
    def __init__(self, files: docums.structure.files.Files):
