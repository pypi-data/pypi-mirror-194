...
class Agent(object):
        ...
        if self._agent_jar_path:
            ...
            if self._agent_jar_path is None:
                raise Exception("Error: MLOps Agent JAR file not found. "
                                "Reinstall the agent or specify its location using "
                                "MLOps.agent(agent_jar_path=/path/to/mlops-agent-x.y.z.jar, ...)")
    ...
    def _validate_and_set_spooler(self):
        ...
        if spooler_type == SpoolerType.FILESYSTEM.name:
            ...
            raise Exception("Embedded Agent does not support "
    ...
    def _ensure_mlops_agent_is_compatible(self):
        ...
        try:
            ...
            message = "Failed to determine Agent version. " + \
                      "This MLOps library requires Agent version: " + \
                      Constants.MLOPS_VERSION
            logger.debug("Failed to get Agent's version",
                         exc_info=True)  # details of unimplemented function or otherwise
