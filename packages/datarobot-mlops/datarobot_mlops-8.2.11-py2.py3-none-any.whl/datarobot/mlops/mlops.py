...
class MLOps(object):
    ...
    def set_feature_types_filename(self, feature_type_filename):
        ...
        config.set_config_safely(
            ConfigConstants.FEATURE_TYPES_FILENAME, feature_type_filename
        )
        return self
    ...
    def set_feature_types_json(self, feature_type_json):
        ...
        config.set_config_safely(
            ConfigConstants.FEATURE_TYPES_JSON, feature_type_json
        )
        return self

    def set_feature_types(self, feature_types):
        """
        Set feature types for aggregation purposes, if report_aggregated_predictions_data() is
        used this must be set before.
    ...
    def set_aggregation_max_records(self, aggregation_max_records):
        self._validate_input_positive_num(aggregation_max_records, "aggregation_max_records")
        config.set_config_safely(
            ConfigConstants.STATS_AGGREGATION_MAX_RECORDS, aggregation_max_records
        )
        return self
    ...
    def set_segment_attributes(self, segment_attributes):
        self._validate_input_string(segment_attributes, "segment_attributes")
        config.set_config_safely(
            ConfigConstants.STATS_AGGREGATION_SEGMENT_ATTRIBUTES, segment_attributes
        )
        return self
    ...
    def report_predictions_data(
        ...
        _deployment_id = self._get_id(deployment_id, ConfigConstants.DEPLOYMENT_ID)
        _model_id = self._get_id(model_id, ConfigConstants.MODEL_ID)
        return self._model.report_predictions_data(
            _deployment_id,
            _model_id,
            features_df,
            predictions,
    ...
    def report_aggregated_predictions_data(
        ...
        _deployment_id = self._get_id(deployment_id, ConfigConstants.DEPLOYMENT_ID)
        _model_id = self._get_id(model_id, ConfigConstants.MODEL_ID)
        return self._model.report_aggregated_predictions_data(
            _deployment_id, _model_id,
