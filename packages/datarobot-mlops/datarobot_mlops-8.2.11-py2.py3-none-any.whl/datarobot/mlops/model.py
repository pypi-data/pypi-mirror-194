...
from datarobot.mlops.common.aggregation_util import build_aggregated_stats, validate_feature_types
from datarobot.mlops.common.config import ConfigConstants
from datarobot.mlops.common.exception import DRUnsupportedType, DRCommonException, DRApiException
...
class Model(object):
    ...
    DEFAULT_AGGREGATION_MAX_RECORDS = 10000
    ...
    def _init_aggregation_params(self, feature_types):
        self._stats_aggregator_dict = dict()

        features_types_filename = config.get_config_default(
            ConfigConstants.FEATURE_TYPES_FILENAME, None
        )
        features_types_json = config.get_config_default(
            ConfigConstants.FEATURE_TYPES_JSON, None
        )
        if features_types_json or features_types_filename:
            ...
            self._feature_types = self._convert_feature_types_to_descriptor(feature_types)
        ...
        self._aggregation_max_records = config.get_config_default(
            ConfigConstants.STATS_AGGREGATION_MAX_RECORDS,
            self.DEFAULT_AGGREGATION_MAX_RECORDS
        )
    ...
    def shutdown(self, timeout_sec=0):
        self._flush_aggregated_stats(timeout_sec)
    ...
    def _validate_and_copy_feature_predictions(
            self, features_df=None, predictions=None, class_names=None, association_ids=None,
    ):
    ...
    def report_aggregated_predictions_data(
            self, deployment_id, model_id, features_df=None, predictions=None, class_names=None
    ):
        """
        ...
        if (
            ...
            return self._process_data_for_aggregation_with_prediction_timestamp(
                deployment_id, model_id, feature_data_df, predictions_df, class_names
            )
        else:
            # Call method to report aggregated stats directly
            return self._aggregate_stats(
                deployment_id, model_id,
                feature_data_df=features_df, predictions_df=predictions_df, class_names=class_names
            )

    def _process_data_for_aggregation_with_prediction_timestamp(
            self, deployment_id, model_id, features_df=None, predictions_df=None, class_names=None
    ):
    ...
    def _aggregate_stats(
            ...
            feature_data_df=None, predictions_df=None,
            class_names=None, timestamp=None
    ):
        ...
        # Aggregate and merge with current stats
        agg_dict_key = (deployment_id, model_id, timestamp)
        stats_list, stats_counter = list(), 0
        if agg_dict_key in self._stats_aggregator_dict:
            stats_list, stats_counter, _ = self._stats_aggregator_dict[agg_dict_key]

        stats_list.append(aggregated_out)
        stats_counter += self._get_num_rows(feature_data_df, predictions_df)

        if stats_counter >= self._aggregation_max_records:
            ret_val = self._report_aggregated_stats(
                deployment_id, model_id, stats_list, class_names, timestamp
            )
            self._stats_aggregator_dict[agg_dict_key] = (list(), 0, class_names)
            return ret_val
        else:
            self._stats_aggregator_dict[agg_dict_key] = (stats_list, stats_counter, class_names)
            return True

    def _report_aggregated_stats(self, deployment_id, model_id, stats_list, class_names, timestamp):
        from datarobot.mlops.stats_aggregator import merge_stats

        aggregated_out = merge_stats(
            stats_list,
            histogram_bin_count=self._histogram_bin_count,
            distinct_category_count=self._distinct_category_count
        )
        ...
        aggregated_stats_container = AggregatedStatsContainer(
            general_stats=GeneralStats(model_id, GeneralStats.to_dr_timestamp(timestamp)),
            aggregated_stats=build_aggregated_stats(aggregated_out, class_names),
        )
        return self._report_stats(deployment_id, model_id, aggregated_stats_container)
    ...
    def _flush_aggregated_stats(self, timeout_sec=10, interval_wait_sec=1):
        if not self._stats_aggregator_dict:
            return

        for agg_dict_key, agg_dict_value in iteritems(self._stats_aggregator_dict):
            (deployment_id, model_id, timestamp) = agg_dict_key
            (stats_list, _, class_names) = agg_dict_value
        ...
        start = time.time()
        while self._report_queue.get_queue_size_bytes() > 0 and time.time() - start < timeout_sec:
    ...
    def _build_feature_types_from_vars(self, features_types_filename, features_types_json):
        ...
        return self._convert_feature_types_to_descriptor(feature_types)

    @staticmethod
    def _convert_feature_types_to_descriptor(feature_types):
