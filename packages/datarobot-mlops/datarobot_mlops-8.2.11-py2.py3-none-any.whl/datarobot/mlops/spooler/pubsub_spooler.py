...
class PubSubRecordSpooler(RecordSpooler):
    ...
    def set_config(self):
        ...
        self._ack_deadline = config.get_config_default(
            ConfigConstants.PUBSUB_ACK_DEADLINE_SECONDS,
            self.PUBSUB_DEFAULT_ACK_DEADLINE_SECONDS
        )
    ...
    def ack_records(self, records_id_list):
        ...
        ack_ids = [
            self._records_pending_ack[record_id] for record_id in records_id_list
    ...
    def dequeue(self):
        ...
        response = self._subscriber.pull(self._full_subscription_name,
                                         max_messages=self._max_records_dequeue)
        ack_ids = []
