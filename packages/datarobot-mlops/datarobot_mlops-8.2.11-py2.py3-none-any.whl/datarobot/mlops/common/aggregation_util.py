...
from datarobot.mlops.metric import AggregatedStats, SerializationConstants
...
def _convert_stats_to_dict(aggregated_stats):
    return {
        k: _aggregated_stat_to_dict(v) for k, v in six.iteritems(aggregated_stats)
    }
    ...
    for attribute_name, values in six.iteritems(segment_attributes_stats):
        for value_name, agg_stats in six.iteritems(values):
            return_dict[attribute_name][value_name] = {
                agg_stats_data_keys.NUMERIC_AGGREGATE_MAP:
                    _convert_stats_to_dict(agg_stats.get("numeric_stats")),
                agg_stats_data_keys.CATEGORICAL_AGGREGATE_MAP:
                    _convert_stats_to_dict(agg_stats.get("categorical_stats")),
                agg_stats_data_keys.PREDICTION_AGGREGATE_MAP:
                    _convert_predictions_stats_to_dict(
...
def convert_aggregated_stats_features_to_dr_format(numeric_stat=None, category_stat=None):
    ...
    if numeric_stat:
        for feature_name, stat in six.iteritems(numeric_stat):
            feature_list.append(
                _convert_to_numeric_stat(feature_name, stat)
            )

    if category_stat:
        for feature_name, stat in six.iteritems(category_stat):
            feature_list.append(
                _convert_to_category_stat(feature_name, stat)
            )
