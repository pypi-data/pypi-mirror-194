...
from beni import bcolor, bexecute, bfile, bpath, binput
...
_mirror: Final = 'https://mirrors.aliyun.com/pypi/simple'
# _mirror:Final = 'https://pypi.douban.com/simple'
...
def venv(
    ...
    mirror: bool = typer.Option(True, help=f'使用镜像加速（{_mirror}）')
):
    '''python 虚拟环境配置'''
    ...
    async def pipInstall(pip: Path, file: Path):
        ...
        await bexecute.run(f'{python} -m pip install --upgrade pip {"-i " + _mirror if mirror else "" }')
        ...
        if mirror:
            cmdAry.insert(0, f'{pip} install -i {_mirror}')
