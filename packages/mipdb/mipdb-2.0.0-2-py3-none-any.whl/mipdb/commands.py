...
_db_configs_options = [
    cl.option(
        "--ip", "ip", required=False, default=os.getenv("DB_IP"), help="The ip of the database"
    ),
    cl.option(
        "--port",
        "port",
...
@db_configs_options
@handle_errors
def add_dataset(csv_path, data_model, version, copy_from_file, ip, port, username, password, db_name):
    print(f"CSV '{csv_path}' is being loaded...")
    dbconfig = get_db_config(ip, port, username, password, db_name)
    db = MonetDB.from_config(dbconfig)
...
@db_configs_options
@handle_errors
def validate_dataset(csv_path, data_model, version, copy_from_file, ip, port, username, password, db_name):
...
@db_configs_options
@handle_errors
def disable_dataset(dataset, data_model, version, ip, port, username, password, db_name):
    db = MonetDB.from_config(get_db_config(ip, port, username, password, db_name))
...
@db_configs_options
@handle_errors
def tag_data_model(name, version, tag, remove, force, ip, port, username, password, db_name):
    db = MonetDB.from_config(get_db_config(ip, port, username, password, db_name))
    if "=" in tag:
        key, value = tag.split("=")
        if remove:
...
@db_configs_options
@handle_errors
def tag_dataset(dataset, data_model, version, tag, remove, force, ip, port, username, password, db_name):
    db = MonetDB.from_config(get_db_config(ip, port, username, password, db_name))
    if "=" in tag:
        key, value = tag.split("=")
        if remove:
