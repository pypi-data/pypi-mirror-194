...
from sherpa_client.api.annotate import (
    annotate_format_binary_with_project_annotator,
    annotate_binary_with_project_annotator,
    annotate_documents_with_project_annotator,
    annotate_format_documents_with_project_annotator,
    annotate_format_text_with_project_annotator,
    annotate_text_with_project_annotator,
    annotate_corpus_with, annotate_binary_with_plan,
)
...
from sherpa_client.api.projects import get_projects, create_project, delete_project
...
from sherpa_client.models import (
    ...
    LaunchDocumentImportSegmentationPolicy, ShareMode, AnnotateBinaryWithProjectAnnotatorMultipartData,
)
...
class StreamlitSherpaClient:
    ...
    def annotate_format_text(
        ...
        long_client = self.client.with_timeout(1000)
        r = annotate_format_text_with_project_annotator.sync_detailed(
            pname, aname, text_body=text, client=long_client, parallelize=True
        )
    ...
    def annotate_binary(
        ...
        files = AnnotateBinaryWithProjectAnnotatorMultipartData(
            file=File(
        ...
        long_client = self.client.with_timeout(1000)
        r = annotate_binary_with_project_annotator.sync_detailed(
    ...
    def annotate_format_binary(
        ...
        files = AnnotateFormatBinaryWithProjectAnnotatorMultipartData(
            file=File(
                file_name=datafile.name,
                payload=datafile.getvalue(),
                mime_type=datafile.type,
        ...
        long_client = self.client.with_timeout(1000)
        r = annotate_format_binary_with_project_annotator.sync_detailed(
            pname, aname, multipart_data=files, client=long_client, parallelize=True
        )
        if r.is_success:
            return self._file_from_response(r)
        else:
    ...
    def convert_binary(
        ...
        long_client = self.client.with_timeout(1000)
        r = annotate_binary_with_plan.sync_detailed(
    ...
    def annotate_json(
        ...
        long_client = self.client.with_timeout(1000)
        r = annotate_documents_with_project_annotator.sync_detailed(
            pname, aname, json_body=documents, client=long_client, parallelize=True
        )
        if r.is_success:
            docs = r.parsed
        else:
    ...
    def annotate_format_json(
        ...
        long_client = self.client.with_timeout(1000)
        r = annotate_format_documents_with_project_annotator.sync_detailed(
            pname, aname, json_body=documents, client=long_client, parallelize=True
        )
        if r.is_success:
            return self._file_from_response(r)
    ...
    def delete_project(
        ...
        pname = project.name if isinstance(project, ProjectBean) else project
        r = delete_project.sync_detailed(
            pname,
            client=self.client)
        return r.is_success
