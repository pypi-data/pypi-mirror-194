...
def option_vte(args):
    ...
    if id != None:
        return VlocityTrackingEntry.query_and_print_error_record(id=id)

    VlocityTrackingEntry.print_error_list(limit=100)
def option_ipa(args):
    if '-h' in args: return
    if '-hu' in args: return
    ...
    ip_attachments.print_attachments(limit=100)

def option_ipe(args):
    if '-hu' in args:
        help = f"""
        {utils.CYELLOW}-ipe{utils.CEND} displays the last 50 errors.  {utils.CBLINK}NEW{utils.CEND}
    ...
    if ipe_id != None:
        return VlocityErrorLog.query_and_print_error_record(ipe_id)
