...
class FinanceTransactService:
    ...
    @staticmethod
    def finance_flow_writing(params, fund_type=None):
        ...
        user_finance_data = {}
        if fund_type == "RECHARGE":
            platform_finance_data['account_id'] = sub_appid  # 收款方
            platform_finance_data['their_account_name'] = params['user_id']  # 付款方
            finance_add_data, err_txt = FinanceTransactService.post(platform_finance_data)
            if err_txt:
                return None, err_txt
            user_finance_data['account_id'] = params['user_id']  #
            user_finance_data['their_account_name'] = sub_appid  # 付款方
            user_add_data, user_err_txt = FinanceTransactService.post(platform_finance_data)
            if user_err_txt:
            ...
            user_finance_data['account_id'] = params['user_id']  # 收款方
            user_finance_data['their_account_name'] = sub_appid  # 付款方
    ...
    def create(item):
        ...
        serializer.validated_data['snapshot'] = item.get('snapshot', None)
