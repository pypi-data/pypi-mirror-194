...
def export_ref_for_diff(kind, current, info, line):
    name = get_name(current, kind, info)
...
def export_ref_recursive(all_refs, id_to_filename, kind, current, info, root_filename):
    info = deepcopy(info)
    refs, root = export_ref(kind, current, info)
    for id, v in refs.items():
        if id not in all_refs:
            ...
            if name:
                info["name"] = name
            info["filename"] = f"{info['name']}.yaml"
            filename = layout.format(info)
...
def export_ref_wrap(kind, current, info, line):
    ...
    root = export_ref_recursive(
        all_refs, id_to_filename, kind, current, info, line)
    return root
...
def main(action, filename):
    ...
    if action == "create":
        if content.get("pipelineConfig", {}).get("no-apply"):
            print(f"file does not allow apply.\n{filename}")
            return
        ...
        current_expanded = {}
        if current:
            current_crd = export_ref_for_diff(kind, current, info, filename)
