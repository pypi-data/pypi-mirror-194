...
def get_themes():
    ...
    themes = {}
    eps = set(importlib_metadata.entry_points(group='docums.themes'))
    builtins = {ep.name for ep in eps if ep.dist.name == 'docums'}

    for theme in eps:

        if theme.name in builtins and theme.dist.name != 'docums':
            raise exceptions.ConfigurationError(
...
# `docums` logger get counted automatically.
