...
def api_request(
    ...
    try:
        r = requests.request(method, url, *args, verify=verify, cert=cert, **kwargs)
...
def setup(
    ...
    *,
    ...
    restapi = {**settings.restapi.dict(), **service.restapi.dict()}
    args.setdefault("restapi", restapi)
    args.setdefault("postgresql", {"use_pg_rewind": settings.use_pg_rewind})
...
def init(
    ...
    def wait_logfile(
        ...
        logfile = _logpath(instance.qualname, settings) / "patroni.log"
    ...
    with logstream(logf) as f:
        try:
            wait_ready(ctx, instance, patroni, service.settings, f)
...
def delete(ctx: "Context", service: Service) -> None:
    ...
    (_logpath(service.name, service.settings) / "patroni.log").unlink(missing_ok=True)
