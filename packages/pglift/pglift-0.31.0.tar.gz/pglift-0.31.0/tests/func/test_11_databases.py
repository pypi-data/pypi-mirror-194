...
def test_apply(
    ...
    database = interface.Database(
        ...
        extensions=[{"name": "hstore"}, "unaccent"],
    )
    assert not databases.exists(ctx, instance, database.name)
    ...
    assert db.extensions == [
        interface.Extension(name="hstore"),
        interface.Extension(name="unaccent"),
    ]

    assert databases.apply(ctx, instance, database).change_state is None  # no-op

    execute(ctx, instance, "CREATE SCHEMA my_schema", fetch=False, dbname="db2")
    database = interface.Database(
        owner="postgres",
        name="db2",
        settings={"work_mem": "1MB"},
        extensions=[{"name": "hstore", "schema": "my_schema"}, {"name": "unaccent"}],
    )
    assert (
        databases.apply(ctx, instance, database).change_state
...
def test_get(
    ...
    execute(ctx, instance, "CREATE SCHEMA my_schema", fetch=False, dbname="describeme")
    execute(
        ctx,
        instance,
    ...
    assert database.extensions == [
        interface.Extension(name="unaccent", schema="my_schema")
    ]
...
def test_alter(
    ...
    execute(ctx, instance, "CREATE SCHEMA my_schema", fetch=False, dbname="alterme")
    execute(
        ctx,
        instance,
        "CREATE EXTENSION unaccent WITH SCHEMA my_schema",
    ...
    assert databases.get(ctx, instance, "alterme") == database.copy(
        update={
            ...
            "extensions": [interface.Extension(name="unaccent", schema="my_schema")],
        }
    )
    ...
    databases.alter(ctx, instance, database)
    assert databases.get(ctx, instance, "alterme") == database._copy_validate(
        update={
            "settings": {"maintenance_work_mem": "9MB"},
            "extensions": [
                interface.Extension(name="pg_stat_statements", schema="my_schema"),
                "unaccent",
            ],
        }
    )
