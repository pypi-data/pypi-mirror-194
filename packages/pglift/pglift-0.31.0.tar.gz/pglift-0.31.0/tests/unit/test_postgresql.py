...
def test_libpq_environ(settings: Settings, instance: system.Instance) -> None:
    ...
    assert ctl.libpq_environ(
        instance, postgres_settings.auth, postgres_settings.surole.name, base={}
    ) == {"PGPASSFILE": str(postgres_settings.auth.passfile)}
    assert ctl.libpq_environ(
        instance,
        postgres_settings.auth,
...
def test_libpq_environ_password_command(
    instance: system.Instance, pg_version: str, tmp_path: Path
) -> None:
    ...
    assert ctl.libpq_environ(
        instance, settings.auth, settings.surole.name, base={}
    ) == {
