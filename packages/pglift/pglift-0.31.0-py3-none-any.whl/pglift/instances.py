...
def configuration(
    ...
    conf.format_values(confitems, manifest.name, ctx.settings.postgresql)
...
def upgrade(
    ...
    *,
    ...
    surole_password = libpq_environ(
        instance, postgresql_settings.auth, surole.name
    ).get("PGPASSWORD")
    if (
        not surole_password
    ...
    env = libpq_environ(
        instance, ctx.settings.postgresql.auth, ctx.settings.postgresql.surole.name
    )
    if surole_password:
...
def env_for(
    ...
    postgresql_settings = ctx.settings.postgresql
    env = libpq_environ(
        instance, postgresql_settings.auth, postgresql_settings.surole.name, base={}
    )
    config = instance.config()
    try:
