...
@hookimpl
def logrotate_config(ctx: "Context", instance: "system.PostgreSQLInstance") -> str:
    config = instance.config()
    log_directory = config.get("log_directory", "log")
    assert isinstance(log_directory, str), log_directory
    return util.template("postgresql", "logrotate.conf").format(
        name=instance.name,
        version=instance.version,
        log_directory=conf.log_directory(instance.datadir, Path(log_directory)),
    )
