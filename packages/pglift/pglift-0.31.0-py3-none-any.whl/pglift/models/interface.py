...
class _DatabaseExisting(BaseDatabase):
    ...
    extensions: List[Extension] = Field(
        ...
        examples=[{"name": "unaccent", "schema": "ext"}, "hstore"],
    )
    ...
    def __validate_extensions_(
        ...
        >>> Database(name="x", extensions=["named", {"name": "obj", "schema": "sch"}])  # doctest: +ELLIPSIS
        Database(name='x', ..., extensions=[Extension(name='named', schema_=None), Extension(name='obj', schema_='sch')], ...)
        """
...
class Instance(BaseInstance, CompositeManifest):
    """PostgreSQL instance"""
