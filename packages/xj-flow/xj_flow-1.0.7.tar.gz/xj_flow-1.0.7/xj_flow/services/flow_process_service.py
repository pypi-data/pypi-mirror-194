...
class FlowProcessService:
    ...
    def do_once_flow_in_service(self, flow_node_id, flow_action_id, source_params: dict = None):
        """
        根据流程节点调用配置好的服务方法，也可以执行表达式，脚本，默认字
        @param flow_node_id 流程节点ID
        @param flow_action_id 希望处理的流程动作ID
        ...
        if not flow_node_id and not flow_action_id:
        ...
        for item in flow_rule_list:
            ...
            if service:
                ...
                current_context["result"] = current_context["err_msg"] = "没有找到服务"
