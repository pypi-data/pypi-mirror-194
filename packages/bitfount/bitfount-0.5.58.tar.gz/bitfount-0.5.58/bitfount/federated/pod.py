...
class Pod:
    ...
    def __init__(
        ...
        # Establish views/sub-datasources
        self._datasources: Dict[str, Union[BaseSource, _ViewDatasourceConfig]] = {}
    ...
    def _update_pod_db(self) -> None:
        ...
        if self.datasource.multi_table:
            datasource = cast(MultiTableSource, self.datasource)
            ...
            _add_data_to_pod_db(
                pod_name=self.name,
                data=self.datasource.data.copy(),
                table_name=self.name,
            )
    ...
    def _get_target_datasource(self, message: _BitfountMessage) -> BaseSource:
        """Extract the view config associated with view requested."""
        # TODO: [BIT-2704] Merge extraction to work for both subsidiary and
        #                  "main" datasource(s).
        target_datasource = self._extract_requested_datasource(message)

        # Currently the pod has a concept of a "main" datasource (self.datasource)
        # and "subsidiary" datasources (self._datasources) for things like views.
        #
        # Post-datasource overhaul _all_ datasources will be located in
        # self._datasources and so there will be a minor overhaul needed for this
        # section to work with that new system.
        if target_datasource in self._datasources:
            target = self._datasources[target_datasource]
            if isinstance(target, _ViewDatasourceConfig):
                # If the target datasource is a view, build it using the "main"
                # datasource
                return target.build(self.datasource)
            else:
                # Otherwise, if not a view, must be a datasource itself
                return target

        # If target is not specified in self._datasources, must be the
        # "main" datasource
    ...
    def _extract_requested_datasource(self, message: _BitfountMessage) -> Optional[str]:
        ...
        Returns `None` if the requested datasource is the main datasource of the
        pod, otherwise the datasource name.
        """
        ...
        if recipient_mailbox_id == self.name:
            logger.info("Requested datasource was main pod datasource")
            return None
        else:
