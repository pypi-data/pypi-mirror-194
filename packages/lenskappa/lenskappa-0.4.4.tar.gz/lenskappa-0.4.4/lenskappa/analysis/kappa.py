...
def kappa_pdf(
        ...
        max_kappa = 0.4, kappa_bins = 1000, overwrite_wlm = False,
        *args, **kwargs
        ):
    ms_weights = load_ms_weights(ms_wnc)
    if "kappa" not in ms_weights.columns or overwrite_wlm:
        ms_weights = attach_wlm(ms_wnc, z_s, *args, **kwargs)
    ...
    p = pathlib.Path(lenskappa.__file__).parents[0]
    j = julia.Julia(compiled_modules=False)
    j.include(str(p / "analysis" / "kappa_jl.jl"))
