...
class StoreClient(BaseClient):
    ...
    def upload_many(
        ...
        with console.status(
            f"☁️  [green]Uploading {len(uris)} videos with {threads} threads ... "
        ) as status:  # noqa: F841
    ...
    def _upload_file(self, filename: str):
        ...
        with open(filename, "rb") as f:
            ...
            if response.status_code == 201:
                console.print(
                    f":heavy_check_mark: [green]Successfully uploaded (filename={filename})[/green]"
                )
                return response.json()
            else:
                console.print(
                    f":cross_mark: [red]Failed to upload (filename={filename}, error={response.text})[/red]"
                )
                return None
    ...
    def _upload_url(self, url: str):
        ...
        if urlparse(url).scheme in {"s3", "gs"}:
            try:
                console.print(f"🔐 [green]Generating pre-signed URL (url={url})[/green]")
                presigned_url = generate_presigned_url(url)
            except Exception as e:
                console.print(
                    f"[red]Failed to generate pre-signed URL (url={url}, error={e})[/red]"
                )
                return None
        ...
        if response.status_code == 201:
            url_str = f"url={url}" + (
                f", presigned_url={presigned_url}" if presigned_url else ""
            )
            console.print(
                f":heavy_check_mark: [green]Uploaded pre-signed URL ({url_str})[/green]"
            )
            return response.json()
        else:
            console.print(
                f"[red]Failed to upload pre-signed URL (url={url}, error={response.text})[/red]"
            )
            return None
