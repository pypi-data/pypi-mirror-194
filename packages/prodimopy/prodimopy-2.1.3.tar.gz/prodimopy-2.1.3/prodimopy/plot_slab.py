...
def plot_spectra(dat, normalise = False, fig=None, ax=None, overplot=False, add=False, cmap=None, colors=None, style='step', figsize=None, NLTE=False, label='', lw=1, c=None, scaling=1,sampling=1,offset=0):
    ...
    if add:
        ...
        fig,ax = _get_set_fig(ax,fig,figsize)
        fig,ax = _basic_spectra_plot(add_wave, add_flux,convR = dat[0].convR, sampling=sampling, scaling=scaling, normalise=normalise, fig=fig, ax=ax, style=style, figsize=figsize, label=label, color=c,lw=lw, where='mid')
        return(fig,ax)
    ...
    if overplot:
        for i,d in enumerate(dat):
            ...
            if NLTE:
                ...
                add_flux=d.convLTEflux
            fig_list[0],ax_list[0] = _basic_spectra_plot(add_wave, add_flux,convR= dat[i].convR,sampling=sampling, scaling=scaling_list[i], offset=offset_list[i],  normalise=normalise, fig=fig_list[0], ax=ax_list[0], style=style, figsize=figsize, label=label_list[i], color=color_list[i],lw=lw, where='mid')
        ...
        for i,d in enumerate(dat):
            ...
            if NLTE:
                ...
                add_flux=d.convLTEflux
            fig_list[i],ax_list[i] = _basic_spectra_plot(add_wave, add_flux,convR= dat[i].convR,sampling=sampling, scaling=scaling_list[i], offset=offset_list[i],  normalise=normalise, fig=fig_list[i], ax=ax_list[i], style=style, figsize=figsize, label=label_list[i], color=color_list[i],lw=lw, where='mid')
        if axx_len>1:
            return(fig_list[0],np.array(ax_list))
        else:
            return([(fig,ax) for (fig,ax) in zip(fig_list,ax_list)])
...
def _basic_spectra_plot(x, y, convR,sampling=1,normalise=False, fig=None, ax=None, scaling=1, offset=0.0, figsize=None, label='', color='k', style='step', lw=1, where='post'):
    if x is None or y is None:
