...
class NamespaceSerializer2(NamespaceSerializerBase):
    ...
    def _dump_python_object_unsafe(self, obj, variant, current_depth, config, log_errors):
        ...
        elif NamespaceSerializer2.is_bson(obj):
            self._dump_bson(obj, variant, current_depth, config, log_errors)
    ...
    def _dump_bson(self, obj, variant, current_depth, config, log_errors):
        if isinstance(obj, ObjectId):
            return self._dump_primitive(str(obj), str(type(obj)), variant, config.max_string)
    ...
    def _dump_user_class(self, obj, variant, current_depth, config, log_errors):
        ...
        if getattr(obj, '__slots__', None) is not None:
            items = obj.__slots__
