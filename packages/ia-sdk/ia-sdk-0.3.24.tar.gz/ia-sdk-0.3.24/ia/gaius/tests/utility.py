...
class Tester:
    ...
    def train(self, sequence):
        record = {'phase': 'training',
                  'historical_expecting': sum([sum(data['emotives'].values())
    ...
    def test(self, sequence):
        ...
        for answer in answers:
            for node, ensemble in answer.items():
                ...
                # Add to hive model's list for later
        ...
        if self.learning_strategy == 'continuous' or (
            ...
            # NOTE!: Unlike regular symbols for the classification tests,
            # utilities pass through all nodes, so we don't need to send
            # directly to egress nodes. Simply send through ingress nodes.
            data = {"strings": [],
                    "vectors": [],
                    "emotives": {'utility': historical_expecting}}
            self.bottle.observe(data)
            self.bottle.learn()
        return record
