...
class JSONRPCServer(ContextableService, ServiceOfServices, AbstractRPCCompatible):
    ...
    def get_context() -> RequestContext:
        ...
        if not ctx:
            ...
            ctx = RequestContext(correlation_id=b2a_hex(os.urandom(4)).decode(), session=None, request_deadline=None)
    ...
    def registered_routes(self):
        ...
        routes = [
            ...
            if method['permission'].value >= ctx['session'].scope.value
        ]
    ...
    async def _execute_single(
        ...
        REQUEST_CONTEXT.set(
            RequestContext(session=session, request_deadline=request_deadline, correlation_id=correlation_id)
        )
        try:
