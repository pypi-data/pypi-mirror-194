...
import pandas as pd
import datetime
from ..HELPERS import misc, pandas_ext
# from ..HELPERS.misc import parseCmdLine,groupOutput
# from ..HELPERS.pandas_ext import excel_date
...
def BDH(listSecID,listFieldID,listFieldScale,startDateYYYYMMDD,endDateYYYYMMDD,periodicityAdjustment="ACTUAL",periodicitySelection="DAILY",maxDataPoints=0,listOverrides = [],convertDatesToNumbers=False,oneTablePer="SECURITY",onlyCommonDates=True,return_as_dict=False):
    '''
    ...
    try:
        ...
        RESPONSE=blpapi.Event.RESPONSE
        PARTIAL_RESPONSE=blpapi.Event.PARTIAL_RESPONSE
                ...
                if (ev.eventType()==PARTIAL_RESPONSE) or (ev.eventType()==RESPONSE):
                    ...
                    try:
                        ...
                        for a in range(0,numDataRows):#each a "row of data", bbg calls them "fields"
                            ...
                            oneRow = [nan]*len(labels)
                            numFields = dataRow.numElements()
                            for b in range(0,numFields):
                                v=dataRow.getElement(b).getValue(0)
                                if type(v) is datetime.date and convertDatesToNumbers:
                                    v=pandas_ext.excel_date(v)
                                # oneRow.append(v)
                            ...
                            #print(row_list)
                        df=pd.DataFrame.from_records(securityData, columns=labels)
                        # df.set_index('date')
        ...
        allSecuritiesGrouped=misc.groupOutput(listSecID,listFieldID,listFieldScale,oneTablePer,allSecurities,onlyCommonDates)
...
def BDP_One_Field(listSecID,targetFieldName,overrideList,as_columns=True):
    ...
    try:
        ...
        session.stop()
        # return values
        output=pd.DataFrame.from_records(data=[fieldValues],columns=tickers, index=[targetFieldName])
        if as_columns==False:
...
def BDP_Multiple_Fields(listSecID,listFields,overrideList):
    ...
    try:
        ...
        session.stop()
        # return values
        output = pd.DataFrame.from_records(data=securityData, columns=listFields)
...
def BDP_Dividends(listSecID,overrideList):
    ...
    dataLineTemplate = pd.DataFrame(index=[None],columns= (['ticker']+dividendFields), data=None)
...
def generate_universe(date_list=["20220101","20220630","20221231"], private_screen_name="FTSE100", only_index_names = False):
    ...
    indexConstituents = pd.DataFrame(columns=allTickers)
    for (endDateYYYYMMDD,listSecID) in universeComposition:
        row_1 = pd.DataFrame(columns=listSecID, data=np.ones(shape=(1, len(listSecID))))
        indexConstituents = pd.concat([indexConstituents, row_1], axis=0, ignore_index=True)
    ...
    df = pd.DataFrame(columns=indexConstituents.columns)
    df_raw=df.copy()
    ...
    for (endDateYYYYMMDD,row) in indexConstituents.iterrows():
        ...
        startDateYYYYMMDD = (datetime.datetime.strptime(endDateYYYYMMDD, date_format) - datetime.timedelta(days=3)).strftime(date_format)
        maxDataPoints = 1
        ...
        df = pd.concat([df, response[0]], axis=0, ignore_index=True)
        df_raw=pd.concat([df_raw, raw_cap], axis=0, ignore_index=True)
