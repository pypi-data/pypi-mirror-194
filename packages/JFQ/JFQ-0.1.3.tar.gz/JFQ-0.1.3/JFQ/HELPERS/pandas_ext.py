...
def parse_dates_quickly_strict(s, formato = '%Y-%m-%d'):
    ...
    """
    dates = {date:pd.to_datetime(date,format=formato) for date in s.unique()}
    return s.map(dates)
...
def read_named_range_from_xlsx(xlsx_file, range_name, has_labels=True, first_col_index=False):
    ...
    if has_labels:
        ...
        df = pd.DataFrame(data=[[cell.value for cell in row] for row in region[1:]], columns=cols)
    else:
        df = pd.DataFrame(data=[[cell.value for cell in row] for row in region[1:]])
    # index
    if first_col_index:
...
def read_ADR_table(file_path='', range_name='',
    ...
    if (file_path == '') or (range_name == ''):
        adr = pd.read_clipboard(sep='\t', thousands=',')
    else:
