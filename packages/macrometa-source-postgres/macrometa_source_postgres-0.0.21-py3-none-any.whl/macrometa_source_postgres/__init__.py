...
def main_impl():
    ...
    config={}
    try:
        config = create_certficate_files(args.config)
        if args.config.get('ssl') == 'true':
            conn_config['sslmode'] = 'require'
            if config.get('ssl_root_ca_cert'):
                conn_config['sslrootcert'] = config['ssl_root_ca_cert']
            if config.get('ssl_client_certificate'):
                conn_config['sslcert'] = config['ssl_client_certificate']
                if config.get('ssl_client_key'):
                    conn_config['sslkey'] = config['ssl_client_key']
                if config.get('ssl_client_password'):
                    conn_config['sslpassword'] = config['ssl_client_password']
    ...
    def config(self) -> list[ConfigProperty]:
        ...
        return [
            ...
            ConfigProperty('ssl', 'Use SSL', ConfigAttributeType.STRING, False, False,
                           ...
                           default_value='false'),
            ConfigProperty('ssl_root_ca_cert', 'SSL/TLS CA Certificate', ConfigAttributeType.STRING, False, False,
            ...
            ConfigProperty('ssl_client_certificate', 'SSL/TLS Client Certificate', ConfigAttributeType.STRING, False, False,
            ...
            ConfigProperty('ssl_client_key', 'SSL/TLS Client Key', ConfigAttributeType.STRING, False, False,
                           description='Specific client key in PEM string format.',
                           placeholder_value="my_client_key"),
            ConfigProperty('ssl_client_password', 'SSL/TLS Client Password', ConfigAttributeType.STRING, False, False,
                           description='If the private key contained in the SSL Key is encrypted, users can provide a '
