...
class Dataset(SendableMixin, LoggingMixin):
    ...
    @transport.setter
    def transport(self,
                  transport: tp.transport.Transport = None) -> None:
        """
    ...
    def run(self,
        ...
        if self.dependency:
            ...
            self.dependency.run(force=force,
                                dry_run=dry_run,
                                **run_args)
            Quiet.state = quiet
            return
    ...
    def _run(self,
        ...
        for remote, lines in master_scripts.items():
            ...
            i += 1
            _scriptfile = TrackedFile(self.local_dir,
                                      remote,
                                      scriptname)
            self._master_scripts.append(_scriptfile)
            # newline='\n' is required to stop windows clients adding the \r\n
            self._logger.info(f"writing to master script at "
                              f"{_scriptfile.local}")
...
\n""")
    ...
    def fetch_results(self,
        ...
        for runner in self.runners:
            if present_runfiles[runner.resultfile.remote]:
                self.transport.queue_for_pull(os.path.split(
                                              runner.resultfile.remote)[1],
                                              runner.local_dir,
                                              os.path.split(
                                                  runner.resultfile.remote)[0])
    ...
    def _is_finished(self,
        ...
        for runner in self.runners:
            ...
            self._logger.debug(f'checking file {runner.resultfile.remote}. '
