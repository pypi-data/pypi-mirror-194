            ...
            if control_type is None:
                if self.model_name != "pix2pix":
                    ...
                    with torch.no_grad():
                        for n in range(n_iter):
                            ...
                            uc = model.get_learned_conditioning(negprompt, clip_skip)

                            if isinstance(prompts, tuple):
                                prompts = list(prompts)

                            # no need to apply the fix here, it's applied internally
                            c = torch.cat([model.get_learned_conditioning(prompt, clip_skip) for prompt in prompts])
                ...
                with torch.no_grad():
                    for n in range(n_iter):
                        ...
                        cond = {
                            "c_concat": [control],
                            "c_crossattn": [self.control_net_model.get_learned_conditioning(prompt, clip_skip)],
                        }
                        un_cond = {
                            "c_concat": [control],
                            "c_crossattn": [self.control_net_model.get_learned_conditioning(negprompt, clip_skip)],
                        }
