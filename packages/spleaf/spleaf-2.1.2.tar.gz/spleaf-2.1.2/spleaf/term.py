...
class _ESP_PKernel(SumKernel):
  ...
  def _calccoefs(self):
    self._eta2 = self._eta * self._eta
    self._f = 1 / (4 * self._eta2)
    # Compute one more Bessel function for derivative (see _kernel_param_back)
    self._bessel = ive(np.arange(self._nharm + 2), self._f)
    self._a = self._bessel[:-1].copy()
    self._a[0] /= 2
    self._deno = np.sum(self._a)
    self._a /= self._deno
...
class ESPKernel(ProductKernel):
  ...
  .. math:: k(\Delta t) = \sigma^2 k_\mathrm{ES}(\rho;\Delta t)
      \frac{I_0(f) + 2 \sum_{h=1}^{n_\mathrm{harm}}
        I_h(f) \cos\left(h \nu \Delta t\right)}{
        I_0(f) + 2 \sum_{h=1}^{n_\mathrm{harm}} I_h(f)},

  where :math:`\nu = \frac{2\pi}{P}`, :math:`f = \frac{1}{4\eta^2}`,
  :math:`I_h` are the modified Bessel functions of the first kind,
  and :math:`k_\mathrm{ES}` is an :class:`ESKernel` with scale :math:`\rho`.

  The rank of the ESP kernel is :math:`6 n_\mathrm{harm} + 3`.
  ...
  def __init__(self, sig, P, rho, eta, nharm=2):
    self._sig = sig
    self._P = P
    self._rho = rho
    self._eta = eta
    super().__init__(ESKernel(sig, rho), _ESP_PKernel(P, eta, nharm))
    self._param = ['sig', 'P', 'rho', 'eta']
