...
from spins2cpu.single import square_update
...
def run(file, init, X, Y, J, A, arrays_temperatures, nequilibrium, nworks):
    ...
    if p == 3:
        ...
        X_s = X // 2
        Y_s = Y // 2
        logging.info("{} {:<8} {} {:<8} {:<8} {:<8} {} {:<8}".format("init:", init, "parameters(meV):", Ja, Jb, Jc, "anisotropy(meV):", Aa))
        if init == "fm":
            ...
            for i in range(lav):
                t, Nw, Ew = square_update.iteration3(latt, X_s, Y_s, Ja, Jb, Jc, Aa, arrays_values[i], nequilibrium, nworks)
