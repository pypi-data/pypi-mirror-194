...
from spins2cpu.single import c2m_update
...
def run(file, init, X, Y, J, A, arrays_temperatures, nequilibrium, nworks):
    ...
    if p == 4:
        ...
        X_s = X // 2
        Y_s = Y // 2
        logging.info("{} {:<8} {} {:<8} {:<8} {:<8} {:<8} {} {:<8}".format("init:", init, "parameters(meV):", Ja, Jb, J0, J1, "anisotropy(meV):", Aa))
        if init == "fm":
            ...
            for i in range(lav):
                t, Nw, Ew = c2m_update.iteration3(latt, X_s, Y_s, Ja, Jb, J0, J1, Aa, arrays_values[i], nequilibrium, nworks)
