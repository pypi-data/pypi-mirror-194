...
from spins2cpu.mae import square_update
...
def run(file, init, X, Y, J, JX, JY, A, arrays_temperatures, nequilibrium, nworks):
    ...
    if p == 3:
        ...
        Jax, Jbx, Jcx = JX[0], JX[1], JX[2]
        Jay, Jby, Jcy = JY[0], JY[1], JY[2]
        Aa, Ab = A[0], A[1]
        X_s = X // 2
        Y_s = Y // 2
        logging.info("{} {:<8} {} {:<8} {:<8} {:<8} {} {:<8} {:<8}".format("init:", init, "parameters(meV):", Ja, Jb, Jc, "anisotropy(meV):", Aa, Ab))
        logging.info("{:<32}{:<8} {:<8} {:<8}".format("",Jax, Jbx, Jcx))
        logging.info("{:<32}{:<8} {:<8} {:<8}".format("",Jay, Jby, Jcy))
        if init == "fm":
            ...
            for i in range(lav):
                t, Nw, Ew = square_update.iteration3(latt, X_s, Y_s, Ja, Jb, Jc, Jax, Jbx, Jcx, Jay, Jby, Jcy, Aa, Ab, arrays_values[i], nequilibrium, nworks)
            ...
            latt[0,[1,3],:,:,2] = -1
            latt[1,[0,2],:,:,2] = -1
            m_ave0 = functions.Average(latt[0,[0,2],:,:,2] + latt[1,[1,3],:,:,2]) / 2
            m_ave1 = functions.Average(latt[0,[1,3],:,:,2] + latt[1,[0,2],:,:,2]) / 2
            logging.info("{:>16} {:>16.6} {:>16.6}".format(0, m_ave0, m_ave1))
            logging.info("{:>16} {:>16} {:>16} {:>18} {:>18} {:>16} {:>16}".format(
                "Temperature", "magnetism0(e)", "magnetism1(e)", "susceptibility0(e)", "susceptibility1(e)", "specific heat", "time(s)"))
            for i in range(lav):
                t, Nw, Ew = square_update.iteration3(latt, X_s, Y_s, Ja, Jb, Jc, Jax, Jbx, Jcx, Jay, Jby, Jcy, Aa, Ab, arrays_values[i], nequilibrium, nworks)
                arr_a, arr_b = Nw[:,[0,2,5,7]], Nw[:,[1,3,4,6]]
                m_ave0 = functions.Average(arr_a)
                m_ave1 = functions.Average(arr_b)
                s_ave0 = functions.Average2(arr_a)
                s_ave1 = functions.Average2(arr_b)
