...
from spins2cpu.ising import c2m_update
...
def run(file, init, X, Y, J, arrays_temperatures, nequilibrium, nworks):
    ...
    if p == 4:
        ...
        X_s = X // 2
        Y_s = Y // 2
        logging.info("{} {:<8} {} {:<8} {:<8} {:<8} {:<8}".format("init:", init, "parameters(meV):", Ja, Jb, J0, J1))
        if init == "fm":
            ...
            for i in range(lav):
                t, Nw, Ew = c2m_update.iteration3(latt, X_s, Y_s, Ja, Jb, J0, J1, arrays_values[i], nequilibrium, nworks)
            ...
            latt[0,[1,3]] = -1
            latt[1,[0,2]] = -1
            m_ave0 = functions.Average(latt[0,[0,2]] + latt[1,[1,3]]) / 2.0
            m_ave1 = functions.Average(latt[0,[1,3]] + latt[1,[0,2]]) / 2.0
            logging.info("{:>16} {:>16.6} {:>16.6}".format(0, m_ave0, m_ave1))
            logging.info("{:>16} {:>16} {:>16} {:>16} {:>16} {:>16} {:>16}".format(
                "Temperature", "magnetism0", "magnetism1", "susceptibility0", "susceptibility1", "specific heat", "time(s)"))
            for i in range(lav):
            ...
            latt[0,[2,3]] = -1
            latt[1,[1,2]] = -1
            m_ave0 = functions.Average(latt[0,[0,1]] + latt[1,[0,3]]) / 2.0
            m_ave1 = functions.Average(latt[0,[2,3]] + latt[1,[1,2]]) / 2.0
            logging.info("{:>16} {:>16.6} {:>16.6}".format(0, m_ave0, m_ave1))
            logging.info("{:>16} {:>16} {:>16} {:>16} {:>16} {:>16} {:>16}".format(
                "Temperature", "magnetism0", "magnetism1", "susceptibility0", "susceptibility1", "specific heat", "time(s)"))
            for i in range(lav):
