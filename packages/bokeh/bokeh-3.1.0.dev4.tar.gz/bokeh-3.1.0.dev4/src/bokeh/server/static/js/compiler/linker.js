...
class Linker {
    ...
    get_package(dir) {
        ...
        if ((0, sys_1.file_exists)(pkg_path))
            return JSON.parse((0, sys_1.read)(pkg_path));
        else
            return {};
    }
    resolve_export_map(dir, subpath) {
        const pkg = this.get_package(dir);
        const export_map = (pkg.exports ?? {});
        const path = (0, path_1.join)(dir, subpath);
        for (const [key, val] of Object.entries(export_map)) {
            if ((0, path_1.join)(dir, key) == path) {
                return (0, path_1.join)(dir, val);
            }
        }
        return null;
    }
    ...
    resolve_absolute(dep, parent) {
        for (const base of this.bases) {
            ...
            const [dir, ...rest] = dep.split(path_1.sep);
            if (rest.length >= 1) {
                const path = (0, path_1.join)(base, dir);
                if ((0, sys_1.directory_exists)(path)) {
                    const file = this.resolve_export_map(path, rest.join(path_1.sep));
                    if (file != null)
                        return file;
                }
            }
    ...
    new_module(file) {
        ...
        for (const [path, override] of this.overrides) {
            if (file.endsWith((0, path_1.normalize)(path))) {
                source = override;
                break;
            }
        }
...
`;
        ...
        if (changed) {
            ...
            if ((this.target != null && resolution == "ESM") || type == "json") {
                const { ES2022, ES2020, ES2017, ES2015 } = typescript_1.default.ScriptTarget;
                const target = (() => {
                    switch (this.target) {
