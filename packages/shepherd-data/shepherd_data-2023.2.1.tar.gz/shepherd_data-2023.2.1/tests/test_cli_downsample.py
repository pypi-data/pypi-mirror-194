...
def test_cli_downsample_file_full(data_h5_path):
    res = CliRunner().invoke(
        cli, ["-vvv", "downsample", "--ds-factor", "10", str(data_h5_path)]
    )
...
def test_cli_downsample_dir_full(data_h5_path):
    ...
    res = CliRunner().invoke(
        cli, ["-vvv", "downsample", "--ds-factor", "40", str(data_h5_path.parent)]
    )
    assert res.exit_code == 0
...
def test_cli_downsample_rate_file_full(data_h5_path):
    res = CliRunner().invoke(
        cli, ["-vvv", "downsample", "--sample-rate", "100", str(data_h5_path)]
    )
    assert res.exit_code == 0
...
def test_cli_downsample_rate_file_short(data_h5_path):
    res = CliRunner().invoke(
        cli, ["-vvv", "downsample", "-r", "200", str(data_h5_path)]
    )
    assert res.exit_code == 0
