...
try:
    ...
    class ExcelOut(BaseOut):
            ...
            if self._previous_table:
                ...
                if new_headers:
                    ...
                    for header in new_headers:
                        ...
                        self._set_cell(self._first_row_index, self._last_col_index, header)
                ...
                for header in self._headers:
                    self._last_col_index += 1
                    self._set_cell(self._first_col_index, self._last_col_index, header)
        ...
        def _on_append(self, row: list):
            ...
            for i in range(0, len(row)):
                if i >= len(self._headers):
                    ...
                    cell = self._set_cell(self._next_row_index, col_index, row[i])
            ...
            if self._unmanaged_col_indices:
                for col_index in self._unmanaged_col_indices:
                    cell = self._erase_cell(self._next_row_index, col_index, keep_formula=True)
                    self._apply_column_format(cell)
        ...
        def _set_cell(self, row_index: int, col_index: int, value) -> Cell:
        ...
        def _erase_cell(self, row_index: int, col_index: int, keep_formula = False) -> Cell:
            cell: Cell = self._worksheet.cell(row_index, col_index)
            cell.style = 'Normal'
        ...
        def _apply_column_format(self, cell: Cell|tuple(int,int)):
            if not self._column_formats:
                return

            if not isinstance(cell, Cell):
                row_index, col_index = cell
                cell = self._worksheet.cell(row_index, col_index)
