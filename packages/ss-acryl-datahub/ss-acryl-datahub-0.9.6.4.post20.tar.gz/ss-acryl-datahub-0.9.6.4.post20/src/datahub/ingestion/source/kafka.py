...
class KafkaSource(StatefulIngestionSourceBase):
    ...
    def get_workunits_internal(self) -> Iterable[MetadataWorkUnit]:
        ...
        topic_is_present = "mse-accounts.fact.subscription-updated.L2.0" in topics.keys()
        logger.info(f"1. topic mse-accounts.fact.subscription-updated.L2.0 is in list {topic_is_present}")
        logger.info(f"1. Topics list size = {len(topics.keys())}")
        #logger.info(f"Topics List = {topics}")
        extra_topic_details = self.fetch_extra_topic_details(topics.keys())
        #logger.info(f"Extra topics details = {extra_topic_details}")
        logger.info(f"2. Topics list size = {len(topics.keys())}")
        topic_is_present = "mse-accounts.fact.subscription-updated.L2.0" in topics.keys()
        logger.info(f"2. topic mse-accounts.fact.subscription-updated.L2.0 is in list {topic_is_present}")
        allowed_topics_counter = 0
        dropped_topics_counter = 0
        for t, t_detail in topics.items():
            allowed_topics_counter += 1
            logger.info(f"Scanning Topic => {t}")
            self.report.report_topic_scanned(t)
            logger.info(f"Topic {t} has been scanned!")
            
            if  "mse-accounts.fact.subscription-updated.L2.0" in t:
                logger.info(f"Topic {t} found!!! and is_allowed? {self.source_config.topic_patterns.allowed(t)}")
                
            if self.source_config.topic_patterns.allowed(t):
                t_is_allowed = t == "mse-accounts.fact.subscription-updated.L2.0"
                if t_is_allowed:
                    logger.info(f"Topic {t} is scanned and allowed")
                logger.info(f"Topic {t} is allowed")
                logger.info(f"3. Topics list size = {len(topics.keys())}")
                logger.info(f"scanned topics count => {allowed_topics_counter}")
                yield from self._extract_record(t, t_detail, extra_topic_details.get(t))
            else:
                dropped_topics_counter += 1
                self.report.report_dropped(t)
                logger.info(f"Topic {t} has been dropped... Counter is at {dropped_topics_counter}")
    ...
    def _extract_record(
        ...
        logger.info(f"Extracting record for topic = {topic}") # from debug
        ...
        if schema_metadata is not None:
            ...
            #logger.info(f"Topic '{topic}' has a schema registry!")
            #logger.info(f"Schema Metadata for topic '{topic}' => \n {schema_metadata}")
            #logger.info(f"dataset snapshot aspects => {dataset_snapshot.aspects}")
        #else:
        #    logger.info(f"Topic '{topic}' does not have a schema registry entry...")
        ...
        logger.info(f"Emitting MCE for topic {topic}")
        ...
        logger.info(f"Emitting MCPW for topic {topic}")
        for domain, pattern in self.source_config.domain.items():
            if pattern.allowed(dataset_name):
                logger.info(f"Sub branch: Pattern allowed for topic {topic}")
                domain_urn = make_domain_urn(
                    self.domain_registry.get_domain_urn(domain)
                )

        if domain_urn:
            logger.info(f"Adding domain '{domain_urn}' to entity WU '{dataset_urn}' FOR Topic = {topic}")
