...
class ConfluentSchemaRegistry(KafkaSchemaRegistryBase):
    ...
    def _get_subject_for_topic(self, topic: str, is_key_schema: bool) -> Optional[str]:
        logger.info("H1")
        logger.info(f"Topic =  {topic}, is_key_schema = {is_key_schema}")
        ...
        #logger.info(f"Subject_key = {subject_key} \nTopic_subject Map = {self.source_config.topic_subject_map}")
        ...
        #logger.info(f"Known_schema_registry_objects = {self.known_schema_registry_subjects}")
        for subject in self.known_schema_registry_subjects:
            #if subject.startswith(topic) and subject.endswith(subject_key_suffix):
            if subject == topic + subject_key_suffix:
                logger.info(f"Found the subject {subject}")
                return subject
        logger.info(f"could not find subject for Topic '{topic}")
        return None
    ...
    def get_schema_str_replace_confluent_ref_avro(
        ...
        for schema_ref in schema.references:
            ...
            if ref_subject in schema_seen:
                logger.info(f"Subject has been seen already... {ref_subject}")
                continue
            ...
            logger.info(f"Latest version for Ref-subject '{ref_subject}' is => {reference_schema.version}")
            logger.info(f"Reference Schema ID: {reference_schema.schema_id}")
            ...
            logger.info(f"H3 - SCHEMA_STR: {schema_str}")
        return schema_str
    ...
    def _get_schema_and_fields(
        ...
        if topic_subject is not None:
            ...
            try:
                ...
                logger.info(f"Latest version for topic-subject '{topic_subject}' is => {registered_schema.version}")
                logger.info(f"Registered Schema ID: {registered_schema.schema_id}")
            except Exception as e:
                logger.warning(
            ...
            logger.info(f"is_key_schema = {is_key_schema}")
        ...
        logger.info(f"H5d: Fields: {fields}")
    ...
    def get_schema_metadata(
        ...
        logger.info("H9")
        logger.info(f"Inside _get_schema_metadata {topic} {platform_urn}") #from debug
        # Process the value schema
        schema, fields = self._get_schema_and_fields(
            topic=topic, is_key_schema=False
        )  # type: Tuple[Optional[Schema], List[SchemaField]]
        ...
        logger.info(f"H9b: schema  = None {schema is None} \t key_schema = None {key_schema is None}")
        # Create the schemaMetadata aspect.
        if schema is not None or key_schema is not None: 
            logger.info("H10")
            # create a merged string for the combined schemas and compute an md5 hash across
            ...
            logger.info(f"H10b: topic: {topic}")
            logger.info(f"H10c: schema_as_str {schema_as_string}")
            return SchemaMetadata(
                schemaName=topic,
                version=0,
                hash=md5_hash,
