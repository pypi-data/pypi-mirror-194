...
class Resolver(BaseModel, T.Generic[NodeType, InsertType, PatchType], metaclass=Meta):
        ...
        *,
        ...
        if include_changes:
            ...
            select {{ 
                ...
                _before := {{ {', '.join([f'{k} := before.{k}' for k in changes_keys])} }},
                _after := {{ {', '.join([f'{k} := after.{k}' for k in changes_keys])} }}
            }}
            """
        else:
    ...
    async def _update_one(
        ...
        *,
        ...
        raw_response = T.cast(RAW_RESP_ONE, raw_response)
        if not raw_response:
            raise errors.ObjectNotFound("No object to update.")
        return self.parse_obj_with_cache(raw_response)
    ...
    async def _update_one_with_changes(
        ...
        *,
        ...
        # now make changes d
        before_d, after_d = raw_response["_before"], raw_response["_after"]
        changes: CHANGES = {
            field_name: (before_d[field_name], after_d[field_name])
            for field_name in before_d.keys()
        }
        return n, changes
    ...
    async def _delete_one(
        ...
        *,
        ...
        raw_response = T.cast(RAW_RESP_ONE, raw_response)
        if not raw_response:
            raise errors.ObjectNotFound("No object to delete.")
        return self.parse_obj_with_cache(raw_response)
