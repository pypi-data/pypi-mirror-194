...
def build_object_types_query_str(global_objects: set[str]) -> str:
    ...
    } filter contains(.name, 'default::') and .abstract = false
    """
    if global_objects:
        inner = [f"'default::{o}'" for o in global_objects]
...
async def introspect_objects(
    client: edgedb.AsyncIOClient, global_objects: set[str]
) -> T.List[ObjectType]:
    object_types_raw = await execute.query(
        client=client,
        query_str=build_object_types_query_str(global_objects=global_objects),
        only_one=False,
    )
