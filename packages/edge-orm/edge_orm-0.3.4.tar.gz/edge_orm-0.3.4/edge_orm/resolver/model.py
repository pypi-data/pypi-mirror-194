...
class Resolver(BaseModel, T.Generic[NodeType, InsertType, PatchType], metaclass=Meta):
    ...
    async def insert_one(
        ...
        *,
        client: edgedb.AsyncIOClient | None = None,
        global_auth_id: str | None = None,
        upsert_given_conflict_on: str = None,
        custom_conflict_on_str: str = None,
        return_model_for_conflict_on: str = None,
        mutate_on_update: bool = True,
    ) -> NodeType:
        if client and global_auth_id:
            raise errors.ResolverException(
                "Cannot give both a client and a global_auth_id"
            )
        ...
        if not client:
            client = self._node_config.client
            if global_auth_id:
                client = client.with_globals(current_user_auth_id=global_auth_id)
        ...
        *,
        ...
        if include_permissions:
            ...
            with
                ...
                after := (delete before),
            select {{
                after := {select_s.replace('SELECT model', '', 1)},
                _updated := exists after,
                _exists := exists before
            }}
            """
