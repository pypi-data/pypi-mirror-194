...
class Resolver(BaseModel, T.Generic[NodeType, InsertType, PatchType], metaclass=Meta):
    ...
    async def query(
        ...
        if has_permission_str:
            ...
            first_part = query_str[:first_brace_index]
            second_part = query_str[first_brace_index + 1 : last_brace_index]
            third_part = query_str[last_brace_index + 1 :]

            query_str = f"""
            with
                __has_permission := {has_permission_str},
                __models := ({query_str}) if __has_permission else <{self.model_name}>{{}},
                select {{
