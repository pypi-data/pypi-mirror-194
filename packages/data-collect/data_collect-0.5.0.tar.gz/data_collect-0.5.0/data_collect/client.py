...
class DataCollectClient:
    ...
    def log(self,
        ...
        if image is not None and self.input_enable:
            self._blob_logger.log(message, image)

        if blob is not None and self._is_data_capture_enable():
            self._log_image(correlation_id, blob, designation)

        if self._is_data_capture_enable() and len(message) > 0:
            self._log_blob(correlation_id, image, designation)
    ...
    def _gen_blob_save_path(self, correlation_id: str, metadata: Dict = None):
        ...
        extension = metadata.get("extension") if metadata is not None and metadata.get("extension") is not None else "bin"
