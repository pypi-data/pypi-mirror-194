...
class DataCollectClient:
    ...
    def __init__(self, config):
        ...
        asyncio.run(self._init_save_image_task())
    ...
    def _log_image(self,
        ...
        if "PIL" in sys.modules and _is_pillow_image(image):
            ...
            await self.queue.put({"tmp_path": tmp_path, "image": image})
    ...
    async def consumer_image(self):
        while True:
            msg = await self.queue.get()
            cv2.imwrite(msg['tmp_path'], msg['image'])
            self.queue.task_done()

    async def _init_save_image_task(self):
        asyncio.create_task(self.consumer_image())
