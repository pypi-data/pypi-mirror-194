...
class DataCollectClient:
    ...
    def _gen_image_save_path(self, correlation_id: str, designation: str = '', metadata=None) -> str:
        ...
        if metadata.get('extension'):
            file_name = f"{dir_name}/{correlation_id}.{metadata.get('extension')}"
        else:
    ...
    def _log_image(self,
        ...
        """
        def _covert_image_mode(x):
            if PIL.Image.fromarray(x).mode != 'RGB':
                x = x[..., ::-1]
            return x
        ...
        if "PIL" in sys.modules and isinstance(image, PIL.Image.Image):
            image = np.asarray(image)
            image = _covert_image_mode(image)
            self._thread_executor.submit(cv2.imwrite, tmp_path, image)
        elif "numpy" in sys.modules and isinstance(image, np.ndarray):
            ...
            image = _covert_image_mode(image) # BGR to RGB
            self._thread_executor.submit(cv2.imwrite, tmp_path, image)

        elif isinstance(image, WindmillImage):
    ...
    def _log_blob(self, correlation_id: str, blob: WindmillBlob):
        tmp_path = self._gen_blob_save_path(correlation_id, blob.metadata.get('extension'))
