...
class SoPZeroMQClient:
    def __init__(self, host: str, port: int, ssl_ca_path: str = None, ssl_enable: bool = False,
                 network_type: SoPNetworkType = SoPNetworkType.MQTT) -> None:
        self.host = host
        self.port = port
    ...
    def unsubscribe(self, topic_filter):
        self.socket.setsockopt_string(zmq.UNSUBSCRIBE, topic_filter)
...
class SoPBigThing(SoPThing):
    def __init__(self, name: str, service_list: List[SoPService], alive_cycle: float, is_super: bool = False, is_parallel: bool = True,
        ...
        if append_mac_address:
            ...
            pass
        self._mqtt_client = MQTTClient(userdata=None, client_id=self._name)
...
# ===================================================================================
    ...
    def _on_connect(self, client: MQTTClient, userdata, flags, result):
        if result == 0:
    ...
    def _on_disconnect(self, client: MQTTClient, userdata, rc):
        if rc == 0:
            self._connected = False
            SOPLOG_DEBUG(
    ...
    def _on_subscribe(self, client: MQTTClient, userdata: str, mid, granted_qos):
        pass

    def _on_unsubscribe(self, client: MQTTClient, userdata: str, mid):
        pass

    def _on_publish(self, client: MQTTClient, userdata: mqtt.MQTTMessage, mid):
        pass

    def _on_message(self, client: MQTTClient, userdata: Callable, msg: mqtt.MQTTMessage):
        self._print_packet(msg, mode=self._log_mode)
        self._receive_queue.put(msg)

    # TODO: test this functions
    # for MQTT version>=5.0
    def _on_connect_v5(self, client: MQTTClient, userdata, flags, reason, properties):
        if reason == 0:
            self._connected = True
            SOPLOG_DEBUG(
                f'{PrintTag.CONNECT} Connect to Host: {self._ip}:{self._port}')
    ...
    def _on_disconnect_v5(self, client: MQTTClient, userdata, rc, properties):
        if rc == 0:
            self._connected = False
            SOPLOG_DEBUG(
                f'{PrintTag.DISCONNECT} Disconnect from Host: {self._ip}:{self._port}')
    ...
    def _on_subscribe_v5(self, client: MQTTClient, userdata: str, mid, reasoncodes, properties):
        pass

    def _on_unsubscribe_v5(self, client: MQTTClient, userdata: str, mid,  properties, reasoncodes):
        pass

    def _on_publish_v5(self, client: MQTTClient, userdata: mqtt.MQTTMessage, mid):
        pass

    def _on_message_v5(self, client: MQTTClient, userdata: Callable, msg: mqtt.MQTTMessage):
        self._print_packet(msg, mode=self._log_mode)
        self._receive_queue.put(msg)
