...
class ETSFormer(nn.Module):
    def __init__(
        ...
        self.embed = InputEmbedding(time_features, d_model, kernel_size=embed_kernel_size, dropout=dropout)
        ...
        for ind in range(layers):
            ...
            self.encoder_layers.append(nn.ModuleList([
                ...
                MHESA(dim=d_model, heads=n_heads, dropout=dropout),
                FeedForwardBlock(dim=d_model) if not is_last_layer else None,
                Level(time_features=time_features, model_dim=d_model)
            ]))

        self.growth_dampening_module = GrowthDampening(dim=d_model, heads=n_heads)

        self.latents_to_time_features = nn.Linear(d_model, time_features)
    ...
    def forward(
        ...
        forecasts = level + self.latents_to_time_features(summed_latents)

        if one_time_feature:
            forecasts = rearrange(forecasts, 'b n 1 -> b n')

        if return_latents:
            return forecasts, latents
