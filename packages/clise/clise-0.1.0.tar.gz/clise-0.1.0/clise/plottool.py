...
def wrap(plt, xr=None, yr=None, xlabel=None, ylabel=None, fig=None,
	...
	if grid is not None:
		if type(grid) is bool:
			if grid is True:
				ax.grid(True)
				if ax2 is not None: ax2.grid(False)
				if ay2 is not None: ay2.grid(False)
			else:
				ax.grid(False)
				if ax2 is not None: ax2.grid(False)
				if ay2 is not None: ay2.grid(False)
...
def prep_data_deco(func):
	...
	def prep_data(*args, xdata=None, ydata=None, weights=None,
				...
				verbose=0, ftype=None, hdu=None, help=None, **kwargs):
		...
		if xdata is None and loaded is None:
			...
			if infile2 is None:
				data, x, y, xlabel, ylabel = read(infile, x=x, y=y, data=data,
							xlabel=xlabel, ylabel=ylabel, ftype=ftype, hdu=hdu, attr=attr)
				if data is None: return False
				data2 = data
			else:
				data, x, y_, xlabel, ylabel_ = read(infile, x=x, y=None, data=data,
							xlabel=xlabel, ylabel=None, ftype=ftype, hdu=hdu, attr=attr)
				data2, x_, y, xlabel_, ylabel = read(infile2, x=None, y=y,
					xlabel=None, ylabel=ylabel, ftype=ftype, hdu=hdu, attr=attr)
			ydata = data2[y]
