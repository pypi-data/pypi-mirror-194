...
from itkdb.utilities import get_mimetype, sizeof_fmt

log = logging.getLogger(__name__)


class BinaryFile:
    def __init__(self, fp: IO[bytes], suggested_filename=None, mimetype=None):
        self.fp = fp
    ...
    def extension(self) -> str | None:
        ...
        if self.suggested_filename and "." in self.suggested_filename:
            ...
            if self.mimetype != self.content_type:
                log.warning(
                    f"MIME type specified '{self.mimetype}' does not match what the content looks like '{self.content_type}'. Will return extension based off provided MIME type."
                )
    ...
    def content_type(self) -> str:
        return get_mimetype(None, self.fp)
    ...
    def save(self, filename=None, mode="wb") -> int:
        ...
        log.info(f"Written {self.size_fmt} ({self.size:d} bytes) to {new_filename!s}")
        return self.size
    ...
    def from_response(cls, response) -> Self:
        ...
        fp.seek(0)
        return cls(fp, suggested_filename=filename, mimetype=mimetype)
...
class ZipFile(zipfile.ZipFile):
    ...
    def save(self, filename=None, mode="wb") -> int:
        return self._file.save(filename=filename, mode=mode)
