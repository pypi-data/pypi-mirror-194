...
def get_sequencer_config_format(is_qrm_type: bool) -> str:
    ...
    seq_proc_cfg_format = "?IIIIIIIIIIIIIIII???????????????"
    awg_cfg_format = "??IIIIiiiiI?IiiiiBBI?i"
    awg_float_cfg_format = "ddffffff"
    acq_cfg_format = "???IIIiiiq??I??I"
...
def set_sequencer_config(funcs: FuncRefs, sequencer: int, config: Dict) -> None:
    ...
    for x in range(1, 16):
        cfg[1 + x] = cfg_dict["trg{}_count_threshold".format(x)]
        cfg[16 + x] = cfg_dict["trg{}_threshold_invert".format(x)]
    ...
    if funcs.is_qrm_type():
        cfg += [
            ...
            cfg_dict["th_acq_trg_map_en"],            # Enable thresholded acquisition mapping to TNN
            cfg_dict["th_acq_trg_map_inv"],          # Thresholded acquisition TNN address
            cfg_dict["th_acq_trg_map_addr"],           # Thresholded acquisition result inversion (TNN)
            cfg_dict["th_acq_mrk_map_en"],            # Enable thresholded acquisition mapping to markers
            cfg_dict["th_acq_mrk_map_inv"],          # Thresholded acquisition marker map address
            cfg_dict["th_acq_mrk_map_addr"],           # Thresholded acquisition result inversion (markers)
...
def get_sequencer_config(funcs: FuncRefs, sequencer: int) -> Dict:
    ...
    cfg_dict = {
        ...
        # AWG
        "cont_mode_en_awg_path_0":           cfg[32],
        "cont_mode_en_awg_path_1":           cfg[33],
        "cont_mode_waveform_idx_awg_path_0": cfg[34],
        "cont_mode_waveform_idx_awg_path_1": cfg[35],
        "upsample_rate_awg_path_0":          cfg[36],
        "upsample_rate_awg_path_1":          cfg[37],
        "mod_en_awg":                        cfg[49],
        "mrk_ovr_en":                        cfg[50],
        "mrk_ovr_val":                       cfg[51],
        "nco_prop_delay_comp_en":            cfg[52],
        "nco_prop_delay_comp":               cfg[53],
        # AWG floating point values
        "freq_hz":                              cfg[54],
        "phase_offs_degree":                    cfg[55],
        "gain_awg_path_0_float":                cfg[56],
        "gain_awg_path_1_float":                cfg[57],
        "offset_awg_path_0_float":              cfg[58],
        "offset_awg_path_1_float":              cfg[59],
        "mixer_corr_phase_offset_degree_float": cfg[60],
        "mixer_corr_gain_ratio_float":          cfg[61],
    }

    for x in range(1, 16):
        cfg_dict["trg{}_count_threshold".format(x)] = cfg[1 + x]
        cfg_dict["trg{}_threshold_invert".format(x)] = cfg[16 + x]
    ...
    if funcs.is_qrm_type():
        cfg_dict = {**cfg_dict, **{
            ...
            "demod_en_acq":                cfg[62],
            "ttl_in":                      cfg[63],
            "ttl_auto_bin_incr_en":        cfg[64],
            "upsample_rate_acq_path_0":    cfg[65],
            "upsample_rate_acq_path_1":    cfg[66],
            "non_weighed_integration_len": cfg[67],
            "th_acq_trg_map_en":           cfg[72],
            "th_acq_trg_map_inv":          cfg[73],
            "th_acq_trg_map_addr":         cfg[74],
            "th_acq_mrk_map_en":           cfg[75],
            "th_acq_mrk_map_inv":          cfg[76],
            "th_acq_mrk_map_addr":         cfg[77],

            # Acquisition floating point values
            "rotation_matrix_a11": cfg[78],
            "rotation_matrix_a12": cfg[79],
            "ttl_threshold":       cfg[80],
            "discr_threshold":     cfg[81]},
        }
