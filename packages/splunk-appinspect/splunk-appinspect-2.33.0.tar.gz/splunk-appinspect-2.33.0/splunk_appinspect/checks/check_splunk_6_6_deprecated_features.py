    ...
    def check_config(self, app, config):
        for section in config["outputs"].sections():
            if config["outputs"].has_option(section.name, "autoLB"):
                yield FailMessage("Removed support for setting autoLB in outputs.conf,"
...
@splunk_appinspect.tags(
    "splunk_appinspect", "splunk_6_6", "removed_feature", "cloud", "private_app", "private_victoria", "private_classic"
)
@splunk_appinspect.cert_version(min="1.7.1")
