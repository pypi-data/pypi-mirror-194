    ...
    def check_config(self, app, config):
        ...
        for section in config["web"].sections():
            if section.name.startswith("endpoint:"):
                ...
                yield WarningMessage(reporter_output,
                                file_name=file_path)
...
def check_for_advanced_xml_module_elements(app, reporter, target_splunk_version):
    ...
    default_views_dir = os.path.join(app.app_dir, "default", "data", "ui", "views")
    local_views_dir = os.path.join(app.app_dir, "local", "data", "ui", "views")
    xml_views = list(app.get_filepaths_of_files(basedir=default_views_dir, types=[".xml"]))
    xml_views += list(app.get_filepaths_of_files(basedir=local_views_dir, types=[".xml"]))
    ...
    for xml_view in xml_views:
        findings = util.find_xml_nodes_usages([xml_view], [check_routine.xml_node("module")])
        if len(findings) == 0:
        ...
        if not found_sideview:
            ...
            finding_action(reporter_output, xml_view[0])
