...
* Call `alac.config.inputs("/pdf/dir/")` and `alac.config.outputs("/to/table.xlsx")` to configure your input and output paths. Then call `alac.config.set(input_conf, output_conf, **kwargs)` to complete the configuration process. Feed the output to any of the `alac.parse...()` functions to start a task.
