...
class ConsoleApi:
    ...
    def __init__(self):
        self.config = self.pega_dados_configuracao()
        self.url = self.config['url_console']
    ...
    def login(self, _user='', _pass=''):
        try:
            ...
            payload = {"username": f"{self.config['username']}", "password": f"{self.config['password']}", "client_id":'executor'}
    ...
    def pega_dados_configuracao(self):
        try:
            dir_json = os.getcwd() + '/config_ex.json'
