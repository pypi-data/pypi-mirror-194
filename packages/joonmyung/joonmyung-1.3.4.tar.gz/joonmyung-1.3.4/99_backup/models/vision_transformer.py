...
class Block(nn.Module):
    ...
    def forward(self, x):
        [x_, info] = x
        x, attn = self.attn(self.norm1(x_))
        x = x_ + self.drop_path1(self.ls1(x))
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))
        if "encoder_attn" not in info.keys():
            info["encoder_attn"] = [attn]
        else:
            info["encoder_attn"].append(attn)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            x, _ = checkpoint_seq(self.blocks, [x, self.info])
        else:
            x, _ = self.blocks([x, self.info])
