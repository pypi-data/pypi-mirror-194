...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, input_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=48, depth=12,
        ...
        self.decoder_token_num = kwargs["mae_task_type"][4]
        self.mae_avg_type      = kwargs["mae_task_type"][3]
    ...
    def forward_encoder(self, x, analysis=False):
        ...
        if analysis: self.info["encoder_attn"] = attns
        x = self.norm(x)
    ...
    def forward_decoder(self, x, cls_tokens, unmix_tokens, mask, mask_p, mask_n):
        ...
        pred = self.decoder_pred(patch_tokens) # (200(B*2), 16(T), 512(D))

        return avg_tokens, cls_tokens, unmix_tokens, decoder_tokens, pred

    def forward_mae_loss(self, imgs, pred, unmix_tokens):
        B = imgs.shape[0]
        ...
        if self.mae_task_type[2]:
            _, T, D = pred.shape
            pred_AB = pred.reshape(2, -1, T, D)
            unmix_tokens_AB = unmix_tokens.reshape(2, B, self.unmix_token_num, -1)
            target_A1 = torch.stack([target, target.flip(0)], dim=0)
            target_A2 = torch.stack([target.flip(0), target], dim=0)
        ...
        self.info["mae_pred_1"] =  self.unpatchify(pred[:B])
        self.info["mae_pred_2"] =  self.unpatchify(pred[B:])
        ...
        return loss_mae, unmix_tokens


    def forward(self, x, mask=None, mask_p = None, mask_n = None, imgs=None, analysis=False):
        result, self.info = {}, {}
        ...
        if not (self.training or analysis):
            return output_all
        elif mask is not None:
            if self.mae_task_type[1]:
                avg_tokens, cls_tokens, unmix_tokens, decoder_tokens, pred = self.forward_decoder(x, cls_token, unmix_token, mask, mask_p, mask_n)
                loss_mae, unmix_tokens = self.forward_mae_loss(imgs, pred, unmix_tokens)
                result["loss_mae"] = loss_mae

                if self.mae_task_type[2] and unmix_tokens.shape[1]:
                ...
                if self.mae_task_type[3] and avg_tokens.shape[1] and not self.mae_task_type[4]:
                ...
                if self.mae_task_type[4] and decoder_tokens.shape[1]:
            ...
            if self.mae_task_type[5]:  # Part Classification Loss
                result["output_part"] = self.head(torch.cat([x_1, x_2], dim=0))

            if self.mae_task_type[6]:
