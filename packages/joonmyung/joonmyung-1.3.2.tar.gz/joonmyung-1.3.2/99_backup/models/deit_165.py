...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, input_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=48, depth=12,
        ...
        self.mixed_target_type = kwargs["mixed_target_type"]
        self.info = {}
        self.analysis = False
    ...
    def forward_encoder(self, x):
        ...
        if self.mae_task_type[4] or self.analysis: self.info["encoder_attn"] = attns
        x = self.norm(x)
    ...
    def forward_decoder(self, x, mask):
        ...
        mask_tokens_1 = (x * mask).mean(dim=1, keepdim=True).expand(-1, x.shape[1], -1)
        mask_tokens_2 = (x * (1 - mask)).mean(dim=1, keepdim=True).expand(-1, x.shape[1], -1)
        mask_tokens = torch.cat([mask_tokens_1, mask_tokens_2], dim=0)
        x = x.repeat(2,1,1) + self.decoder_pos_embed
    ...
    def forward_mae_loss(self, imgs, pred):
        B = imgs.shape[0]
        ...
        loss_mae = ((pred[:B] - target) ** 2).mean() + ((pred[B:] - target.flip(0)) ** 2).mean()
        mae_pred_1 = self.unpatchify(pred[:B])
        mae_pred_2 = self.unpatchify(pred[B:])
        # drawImgPlot(unNormalize(torch.stack([mae_pred_1, mae_pred_2], dim=1).reshape(-1, 3, 224, 224)[:6].detach(), "imagenet"), col=2)
        self.info["mae_pred_1"] = mae_pred_1
        self.info["mae_pred_2"] = mae_pred_2
        return loss_mae


    def forward(self, x, mask=None, imgs=None, targets=None):
        output_part, loss_part_consis, loss_mae, self.info = None, torch.tensor([0], device=x.device), torch.tensor([0], device=x.device), {}

        x = self.forward_encoder(x)
        output_all = self.head(x.mean(dim=1))
        if not (self.training or self.analysis):
            return output_all
        elif mask is not None:
            x_1, x_2 = (x * mask).mean(dim=1), (x * (1 - mask)).mean(dim=1)
            if self.mae_task_type[1]:
                output_part = self.head(torch.cat([x_1, x_2], dim=0))

            if self.mae_task_type[2]:
                loss_part_consis = ((x_1 - x_2.flip(0)) ** 2).mean()

            if self.mae_task_type[3]:
                pred = self.forward_decoder(x, mask)
                loss_mae = self.forward_mae_loss(imgs, pred)

            if self.mae_task_type[4]:
                attn = rollout_our(self.info["encoder_attn"], "mean", starts=[0]) # (32, 196(Q), 196(K))
                # targets = torch.eye(self.num_classes, device=x.device)[targets]
                if self.mixed_target_type == 1:
                    attn_A = ((attn * mask * mask.transpose(1, 2)).sum(dim=[1, 2]) / mask.sum(dim=[1, 2])).unsqueeze(-1)
                    attn_B = ((attn * (1 - mask) * (1 - mask).transpose(1, 2)).sum(dim=[1, 2]) / (1 - mask).sum(dim=[1, 2])).unsqueeze(-1)
                    self.info["targets"] = targets * attn_A / (attn_A + attn_B) + targets.flip(0) * attn_B / (attn_A + attn_B)
                elif self.mixed_target_type == 2:
                    attn_A = (attn * mask.transpose(1, 2)).sum() / mask.sum()
                    attn_B = (attn * (1 - mask) * (1 - mask).transpose(1, 2)).sum() / (1 - mask).sum()
                    self.info["targets"] = targets * attn_A / (attn_A + attn_B) + targets.flip(0) * attn_B / (attn_A + attn_B)



        return output_all, output_part, loss_part_consis, loss_mae, self.info
