...
class DBaaSServices:
    ...
    def select_a_software(self, api_key, api_auth_token):
        ...
        while(not is_int(software_id) or not 0<int(software_id)<=len(software_engines)):
    ...
    def select_a_template(self, software_id, api_key, api_auth_token):
        ...
        while(not is_int(template_id_choice) or not 0<int(template_id_choice)<=len(template_engines)):
            Py_version_manager.py_print("Please select one of above only")
