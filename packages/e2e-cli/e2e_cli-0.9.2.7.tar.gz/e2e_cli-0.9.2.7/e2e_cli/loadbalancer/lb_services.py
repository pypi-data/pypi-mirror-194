...
class LBServices:
    ...
    def choice_nodes(self, nodes_assigned_to_lb, node_list_dict):
        ...
        while( not is_int(node_choice) or not int(node_choice) in node_list_dict): 
    ...
    def add_lb(self):
        ...
        if api_key_credentials_object["message"] == "Invalid alias provided":
            ...
            while (not is_int(choice_of_lb) or not 0<int(choice_of_lb)<=2):
                Py_version_manager.py_print("please select out of the given choices only")
                choice_of_lb = Py_version_manager.py_input(": ")
            ...
            while (not is_int(plan_choice) or not 0<int(plan_choice)<=len(lb_plans)):
                Py_version_manager.py_print("please select out of the given choices only")
                plan_choice = Py_version_manager.py_input(": ")
    ...
    def delete_lb(self):
        ...
        while (all_lb_in_list is None or not is_int(lb_id)):
