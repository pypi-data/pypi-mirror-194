...
class MemoryObject:
    ...
    def _initialize(self, deserialized_obj: object = None, comparable: Optional[bool] = None) -> None:
        self.deserialized_obj = deserialized_obj
        self.comparable = False if comparable is None else comparable
        self.is_draft = False
...
class DictMemoryObject(MemoryObject):
    ...
    def initialize(self) -> None:
        ...
        comparable = len(self.obj) == len(deserialized_obj) and all(serializer.get_by_id(value_id).comparable for elem in self.items)
