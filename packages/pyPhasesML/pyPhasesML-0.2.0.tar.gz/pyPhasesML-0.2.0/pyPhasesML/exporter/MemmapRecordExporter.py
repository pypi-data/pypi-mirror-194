...
class MemmapRecordExporter(DataExporter, IterDataPipe, MapDataPipe):
    ...
    def saveShape(self, dataId, lengths, saveShape, tmp=False):
        ...
        self.log("saved %s: %s" % (dataId, str(shape)))
    ...
    def saveFromArrayList(self, dataId, arrayList):
        ...
        memmap = self.stream(dataId, {"dtype": "float32", "mode": "w+", "shape": array.shape})
        memmap[:, :, :] = array
        memmap.flush()
        del memmap
    ...
    def __getitem__(self, index):
        self.memmap = np.memmap(self.filePath, dtype=self.type, mode="r", shape=self.fileShape)
