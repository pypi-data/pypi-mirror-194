...
@app.command(help='Adds an entrypoint creator to an environment')
def add_entrypoint_creator(
        name: str = _NAME_ARG,
        entrypoint_creator: str = constants.PERMISSION_ARG,
        server_address: str = constants.SERVER_ADDRESS_OPTION):
    resources.add_permision(name=name,
                            perm_to_add=entrypoint_creator,
                            permission='entrypoint_creators',
                            endpoint=_END_POINT,
                            server_address=server_address)


@app.command(help='Removes an entrypoint creator from an environment')
def remove_entrypoint_creator(
        name: str = _NAME_ARG,
        entrypoint_creator: str = constants.PERMISSION_ARG,
        server_address: str = constants.SERVER_ADDRESS_OPTION):
    resources.remove_permision(name=name,
                               perm_to_remove=entrypoint_creator,
                               permission='entrypoint_creators',
                               endpoint=_END_POINT,
                               server_address=server_address)
