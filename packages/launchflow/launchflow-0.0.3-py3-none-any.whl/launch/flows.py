...
@app.command(help='Adds an environment creator to a flow')
def add_environment_creator(
        name: str = _NAME_ARG,
        environment_creator: str = constants.PERMISSION_ARG,
        server_address: str = constants.SERVER_ADDRESS_OPTION):
    resources.add_permision(name=name,
                            perm_to_add=environment_creator,
                            permission='environment_creators',
                            endpoint=_END_POINT,
                            server_address=server_address)


@app.command(help='Removes an environment creator from a flow')
def remove_environment_creator(
        name: str = _NAME_ARG,
        environment_creator: str = constants.PERMISSION_ARG,
        server_address: str = constants.SERVER_ADDRESS_OPTION):
    resources.remove_permision(name=name,
                               perm_to_remove=environment_creator,
                               permission='environment_creators',
                               endpoint=_END_POINT,
                               server_address=server_address)
