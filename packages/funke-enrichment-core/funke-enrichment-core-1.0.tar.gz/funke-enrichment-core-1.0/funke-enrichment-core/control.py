...
class Invoker(metaclass=RequiredAttributesMetaClass):
    ...
    @abstractmethod
    def _postprocess(self, response_status, response_payload):
        pass
    
    async def process(self, input_data):
        preprocessed_data = self._preprocess(input_data)
        response_status, response_payload = await self._invoke(preprocessed_data)
        return self._postprocess(response_status, response_payload)