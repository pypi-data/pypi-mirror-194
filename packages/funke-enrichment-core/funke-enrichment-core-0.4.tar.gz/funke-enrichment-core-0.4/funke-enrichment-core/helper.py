...
try:
    ...
    print('Logging set to local mode!')
...
def info(msg, extra=None):
    """
    Prints a specific info message to the console

    msg: Message to print to console
    extra: Option to add custom fields to be logged, default is None. Should be passed as dictionary - e.g. logging.info(info_msg, extra={"labels": {"newsid": "123456"}})
    return: Always returns None
    """
    info_msg = msg
    if extra:
        logging.info(info_msg, extra=extra)
    else:
...
def get_current_time(timezone="german"):
    """
    Get current time as a properly formatted datetime object (YYYY:MM:DD HH:MM:SS)

    timezone: Sets the timezone of the datetime object. Choose 'german' for german timezone (default). Anything else will return the local timezone.
    return: current time as datetime object with the format: (YYYY:MM:DD HH:MM:SS)
    """

    if not isinstance(timezone, str):
        raise ValueError('Timezone needs to be a string!')

    if timezone == "german":
        german_timezone = pytz.timezone('Europe/Berlin')
        raw_current_time = datetime.now(german_timezone)
    else:
        raw_current_time = datetime.now()

    current_datetime_str = raw_current_time.strftime("%Y-%m-%d %H:%M:%S")
    current_datetime = datetime.strptime(current_datetime_str, "%Y-%m-%d %H:%M:%S")
    return current_datetime


def generate_uuid(uuid_type='str'):
    """
    Generates a version 4 UUID. The UUID can either be returned as a string in a hex representation or as an int in an integer reprensentation

    uuid_type: Determines the representation type of the UUID. Choose 'str' for hex (default) and 'int' for 'int'.
    return: Returns the version 4 UUID in the specified representation and None in case of an error.
    """
    info_msg = '\n\t To get an int uuid as an int please set uuid_type to "int".\n\t To get an hex uuid as a string please set uuid_type to "str".\n\t "str" is the default case.'
    if isinstance(uuid_type, str):
        ...
        if uuid_type == 'str':
            ...
            raise ValueError('This function can only generate two types of uuids!{}'.format(info_msg))
    else:
        raise ValueError('Argument uuid_type needs to be a string!{}'.format(info_msg))
