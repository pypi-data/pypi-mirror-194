...
class FacebookPost(umbraSnscrpe.base.Item):
	cleanUrl: str
	dirtyUrl: str
...
class _FacebookCommonScraper(umbraSnscrpe.base.Scraper):
...
class _FacebookUserAndCommunityScraper(_FacebookCommonScraper):
	...
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('username', type = umbraSnscrpe.base.nonempty_string('username'), help ='A Facebook username or user ID')
...
class FacebookGroupScraper(_FacebookCommonScraper):
	...
	def get_items(self):
		...
		if 'content:{pagelet_group_mall:{container_id:"' not in r.text:
			raise umbraSnscrpe.base.ScraperException('Code container ID marker not found (does the group exist?)')
		...
		for codeContainerIdStart in ('content:{pagelet_group_mall:{container_id:"', 'content:{group_mall_after_tti:{container_id:"'):
			...
			if not codeContainer:
				raise umbraSnscrpe.base.ScraperException('Code container not found')
			if type(codeContainer.string) is not bs4.element.Comment:
				raise umbraSnscrpe.base.ScraperException('Code container does not contain a comment')
	...
	@classmethod
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('group', type = umbraSnscrpe.base.nonempty_string('group'), help ='A group name or ID')
