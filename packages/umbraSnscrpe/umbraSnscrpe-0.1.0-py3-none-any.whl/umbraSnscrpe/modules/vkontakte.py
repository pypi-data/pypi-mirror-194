...
class VKontaktePost(umbraSnscrpe.base.Item):
	url: str
...
class User(umbraSnscrpe.base.Entity):
	...
	followers: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None
	posts: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None
	photos: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None
	tags: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None
	following: typing.Optional[umbraSnscrpe.base.IntWithGranularity] = None

	followersGranularity = umbraSnscrpe.base._DeprecatedProperty('followersGranularity', lambda self: self.followers.granularity, 'followers.granularity')
	postsGranularity = umbraSnscrpe.base._DeprecatedProperty('postsGranularity', lambda self: self.posts.granularity, 'posts.granularity')
	...
	tagsGranularity = umbraSnscrpe.base._DeprecatedProperty('tagsGranularity', lambda self: self.tags.granularity, 'tags.granularity')
	followingGranularity = umbraSnscrpe.base._DeprecatedProperty('followingGranularity', lambda self: self.following.granularity, 'following.granularity')

	def __str__(self):
		return f'https://vk.com/{self.username}'


class VKontakteUserScraper(umbraSnscrpe.base.Scraper):
	name = 'vkontakte-user'
					...
					for geoblockOffset in range(lastWorkingOffset + 1, offset + 10):
						...
						if not geoPosts.startswith('<div id="post'):
							if geoPosts == '"\\/blank.php?block=119910902"':
								continue
							raise umbraSnscrpe.base.ScraperException(f'Got an unknown response: {geoPosts[:200]!r}...')
						yield from _process_soup(soup = bs4.BeautifulSoup(geoPosts, 'lxml'))
					continue
				raise umbraSnscrpe.base.ScraperException(f'Got an unknown response: {posts[:200]!r}...')
	...
	def _get_entity(self):
		...
		if (countsDiv := soup.find('div', class_ = 'counts_module')):
			for a in countsDiv.find_all('a', class_ = 'page_counter'):
				...
				if label in ('followers', 'posts', 'photos', 'tags'):
					kwargs[label] = umbraSnscrpe.base.IntWithGranularity(count, granularity)
		...
		if (idolsDiv := soup.find('div', id = 'profile_idols')):
			if (topDiv := idolsDiv.find('div', class_ = 'header_top')) and topDiv.find('span', class_ = 'header_label').text == 'Following':
				kwargs['following'] = umbraSnscrpe.base.IntWithGranularity(*parse_num(topDiv.find('span', class_ ='header_count').text))
		...
		if (followersDiv := soup.find('div', id = 'public_followers')):
			if (topDiv := followersDiv.find('div', class_ = 'header_top')) and topDiv.find('span', class_ = 'header_label').text == 'Followers':
				kwargs['followers'] = umbraSnscrpe.base.IntWithGranularity(*parse_num(topDiv.find('span', class_ ='header_count').text))

		return User(**kwargs)

	@classmethod
	def _cli_setup_parser(cls, subparser):
		subparser.add_argument('username', type = umbraSnscrpe.base.nonempty_string('username'), help ='A VK username')
