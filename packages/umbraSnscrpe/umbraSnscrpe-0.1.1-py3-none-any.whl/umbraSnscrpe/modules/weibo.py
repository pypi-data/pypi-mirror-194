...
class Post(umbraSnscrpe.base.Item):
	url: str
	id: str
...
class WeiboUserScraper(umbraSnscrpe.base.Scraper):
	name = 'weibo-user'
	...
	def _ensure_user_id(self):
		...
		if r.status_code == 302 and r.headers['Location'].startswith('/u/') and len(r.headers['Location']) == 13 and r.headers['Location'][3:].strip('0123456789') == '':
			...
			raise umbraSnscrpe.base.ScraperException(f'Got unexpected response on resolving username ({r.status_code})')
	...
	def _get_entity(self):
		...
		if r.status_code != 200:
			raise umbraSnscrpe.base.ScraperException('Could not fetch user info')
		o = r.json()
	...
	def _cli_setup_parser(cls, subparser):
		...
		subparser.add_argument('user', type = umbraSnscrpe.base.nonempty_string('user'), help ='A user ID')
