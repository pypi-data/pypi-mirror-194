		...
		for i, (module, frameRecord) in enumerate(zip(modules, trace)):
			if module is None:
				...
				for j in reversed(range(i)):
					...
					continue
				module = modules[j]
			if not module.__name__.startswith('umbraSnscrpe.') and module.__name__ != 'umbraSnscrpe':
				continue
...
class CitationAction(argparse.Action):
	...
	def __call__(self, parser, namespace, values, optionString):
		try:
			m = importlib.metadata.metadata('umbraSnscrpe')
		except importlib.metadata.PackageNotFoundError:
			print('Error: could not find umbraSnscrpe installation. --citation does not work without the package being installed.', file = sys.stderr)
			parser.exit(1)
...
def parse_args():
	...
	parser.add_argument('--version', action = 'version', version = f'umbraSnscrpe {umbraSnscrpe.version.__version__}')
	...
	classes = umbraSnscrpe.base.Scraper.__subclasses__()
	scrapers = {}
	for cls in classes:
