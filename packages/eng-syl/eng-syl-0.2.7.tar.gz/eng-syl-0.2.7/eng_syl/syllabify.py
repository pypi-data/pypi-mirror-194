...
class wordSegmenter:
    def __init__(self, max_encoder_len=34, max_decoder_len=44, latent_dim = 500, embedding_dim=500):
        ...
        path_e2i = os.path.join(self.this_dir, "e2i_w2s.pkl")
        path_d2i = os.path.join(self.this_dir, "d2i_w2s.pkl")
        path_e2i = path_e2i.replace("\\", "/")
        path_d2i = path_d2i.replace("\\", "/")

        e2i_file = open(path_e2i, 'rb')
        d2i_file = open(path_d2i, 'rb')
    ...
    def load_model_weights(self, filename = "Segmenter2.0_bestweights.h5"):
        weight_path = os.path.join(self.this_dir, filename)
