...
class DumpLoader:
    ...
    def add_syspaths(self, syspaths: Iterable[str]):
        ...
        logging.debug("DumpLoader: syspaths after loading: %s", sys.path)

    def add_imports(self, imports: Iterable[str]):
        for module in imports:
            for i in range(1, module.count('.') + 1):
                submodule_name = '.'.join(module.split('.', maxsplit=i)[:i])
                globals()[submodule_name] = importlib.import_module(submodule_name)
        logging.debug("DumpLoader: globals after loading: %s", globals().keys())

    def load_object(self, python_id: PythonId) -> object:
        logging.debug("DumpLoader: globals before %s loading: %s", python_id, globals().keys())
        ...
        if isinstance(dump_object, ReprMemoryObject):
            ...
            logging.debug("Globals: %s", globals())
            logging.debug("Constructor: %s", dump_object.constructor)
