...
class GtsmMetadata():
    ...
    def get_orientation_xml(self):
        for i, dic in enumerate(self.xml['strain_xml']['inst_info']['sensor_information']['sensor_response']):
            if dic['sensor_type'] == 'Gladwin_BSM_component_1_':
                orientation = float(
                    self.xml['strain_xml']['inst_info']['sensor_information']['sensor_response'][i]['orientation'][
                        '#text'])
        try:
            return orientation
        except Exception as e:
    ...
    def make_weighted_strain_matrix(self, gauge_weights=[1, 1, 1, 1]):
        ...
        gage_weights = np.array(
            [[gauge_weights[0], 0, 0, 0], [0, gauge_weights[1], 0, 0], [0, 0, gauge_weights[2], 0], [0, 0, 0, gauge_weights[3]]])
        orientations = np.array(
            [[.5, .5 * math.cos(math.radians(2 * (90 - self.orientation))),
              .5 * math.sin(math.radians(2 * (90 - self.orientation)))],
             [.5, .5 * math.cos(math.radians(2 * (-30 - self.orientation))),
              .5 * math.sin(math.radians(2 * (-30 - self.orientation)))],
             [.5, .5 * math.cos(math.radians(2 * (-150 - self.orientation))),
              .5 * math.sin(math.radians(2 * (-150 - self.orientation)))],
             [.5, .5 * math.cos(math.radians(2 * (-120 - self.orientation))),
              .5 * math.sin(math.radians(2 * (-120 - self.orientation)))]])
        ...
        strain_matrix = np.matmul(np.matmul(np.linalg.inv(scale_factors), np.linalg.pinv(orientations)),
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if line.startswith("  Manufacturer's Isotropic Strain Matrix"):
                    lab = np.array([lines[i + 1].decode('utf-8').rstrip().split()[1:],
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if line.startswith("  Roeloffs 2010 Tidal Calibration"):
                    er2010 = np.array([lines[i + 3].decode('utf-8').rstrip().split()[1:],
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if line.startswith("  CH Preliminary Tidal Calibration"):
                    ch_prelim = np.array([lines[i + 3].decode('utf-8').rstrip().split()[1:],
                                       lines[i + 4].decode('utf-8').rstrip().split()[1:],
                                       lines[i + 5].decode('utf-8').rstrip().split()[1:]])
    ...
    def get_linearization_params_xml(self):
        ...
        linear_dict = self.xml['strain_xml']['inst_info']['processing']['bsm_processing_history'][-1]['bsm_processing'][
            'linearization']
        self.linearization = {}
    ...
    def get_gauge_weightings_xml(self):
        weight_dict = self.xml['strain_xml']['inst_info']['processing']['bsm_processing_history'][-1]['bsm_processing'][
            'gauge_weightings']
        self.gauge_weightings = [int(weight_dict['gw0']),
                                 int(weight_dict['gw1']),
                                 int(weight_dict['gw2']),
                                 int(weight_dict['gw3'])]
    ...
    def get_detrend_xml(self):
        detrend = {}
        for channel in ['detrend_start_date', 'detrend_g0', 'detrend_g1', 'detrend_g2', 'detrend_g3']:
            if channel == 'detrend_start_date':
                detrend[channel] = \
                self.xml['strain_xml']['inst_info']['processing']['bsm_processing_history'][-1]['bsm_processing'][
            ...
            for line in lines:
                ...
                if baro:
                    if line.startswith("CH"):
                        line = line.split()
                        atmp_coefficients[line[0]] = float(line[1]) * 1e-3  # microstrain
                        if line[0] == "CH3":
                            baro = False
...
def fdsn2bottlename(channel):
    ...
    :return: str
    '''
    codes = {"RS1": "CH0", "LS1": "CH0", "BS1": "CH0",
             "RS2": "CH1", "LS2": "CH1", "BS2": "CH1",
             "RS3": "CH2", "LS3": "CH2", "BS3": "CH2",
             "RS4": "CH3", "LS4": "CH3", "BS4": "CH3",
             "RDO": "atmp", "LDO": "atmp"}
