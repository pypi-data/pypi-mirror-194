...
def find_parent_index(tb_path):
    ...
    for eachfile in file_list:
        ...
        for each_pd_index in list_this_file_pdindex:
            ...
            if now_index == "第四章对发行人进行现场尽职调查的记录":
                print(111111111)
            if now_index:
                while True:
                    if str(now_index).count('-')==0:
                        ...
                        if df_par.shape[0]:
                            ...
                            if df_par.shape[0]:
                                if df_par.shape[0] > 1:
                                    print("A")
                                    print(df_par['页码'] < df_no_parent.loc[each_pd_index, '页码'])
                                    print("C")
                                    #获取
                                    print(type(df_no_parent.loc[each_pd_index, '页码']))
                                    print(df_no_parent.loc[each_pd_index, '页码'])
                                    print("页码信息")
                                    print(df_par['页码'])
                                    df_parz = df_par.loc[df_par['页码'] < df_no_parent.loc[each_pd_index, '页码']]
                                    print(df_parz["页码"].tolist())
                                    print("B")
                                    print(df_parz)
                                    if df_parz.shape[0]:
                                        df_parz = df_parz.sort_values(by=['页码'], ascending=False)
                                        print(df_par)
                                        parent_index = df_parz.iloc[0, 6]
                                        # print(parent_index)
                                    else:
                                        df_par = df_par.sort_values(by=['页码'], ascending=True)
                                        parent_index = df_par.iloc[0, 6]
