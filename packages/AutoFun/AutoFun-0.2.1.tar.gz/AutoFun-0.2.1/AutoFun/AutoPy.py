...
class MainWin(QWidget):
    ...
    def __init__(self, token = ""):
        ...
        self.all_token_signal.connect(self.delete_all_token)

    #这个是设置所有自动化界面的token
    def delete_all_token(self):
        self.CutPdfPicWidget.token = ''
        self.OcrPWidget.token = ""
        self.OcrFinishWidget.token = ''
        self.OcrTableWidget.token = ""
        self.SplitPdfWidget.token= ''
        print('''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''')
