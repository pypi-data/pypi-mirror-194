...
class SplitPdf(QWidget, Ui_SplitPdf):
    ...
    #初始化UI界面
    def UiInit(self):
        ui_list = getOption("splitPdf")
        print(ui_list)
        # 判断类型
        if isinstance(ui_list, list):
            if len(ui_list) == 1:
                self.pdfPathEdit.setText(ui_list[0][1])
                # if isinstance(ui_list[0][2], int):
                # try:
                #     int(ui_list[0][2])
                #     self.FileDeepEdit.setText(ui_list[0][2])
                # except:
                #     print("不用设置默认的层级")
    ...
    # 开始获取隔页
    def splitPdf(self):
        checkTuple = ("splitPdf", self.pdfPathEdit.text(), "", "", '')
