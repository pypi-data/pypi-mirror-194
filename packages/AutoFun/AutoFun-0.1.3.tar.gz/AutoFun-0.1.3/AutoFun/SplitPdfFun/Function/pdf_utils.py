...
#############################下面的是我加的#######################################################################################################
            self.__writeable_pdf.setPageLayout('/SinglePage')
            self.__writeable_pdf.setPageMode('/UseOutlines')
#############################上面的是我加的#######################################################################################################
    ...
    def add_one_bookmark(self,title,page,parent = None, color = None,fit = '/Fit'):
        ...
        if page>self.pages_num:
            page=self.pages_num
        x=self.__writeable_pdf.addBookmark(title,page - 1,parent = parent,color = color,fit = fit)
        print('add_one_bookmark success! bookmark title is: {0}'.format(title))
        return x

    def addJS(self, javascript):
        x = self.__writeable_pdf.addJS(javascript)
