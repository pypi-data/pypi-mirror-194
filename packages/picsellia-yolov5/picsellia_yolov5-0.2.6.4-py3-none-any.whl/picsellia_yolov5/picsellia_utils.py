...
def update_model_hyperparameters_config_file(hyperparameter_file_path: str, experiment_parameters: dict) -> None:
    """ 
    ...
    if not os.path.isfile(hyperparameter_file_path):
        raise FileNotFoundError(f"Expected a .yaml file, got None at path: {hyperparameter_file_path}")
    ...
    for key, value in model_parameters.items():
        if key in experiment_parameters:
            if type(experiment_parameters[key]) not in [int, float]:
                raise TypeError(f"Invalid type for parameter {key}, got {experiment_parameters[key]} expected {type(value)}")
...
def setup_hyp(experiment=None, data_yaml_path=None, config_path= None, params={}, label_map=[], cwd=None):
    ...
    update_model_hyperparameters_config_file(hyperparameter_file_path=hyp_path, experiment_parameters=params)
    opt = Opt()
    opt.cwd = cwd
...
def send_run_to_picsellia(experiment, cwd):
    ...
    torch.onnx.export(model, torch.zeros((1, 3, 640, 640)), model_latest_path, opset_version=12)
