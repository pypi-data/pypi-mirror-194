...
"""A regular expression to match docums-autorefs' special reference markers
in the [`on_post_page` hook][docums_autorefs.plugin.AutorefsPlugin.on_post_page].
"""
...
def fix_ref(url_mapper: Callable[[str], str], unmapped: List[str]) -> Callable:
    ...
    Arguments:
        url_mapper: A callable that gets an object's site URL by its identifier,
            such as [docums_autorefs.plugin.AutorefsPlugin.get_item_url][].
...
class AutorefsExtension(Extension):
    ...
    def extendMarkdown(self, md: Markdown) -> None:  # noqa: N802 (casing: parent method's name)
        ...
        Add an instance of our [`AutoRefInlineProcessor`][docums_autorefs.references.AutoRefInlineProcessor] to the Markdown parser.
