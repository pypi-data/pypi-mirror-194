...
class client_config:
    ...
    solc_path = r"D:\Apps\solidity-windows\solc.exe"
    ...
    bcos3_lib_path = "../../fiscosdk/bcos3sdklib"
    bcos3_config_file = "../../fiscosdk/bcos3sdklib/bcos3_sdk_config.ini"
    group = "group0"
    # -------------------FISCO BCOS3.0 End-----------------------------------------
    ...
    # client_protocol = "rpc"
    remote_rpcurl = "http://192.168.68.71:8545"  # 采用rpc通信时，节点的rpc端口,和要通信的节点*必须*一致,如采用channel协议通信，这里可以留空
    channel_host = "192.168.68.71"  # 采用channel通信时，节点的channel ip地址,如采用rpc协议通信，这里可以留空
