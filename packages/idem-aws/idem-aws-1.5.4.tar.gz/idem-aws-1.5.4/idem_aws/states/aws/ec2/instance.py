...
__contracts__ = ["resource", "soft_fail", "allow_sync_sls_name_and_name_tag"]
    ...
    if new_state and rerun_data:
        ...
        if current_status and isinstance(current_status, str):
            if current_status.casefold() == rerun_data.get("target_instance_state"):
                hub.log.debug(
                    f"No need to reconcile new state {new_state} for instance: {resource_id} with current status {current_status}"
                )
                return False
            else:
                hub.log.debug(
                    f"Reconcile new state {new_state} for instance: {resource_id} with current status {current_status}"
                )
                return True
