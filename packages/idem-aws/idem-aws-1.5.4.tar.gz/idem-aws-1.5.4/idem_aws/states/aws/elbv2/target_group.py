    ...
    if resource_id:
        ...
        if resource_updated and ctx.get("test", False):
            if update_return["ret"] is not None:
                for key, value in update_return["ret"].items():
...
async def absent(
    ...
    Args:
        ...
        resource_id(str, Optional): The Amazon Resource Name (ARN) of ElasticLoadBalancingv2 Target Group. If the target group does not exist or has already been deleted, the call still succeeds.
