...
def validate_data(
    data: pd.DataFrame, split_name: str, schema: 'Schema',
    model_output_context: 'ModelOutputContext', *,
    existing_models: Sequence[str], existing_splits: Sequence[str],
    project_influence_type: str, model_metadata: md_pb.ModelMetadata
):
    ...
    # Validate data against model_output_context
    if schema.prediction_col_names or schema.feature_influence_col_names:
        ...
        if len(schema.feature_influence_col_names):
            validate_feature_influence_data(
                data=data,
                schema=schema,
...
def validate_feature_influence_data(
    data: pd.DataFrame, schema: 'Schema',
    model_output_context: 'ModelOutputContext',
    existing_splits: Optional[Sequence[str]],
    project_influence_type: Optional[str]
):
