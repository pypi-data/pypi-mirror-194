...
from truera.client.experimental.ingestion_validation_util import validate_data
    ...
    *,
    ...
    load_data_request = ds_pb.LoadDataRequest(
        data_source_info=ds_messages_pb.LoadDataInfo(
            ...
            uri=uri,
            format=ds_messages_pb.Format(file_type=ds_messages_pb.FT_PARQUET)
        )
    )

    # Build materialize request
    materialize_request = _build_materialize_request(
        remote_workspace, split_name, schema, model_output_context, split_name
        in existing_splits
    )
    ...
    run_result = remote_workspace.scheduled_ingestion_client.run_single_ingestion_sync(
        tree=si_pb.DataServiceRequestTree(
            ...
            parents=[si_pb.DataServiceRequestTree(load_req=load_data_request)]
        )
    ).run_result

    # Wait for materialize to succeed
    remote_workspace.logger.info("Waiting for data split to materialize...")
    remote_workspace.get_ingestion_client()._wait_for_materialize_operation(
        run_result.materialize_operation_id
    )
    materialize_status = remote_workspace.data_service_client.get_materialize_data_status(
        project_id=project_id,
        materialize_operation_id=run_result.materialize_operation_id
    ).status
    if materialize_status != ds_messages_pb.MATERIALIZE_STATUS_SUCCEDED:
        raise DataOperationFailedError(
            f"Materialize operation {materialize_status.materialize_operation_id} did not succeed, "
        )
