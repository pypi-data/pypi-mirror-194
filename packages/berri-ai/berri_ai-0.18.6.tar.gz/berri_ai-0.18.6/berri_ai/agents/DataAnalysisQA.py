...
class DataAnalysisQA:
    def __init__(self, spreadsheet_loc, openai_api_key, additional_functions = None, additional_descriptions = None):
      self.df = pd.read_csv(spreadsheet_loc)
      self.openai_api_key = openai_api_key
      if additional_functions != None and additional_descriptions != None and len(additional_functions) == len(additional_descriptions): 
    ...
    def pandas_chain(self, user_query, sheet_col_names):
      prompt_template = """
      You are an AI assistant, helping a user formulate pandas expressions based on their queries about the data. The data has been loaded into a pandas dataframe called 'df'. Given a user input, explain what you think the user is trying to achieve and a pandas expression to answer their question. If you are uncertain, say "Hmm, I'm not sure". Do not make things up. 
    ...
    Pandas Expression: `f.sort_values('Search Volume', ascending=False)['cat5']
    ...
    def conditional_search(self, query):
      ...
      pd_output = self.pandas_chain(user_query = query, sheet_col_names = sheet_col_names)
      ...
      if "Pandas Expression" in pd_output: 
        ...
        if match:
          ...
          if len(user_col_names) > 2:
            return ("Here is my response, but I'm not entirely certain since this query involved more than 2 columns. Please check my references to see my working." + value, pd_output)
    ...
    def query(self, user_input: str):
      ...
      descriptions = ["This function takes a none input and returns a none output", "If you're given some sort of question requiring conditional logic on a dataset (e.g. find me top X given Y), use this function. Pass user intent to this function"]
      ...
      CIQAgent = ComplexInformationQA(self.openai_api_key, None, None, functions, descriptions)
