...
class Content:
    def __init__(self, config):
        ...
        config['text']['path'] = f"{template.parent_path}/{config['text']['path']}"
        self.reader = TextReader(**config.get("text", {}))
       
        config['font']['paths'] = [f'{template.parent_path}/{path}' for path in config['font']['paths']]
        self.font = components.BaseFont(**config.get("font", {}))
        self.align = config['layout']['align']
        
        config['micr_font']['paths'] = [f'{template.parent_path}/{path}' for path in config['micr_font']['paths']]
