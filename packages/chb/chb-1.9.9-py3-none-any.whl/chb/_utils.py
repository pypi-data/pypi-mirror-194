...
def bar2(now, total, need_print=True, bar_len_total=50, bar_str='█', info=None):
    """
    打印输出进度条
    now: 当前进度
    total: 需要迭代的总次数
    ...
    if info is None:
        string = f"{now / total:<.0%}|{bar_str * bar_len_now:<{bar_len_total}}| {now}/{total}"
    else:
        string = f"{now / total:<.0%}|{bar_str * bar_len_now:<{bar_len_total}}| {now}/{total}  {info}"
    if need_print:
...
class Open:
    ...
    无论文件是否存在都写入文件，功能待完善
    """
    def __init__(self, path, data, mode, encoding='utf-8'):
        self.path = path
        self.data = data
        self.mode = mode
        if not os.path.exists(path):
            with open(file=path, mode='w', encoding=encoding):
                pass
            self.file_handle = open(file=path, mode=mode, encoding=encoding)
        else:
            self.file_handle = open(file=path, mode=mode, encoding=encoding)
