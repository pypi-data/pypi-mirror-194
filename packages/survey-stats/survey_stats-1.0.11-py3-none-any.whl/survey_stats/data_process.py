...
class Data:
    ...
    def set_names(self, new_names:list[str]=[], old_names:list[str]=[]):
        ...
        for var in self.variables():
            if var in old_names:
                new_dict[new_names[old_names.index(var)]] = self.values[var]
            else:
...
class TimeSeries(Data):
    ...
    def to_weekly(self, method:str='average', estimate_missings:bool=True)->TimeSeries:
        if self.date_type == 'daily':
            values = {}
            for var in self.variables():
                values[var] = {}
    ...
    def to_annual(self, method:str='average', estimate_missings:bool=True)->TimeSeries:
        values, counts = {}, {}
        for date in self.dates:
            try:
                year = int(date[:4])
                for var in self.variables():
                    ...
                    if year in values[var].keys():
                        values[var][year] += float(self.values[var][date])
                    else:
                        values[var][year] = float(self.values[var][date])
        ...
        frequency = statistics.mode(counts.values())
        if method == 'average':
            for year in counts.keys():
                if counts[year] == frequency:
                    for var in values.keys():
                        values[var][year] /= frequency
                elif estimate_missings:
                    if year-1 in counts.keys():
                        for var in values.keys():
                            r_1 = sum([self.values[var][date.replace(str(year), str(year-1))] 
                                            for date in self.dates
                                                if date[:4]==str(year) and 
                                                not is_nan(self.values[var][date.replace(str(year), str(year-1))], True)])
                            values[var][year] = (values[var][year-1]/r_1 * values[var][year])/frequency
                    else:
                        for var in values.keys():
                            values[var][year] *= 1/counts[year]
                else:
                    for var in values.keys():
                        values[var][year] = np.nan
        else:       #sum
            for year in counts.keys():
                if counts[year] != frequency:
                    if estimate_missings:
                        if year-1 in counts.keys():
                            for var in values.keys():
                                r_1 = sum([float(self.values[var][date.replace(str(year), str(year-1))])
                                            for date in self.dates
                                                if date[:4]==str(year) and 
                                                not is_nan(self.values[var][date.replace(str(year), str(year-1))])])
                                values[var][year] = values[var][year-1]/r_1 * values[var][year]
                        else:
                            for var in values.keys():
                                values[var][year] *= frequency/counts[year]
                    else:
                        for var in values.keys():
                            values[var][year] = np.nan
        ...
        for var in vars:
            ...
            for i in range(len(self.dates)):
                ...
                if print_progress:
                    ...
                    print(f"{v} of {len(vars)} variables ({v/len(vars)*100:.2f}%). left: {left_time}, remain: {remain_time}." + ' '*20, end='\r')
        if print_progress:
