...
class Equation:
    ...
    def forecast(self, sample:Sample):
        ...
        data = Formulas([self.dep_var]+self.indep_vars).calculate_all(self.sample.data)
        vals = sorted(list(set([data.values[self.dep_var][i] for i in self.sample.index
                   if not max([np.isnan(data.values[var][i]) for var in self.indep_vars])])))
        
        values = {}
        for i, index in enumerate(res_num):
            for j, p in enumerate(index):
                if not f'{self.dep_var}_{j}' in values.keys():
                    values[f'{self.dep_var}={vals[j]}'] = {}
                values[f'{self.dep_var}={vals[j]}'][sample.index[i]] = p
