...
class MarkdownElement():
  ...
  def __init__(self, objects=None):
    if objects is None:
      objects = []
    # self.__rendered_out = None
    self._parent = None
    self.__children = []
    _objects = objects if isinstance(objects, (list)) else [objects] if objects else []
    for _o in _objects:
      self.__add_child(_o)
  ...
  # def __depth(self, level=0):
  #   if self._parent:
  #     return self._parent.__depth(level + 1)
  #   return level
