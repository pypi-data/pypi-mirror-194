...
class ElementMovement:
    def __init__(self, element_state: 'ElementState', element_buffer: deque):
        ...
        self.movement_1_sec_changed_significantly_by_axis = self.is_movement_vector_significant(element_buffer=element_buffer, movement_vector_variable='movement_vector_xy_1_sec')
        self.movement_1_sec_changed_significantly = any(self.movement_1_sec_changed_significantly_by_axis)
    ...
    def get_movement_vector(self, element_state: 'ElementState', element_buffer: deque, window_size_seconds: int = 1) -> tuple[float, float]:
        ...
        windows_size_frames = _frames_to_past_from_seconds_in_buffer(element_buffer=element_buffer,
        ...
        (curr_x, curr_y), (prev_x, prev_y) = element_state.bbox_center_xy, element_buffer[-windows_size_frames].bbox_center_xy
    ...
    def is_movement_vector_significant(self, element_buffer: deque, movement_vector_variable: str,
                                       ...
                                       deviation_from_std_to_be_relevant: int = STD_OUTLIER_DEVIATION,
        ...
        median, std = np.median(prev_movement_vectors, axis=0), np.std(prev_movement_vectors, axis=0)
        # Check if the movement vector is significant (by axis)
        relevance_x, relevance_y = np.abs(median - movement_vector) > deviation_from_std_to_be_relevant * std
...
def _frames_to_past_from_seconds_in_buffer(element_buffer: deque, current_element_timestamp: float,
