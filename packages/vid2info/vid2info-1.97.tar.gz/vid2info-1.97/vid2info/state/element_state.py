...
class ElementState:
    def __init__(self, element_tracker_info : dict, first_detection_timestamp : float | None = None,
        ...
        self.movement_vector_xy = self.get_movement_vector(element_buffer=element_buffer, window_size=1)
        self.avg_movement_vector_xy_5_frames = self.get_movement_vector(element_buffer=element_buffer, window_size=5)
    ...
    def get_movement_vector(self, element_buffer: deque, window_size: int = 5) -> tuple[float, float]:
        """
        ...
        :param element_buffer: deque. The buffer containing the N previous ElementStates. It will help to compute the
                element evolution.
        :param window_size: int. The size of the window of frames to compute the movement vector.
        :return: tuple[float, float]. The movement vector of the element.
        """
        window_size = min(window_size, len(element_buffer))
        if window_size == 0:
            return (0., 0.)
        # Calculate the new immediate movement vector
        (curr_x, curr_y), (prev_x, prev_y) = self.bbox_center_xy, element_buffer[-1].bbox_center_xy
        movements_by_frame = [(prev_x - curr_x, prev_y - curr_y)]
        # Just to speed it up for the immediate movement vector
        if window_size == 1:
            return movements_by_frame[0]
        # Append the movement vectors of the previous frames
        movements_by_frame.extend([element_buffer[-i].movement_vector_xy for i in range(1, window_size)])
        # Calculate the average movement
        # As windows are small, summing the vectors up with a python native sum is faster than numpy
        avg_movement = tuple(sum(x) / len(x) for x in zip(*movements_by_frame))
        assert len(avg_movement) == 2, f"avg_movement must be a tuple of length 2. Got: {len(avg_movement)}"
        return avg_movement
