...
class MaterializedFeatureView(FeatureView):
    ...
    def local_run(
        ...
        **mock_sources: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        Unlike :py:func:`run`, :py:func:`local_run` will is intended for unit testing. It will not make calls to your
        ...
        # TODO(TEC-12514) Add support for local_run method for PushSource without batch_config
        if data_source_type == DataSourceType.PUSH_NO_BATCH:
            raise TectonValidationError(
                f"The `local_run()` method is currently unsupported for Feature Views that are backed by a PushSource without a batch_config"
            )

        if start_time is None or end_time is None:
            raise TypeError("local_run() requires start_time and end_time to be set.")
    ...
    def local_run(
        ...
        Example:
            .. code-block:: python
                ...
                # Test using `run` API.
                result = transaction_amount_is_high.local_run(transaction_request={'amount': 100})
