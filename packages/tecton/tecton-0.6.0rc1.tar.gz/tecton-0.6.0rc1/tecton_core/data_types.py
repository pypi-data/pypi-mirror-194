...
def data_type_to_column_type(data_type: DataType) -> column_type_pb2.ColumnType:
    if isinstance(data_type, StringType):
        ...
        if isinstance(element_type, StringType):
            return column_type_pb2.ColumnType.COLUMN_TYPE_STRING_ARRAY
        elif isinstance(element_type, Int64Type):
            return column_type_pb2.ColumnType.COLUMN_TYPE_INT64_ARRAY
        elif isinstance(element_type, Float64Type):
            return column_type_pb2.ColumnType.COLUMN_TYPE_DOUBLE_ARRAY
        elif isinstance(element_type, Float32Type):
            return column_type_pb2.ColumnType.COLUMN_TYPE_FLOAT_ARRAY
        else:
            raise TypeError(f"Unexpected element type {data_type} for Array type")
    else:
        raise TypeError(f"Unexpected data type {data_type}")


def data_type_to_raw_spark_type(data_type: DataType) -> str:
    if isinstance(data_type, StringType):
        return "string"
    elif isinstance(data_type, (Int64Type, Int32Type)):
        return "long"
