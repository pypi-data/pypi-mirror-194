...
def if_contact_connected(user_id: str, workspace: str):
    user_workspaces = [cred.bot_name for cred in
                       UserBotCredentialsMongoRepository().get_bot_credentials(user_id, only_valid=True)]
    if workspace in user_workspaces:
        return True
    dedicated_bots_workspaces = [bot.name for bot in DedicatedBotRepository().get_user_bots(user_id, only_valid=True)]
    return workspace in dedicated_bots_workspaces
