...
class LazyRegression(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, n_neighbors:int=10, algorithm='auto',  distance='heom', leaf_size:int=10,weight_by_distance=True,
                 weights = 'uniform', nb_features_to_keep=8, nb_buckets=100, num_futures_dist_importance=1,posterior_sampling=True,
                 objective='RMSE', rsm=0.5, depth=6, boosting_type='Ordered',bootstrap_type='Bayesian', bagging_temperature=1,
                 learning_rate=0.03, l2_leaf_reg=3.0, iterations=100, verbose=False, linear_model=False, augment_samples_ratio=0.0, inddependent_var_name=None):
        "constructor"
        ...
        self.independent_var_index=None
    def fit(self, X, y):
        """
        ...
        * `n_nodes_`: number of nodes
        ...
        if self.inddependent_var_name is not None:
            self.independent_var_index=X.columns.get_loc(self.inddependent_var_name)
    ...
    def _predict_one(self, X):
        ...
        if self.augment_samples_ratio>0:
            nb_values_to_create = self.interval(0.1, 3, int(self.n_neighbors*self.augment_samples_ratio)+1)
            independent_var_for_linear_augmentation = \
            numpy.where(self._feature_importance == self.independent_var_index)[0]
            X_values_to_augment=_X_train[0]
            y_values_to_augment=_y_train[0]
#            nb_values_to_create=[0.25, 0.5, 1.25, 1.5]

            for i, v in enumerate(nb_values_to_create):
                _X_train[-(i+1)]=X_values_to_augment
                _X_train[-(i+1)][independent_var_for_linear_augmentation]=X_values_to_augment[independent_var_for_linear_augmentation]*v
                _y_train[-(i+1)]=y_values_to_augment*v
        try:
            if weights is None:

                estimator=self._fit_local_model(_X_train, _y_train, estimator, important_features=self._feature_importance[:self.nb_features_to_keep])
...
#            y_pred=estimator.predict([X[0][self._feature_importance[:self.nb_features_to_keep]]])
            y_pred=self._predict_local_model(X_test=[X[0][self._feature_importance[:self.nb_features_to_keep]]], estimator=estimator, important_features=self._feature_importance[:self.nb_features_to_keep])
    ...
    def _fit_local_model(self, X_train, y_train, estimator, get_feature_importance=False, important_features=None):

        X_train_df = pd.DataFrame(X_train)
#        remove_features(X_train_df, y_train)
    ...
    def _fit_linear_model(self, X_train, y_train, estimator, important_features=None):
        ...
        colinear=Multicollinearity(configs={"colinearity-threshold": 0.7, "correlation-with-target-threshold": 0.3})
...
if __name__ == '__main__':
    ...
    data_origin = pd.read_csv("/Users/mohamedmentis/Dropbox/Mac (2)/Documents/Mentis/Development/Python/kolibri-ml/examples/Poidspapier- 3 ao_t 2022_V2.csv")
