...
class LatentClassMixedModel(MixedLogit):
    ...
    def fit(self, X, y, varnames=None, alts=None, isvars=None, num_classes=2,
            ...
            halton_opts=None, ftol=1e-8, ftol_lccmm=1e-4,
    ...
    def get_MAE(self, betas, validation_X, validation_Y, weights, panels, avail, batch_size=None):
        ...
        draws, drawstrans = self._generate_draws(N, self.n_draws)

        p = self._compute_probabilities_latent(betas, validation_X, validation_Y, panel_info, draws,
                                               drawstrans, avail)
        # TODO: sort out chol_mat
        p = self._compute_probabilities(betas, validation_X, panel_info, draws, drawstrans,
                                       avail, self.var_list, self.chol_mat)
        pred_prob = np.mean(p, axis=1)
        predicted_probabilities_val = pred_prob * 100
        obs_freq = self.obs_prob * 100
