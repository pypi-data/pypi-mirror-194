    ...
    def _store_sources(self, value: Referring[ProtobufWithUUID]) -> None:
        ...
        for referred_value in referred_values:
            if isinstance(referred_value, DataSpec):
                referred_value = cast(
                    Referring[ProtobufWithUUID], referred_value
                )
                if len(self.sources(referred_value)) == 0:
                    self._store_sources(referred_value)

        sources = set()
        for referred_value in referred_values:
            sources.update(self._sources[referred_value.uuid()])
        if len(sources) == 0:
            raise ValueError(
