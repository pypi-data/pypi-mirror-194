...
yang_template = """{{ task_name}}:
    ...
    test_sections:{% for task in yang_rpc %}
        - {{ task['section_name']}}:
            - yang:
                device: uut
                connection: netconf
                ...
                content:
                    rpc: |
                      {{ task['content']|indent(width=22,
                                          first=False, blank=False) }}
                log:
                    name: {{task['name']}}
                    category: {{ task['category']}}
                    module: ''
                    revision: ''
                banner: {{task['banner']}}{% endfor %}
...
class TaskHandler(object):
    ...
    def get_replay(cls, path, category, replay, variables=None):
        ...
        if 'autogenerated' not in replay_dict:
            if re.match("^\d\d\d\d\d -", replay_dict['name']):
        ...
        try:
            ...
            if segments:
                ...
                for counter, segment in enumerate(segments):
                    ...
                    # Test manager can add returns block.
                    if segment['operation'] == 'rpc':
                        content = {'rpc': segment['content']}
                        ...
                        rpcs['section_name'] = section_name
                        rpcs['content'] = rpc_str
                        rpcs['name'] = task['name']
                        rpcs['category'] = task['category']
                        rpcs['operation'] = segment['operation']
                        rpcs['banner'] = 'YANG CUSTOM'
                        yang_custom_bool = True
                        yang_custom_rpc.append(rpcs)
                    else:
                    ...
                    yang = {
                        'yang': {
                            ...
                            'banner': 'YANG ' + str(segment['operation']
                                                    ).upper()
                            }
                        }
                    test_section[section_name].append(yang)
                ...
                with open(yaml_file_path, mode="w+") as file:
                    if yang_custom_bool:
                        ...
                        yang_custom = tplt.render({
                                                   'task_name': task_name,
                                                   'yang_rpc': yang_custom_rpc
                                                   })
                        file.write(yang_custom)
                    else:
                        yaml.safe_dump(task_string, file,
                                       default_flow_style=False,
                                       sort_keys=False)
            else:
                return task_string
