...
class Machine(MachineBase):
    ...
    def setup_nginx(self, dhparam_b):
        """
        ...
        tls_dir = self.env_dir / "tls"
        self.run(f"mkdir {tls_dir}")
        ...
        self.run(f"cat > {tls_dir}/ip.cnf", stdin=config_output.getvalue())
        self.run(
            f"openssl req -x509 -nodes -days 365 -newkey rsa:2048"
            f" -keyout {tls_dir}/ip.key -out {tls_dir}/ip.crt -config {tls_dir}/ip.cnf"
        )
    ...
    def setup_app(self, package):
        ...
        with self.supervisor(package) as config:
            config["program:app"] = {
                "autostart": "true",
                "command": (
                    f"{self.runinenv} {self.env_dir} gunicorn {package}:app "
