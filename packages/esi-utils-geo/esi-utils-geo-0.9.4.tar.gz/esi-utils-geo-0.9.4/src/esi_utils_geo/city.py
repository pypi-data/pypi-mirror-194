...
class Cities(object):
    ...
    def limitByBounds(self, bounds):
        ...
        if xmax < xmin:
            insidex1 = (newdf["lon"] >= xmin) & (newdf["lon"] < 180)
            insidex2 = (newdf["lon"] >= 180) & (newdf["lon"] < xmax)
            insidex = insidex1 | insidex2
        else:
            insidex = (newdf["lon"] >= xmin) & (newdf["lon"] < xmax)
        insidey = (newdf["lat"] >= ymin) & (newdf["lat"] < ymax)
        newdf = newdf.loc[insidex & insidey]  # get cities inside bounds
    ...
    def limitByRadius(self, lat, lon, radius):
        ...
        newdf = self._dataframe.copy()
        newdf["dist"] = geodetic_distance(lon, lat, newdf["lon"], newdf["lat"])
