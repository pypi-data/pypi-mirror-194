...
def _decode_col_name(col_name: str, column_metadata: Optional[Sequence[ColumnMetadata]]) -> str:
    ...
    if x_split[0] == "ca":
        ...
        assert column_metadata is not None, "Column metadata is required to decode stateful column names"
        c = get_unique_item((c for c in column_metadata if c.column_name == col_name), f"Column {col_name}")
        return c.feature_fqn
    else:
        raise ValueError(f"Unexpected identifier: {x_split[0]}")


def _decode_column_names(
    column_names: List[str], column_metadata: Optional[Sequence[ColumnMetadata]]
) -> Mapping[str, str]:
    ans: Dict[str, str] = {}
    for x in column_names:
        if x.startswith("__"):
            ...
            continue
        feature_name = _decode_col_name(x, column_metadata)
...
def _extract_df_columns(
    ...
    output_id: bool,
    version: DatasetVersion,
    column_metadata: Optional[Sequence[ColumnMetadata]],
) -> Union[pl.DataFrame, pl.LazyFrame]:
    try:
        import polars as pl
    except ImportError:
    ...
    if version in (
        ...
        decoded_col_names = _decode_column_names(df.columns, column_metadata)
