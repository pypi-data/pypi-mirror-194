    ...
    if isinstance(t, (OnlineResolver, OfflineResolver)):
        ...
        for f in t.inputs:
            if (not isinstance(f, type) or not issubclass(f, DataFrame)) and not f.is_autogenerated:
                ...
                all_inputs.append(UpsertFeatureInputUnionGQL(feature=feature_ref))
                inputs.append(feature_ref)
            elif isinstance(f, type) and issubclass(f, DataFrame):
                all_inputs.append(UpsertFeatureInputUnionGQL(dataframe=_convert_df(f)))
    ...
    if isinstance(t, Feature):
        ...
        if t.is_has_one:
            ...
            scalar_kind_gql = UpsertScalarKindGQL(
                ...
                dtype=serialize_pyarrow_dtype(t.converter.pyarrow_dtype).json(),
            )
        return UpsertFeatureGQL(
            ...
            owner=t.owner,
            windowBuckets=t.window_buckets and sorted(list(t.window_buckets)),
