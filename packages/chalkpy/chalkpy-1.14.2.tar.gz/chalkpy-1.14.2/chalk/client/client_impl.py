...
class ChalkAPIClientImpl(ChalkClient):
    ...
    def get_dataset(
        ...
        if context is None:
            context = OfflineQueryContext()
        response: GetOfflineQueryJobResponse = self._get_dataset_urls(dataset_name=dataset_name, context=context)
        if response.errors:
            raise ChalkDatasetDownloadException(dataset_name=dataset_name, errors=response.errors)
        return load_dataset(uris=response.urls, version=DatasetVersion(response.version), lazy=True)
    ...
    def _get_dataset_urls(self, dataset_name: str, context: OfflineQueryContext) -> GetOfflineQueryJobResponse:
        return self._request(
            method="GET",
