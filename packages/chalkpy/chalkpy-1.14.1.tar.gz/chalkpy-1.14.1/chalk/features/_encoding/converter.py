...
class FeatureConverter(Generic[_TPrim, _TRich]):
    def __init__(
        ...
        if pa.types.is_struct(pyarrow_dtype) and is_nullable:
            ...
            warnings.warn(
                UserWarning(
                    (
                        f"Feature '{self._name}' is an nullable struct (e.g. Optional[...]). A `null` value for a struct "
    ...
    def from_rich_to_primitive(
        ...
        if self.is_value_missing(value):
            if missing_value_strategy == "allow":
                warnings.warn(
                    UserWarning(f"Allowing missing value for feature '{self._name}' with strategy 'default_or_allow'")
                )
    ...
    def from_pyarrow_to_rich(self, values: pa.Array, /) -> Sequence[_TRich]:
    ...
    def from_pyarrow_to_primitive(self, values: pa.Array) -> Sequence[_TPrim]:
        return values.to_pylist()

    def from_primitive_to_rich(self, value: Union[_TPrim, _TRich]) -> _TRich:
        return self._from_prim(value)

    def from_primitive_to_pyarrow(self, value: Sequence[_TPrim]) -> pa.Array:
