...
def train_loop(
  ...
  Args:
    ...
    log_real_time: Decide whether or not you want to log loss at each step to Picsellia.
      If not None, it equals the experiment object where to log.
    ...
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        for _ in range(global_step.value(), train_steps,
          ...
          if log_real_time is not None:
            try:
              log_real_time.log("loss", [float(loss.numpy())], 'line')
            except:
              pass
          ...
          if train_steps <= 10000:
            if global_step.value() - logged_step >= 10:
              print(
            ...
            if global_step.value() - logged_step >= int(train_steps/1000):
              print(
                  'Step {} per-step time {:.3f}s loss={:.3f}'.format(
                      global_step.value(), time_taken / num_steps_per_iteration,
                      loss))
