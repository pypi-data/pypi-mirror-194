...
class Shuttle:
    ...
    def shuttle_slots(self):
        exhaustion_helper = {}
        for k, v in self.sources_first.items():
            # logger.info(f"{k} {v['transactions'][0]}")
            logger.info(f"CURRENT VIEW key: {k}")
            current_view = self.views_dict[k]
            logger.info(f"CURRENT VIEW's TYPE: {type(current_view)}")
            # current_classes = current_view.all_classes()
            ...
            for i in v['transactions']:
                ...
                logger.info("ABOUT TO CHECK CURRENT SLOT")
                logger.info(i['slot'])
                logger.info(i['source class'])
                logger.info("ABOUT TO RETRIEVE CURRENT SLOT")
                logger.info(current_view.get_class(i['source class']))
                logger.info("RETRIEVED CURRENT SLOT")
                current_slot = current_view.induced_slot(slot_name=i['slot'], class_name=i['source class'])
                logger.info("DONE CHECKING CURRENT SLOT")
