...
from quantizeml.layers import (QuantizedReLU, QuantizedSeparableConv2D,
...
def _parse_sepconv(layer):
    ...
    assert out_quantizer is None, "In AkidaVersion.v1, output quantizer is not allowed"
...
def convert_sepconv_block(model_ak, layers):
    ...
    sepconv_block = [sepconv] + parse_non_neural_layers(layers, sepconv_params)
    ...
    for layer in sepconv_block:
        if isinstance(layer, QuantizedReLU):
            set_output_variables(sepconv_ak, layer.out_quantizer)

    return len(sepconv_block)
