...
def install_sfgwas() -> None:
    ...
    commands = [
        ...
        "wget -nc https://s3.amazonaws.com/plink2-assets/plink2_linux_x86_64_20230109.zip",
        "mkdir -p ~/.local/bin",
...
def start_sfgwas(role: str, demo: bool = False, protocol: str = "SFGWAS") -> None:
    ...
    run_sfgwas_with_task_updates(command, protocol)
...
def run_sfgwas_with_task_updates(command: str, protocol: str) -> None:
    doc_ref_dict: dict = get_doc_ref_dict()
    num_power_iters: int = int(doc_ref_dict["advanced_parameters"]["num_power_iters"]["value"])

    task_updates = [
        "Begin SFGWAS protocol",
        "Starting QC",
        "Starting PCA",
        "Preprocessing X",
    ]
    ...
    task_updates.extend(f"MatMult: block {i} / 22 starting" for i in range(1, 23))
    task_updates.append("Finished association tests")
    ...
    if process.stdout:
        while line := process.stdout.readline().decode("utf-8").strip():
            print(line + "\t" + f"Current task: {current_task}")
            if current_task in line:
                if prev_task:
                    update_firestore(f"update_firestore::task={prev_task} completed")
                update_firestore(f"update_firestore::task={current_task}")
                if task_updates:
    ...
    if process.stderr and process.stderr.read() != b"":
        ...
        update_firestore(f"update_firestore::task={current_task} completed")
