...
def lvc_func(args):
    ...
    lvc = LVC.from_molcas(args.rassi, args.grad, verbose=True)
...
def eval_func(args, unknown_args):
    ...
    if args.geom is None:
        hpc.store_func(store_args, make_lvc_evaluator, selected,
                       order=args.order, geom=None, coords=coords,
                       truncate=args.truncate)
    else:
        for geom in map(lambda file: xyzp.load_xyz(file)[1], args.geom):
            hpc.store_func(store_args, make_lvc_evaluator, selected,
                           order=args.order, geom=geom * ANG2BOHR,
                           coords=coords, truncate=args.truncate)
...
def read_args(arg_list=None):
    ...
    evaluate.add_argument(
        ...
        default='mode',
        choices=['mode', 'atomic'],
        help='Coordinates for differentiation.'
    )
