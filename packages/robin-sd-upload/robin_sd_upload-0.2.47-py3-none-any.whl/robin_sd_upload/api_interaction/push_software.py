...
def push_software(fpath, radarType, version_name):
    ...
    if os.path.isfile(fpath):
        logger.log(message="ZIP file exists for push software: " + fpath, log_level="info", to_file=True, to_terminal=True)
    else:
        logger.log(message="ZIP not exist for push software: " + fpath, log_level="error", to_file=True, to_terminal=True)
    ...
    if response.status_code == 200:
        logger.log(message="Software pushed successfully!", log_level="info", to_file=True, to_terminal=True)
    else:
        logger.log(message="Software push failed with status code: " + str(response.status_code), log_level="error", to_file=True, to_terminal=True)
