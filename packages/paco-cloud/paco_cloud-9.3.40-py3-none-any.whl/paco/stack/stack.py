...
from paco.utils.filelock import FileLock, FileLockException
    ...
    def stack_success(self):
        ...
        if self.action == "update":
            ...
            if self.paco_ctx.alarms_only == True and self.tags and self.tags.has_kvp('Paco-Resource-Sub-Type', 'Alarms') == False:
                return
            # --ssm <command> was used.
            if self.paco_ctx.ssm_command != None:
                return
...
""".format(param_entry.key, self.resource.paco_ref_parts, param_entry.stack.resource.paco_ref_parts, param_entry.stack.get_name())
    ...
    def save_stack_outputs(self):
        ...
        stack_output_lockfile = self.paco_ctx.outputs_path  / "stack_output_lock"
        filelock = utils.lock_file(self.paco_ctx, stack_output_lockfile)
        # save to disk cache
...
A Stack can cache it's templates to the filesystem or check them against AWS and get their status.
    ...
    def create_stack(self):
        ...
        if suppress == True:
            self.log_action("Provision", "Suppressed")
            return
    ...
    def update_stack(self):
        ...
        if self.paco_ctx.ssm_command != None:
            run_hooks = False
            suppress = True
            if self.template and isinstance(self.template, paco.cftemplates.ASG):
                # send SSM command to update existing instances
                ssm_ctl = self.paco_ctx.get_controller('SSM')
                ssm_ctl.command_update_system_packages(
            ...
            try:
                ...
                if e.response['Error']['Code'] == 'ValidationError':
                    ...
                    if e.response['Error']['Message'].endswith("No updates are to be performed."):
                        ...
                        success = True
                    elif e.response['Error']['Message'].endswith("Cannot specify usePreviousValue as true for a parameter key not in the previous template"):
                        parameter_key = e.response['Error']['Message'].split('Invalid input for parameter key ')[1].split('.')[0]
                        for stack_param in stack_parameters:
                            if stack_param['ParameterKey'] == parameter_key:
                                del stack_param['UsePreviousValue']
                                stack_param['ParameterValue'] = ''
                                print(f'UsePreviousValue: {parameter_key} => False')
                        continue
                    if success == True:
    ...
    def provision(self):
        ...
        # Ignore if not an Alarms stack when alarms_only flag is True
        if self.paco_ctx.alarms_only:
            if self.tags == None or self.tags.has_kvp('Paco-Resource-Sub-Type', 'Alarms') == False:
                suppress = True

        if self.paco_ctx.ssm_command != None and self.resource and schemas.IASG.providedBy(self.resource) == False:
            suppress = True
    ...
    def validate(self):
        ...
        if self.paco_ctx.alarms_only:
            if self.tags == None or self.tags.has_kvp('Paco-Resource-Sub-Type', 'Alarms') == False:
                self.paco_ctx.log_action_col("Validate", "Suppressed", self.account_ctx.get_name() + '.' + self.aws_region, short_yaml_path, col_2_size=col_2_size)
                return
    ...
    def wait_for_complete(self):
        ...
        while True:
            ...
            # run post hooks
            # Do not run hooks when an SSM Command is being run
            if self.paco_ctx.ssm_command == None:
