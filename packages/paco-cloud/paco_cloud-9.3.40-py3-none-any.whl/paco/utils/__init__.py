...
def prefixed_name(resource, name, legacy_flag=None, long_name=True, app_name=None):
    ...
    if env != None:
        str_list.extend([env.name, app_name, group_name])
        if long_name == True:
            str_list.extend([resource.name, name])
    else:
        str_list.extend([app_name, group_name])
        if long_name == True:
            str_list.extend([resource.name, name])
...
def write_to_file(folder, filename, data, supress_file_lock=False):
    ...
    if supress_file_lock == False:
        filelock = lock_file(None, file_path)
...
def lock_file(paco_ctx=None, filename=None, timeout=30):
    if os.getpgrp() != os.tcgetpgrp(sys.stdout.fileno()):
        paco_ctx = None
    filelock = FileLock(filename, timeout=30)
    wait_for_lock = True
    while wait_for_lock:
        try:
            filelock.acquire()
            ...
            if paco_ctx != None:
                answer = paco_ctx.input_confirm_action("\nLock timedout, force the release of this lock?", ignore_force_yes=True)
            else:
                answer = True
            if answer == False:
                continue
            else:
                print(f"!!!!!!!!!!!!!!!!!!!! FORCING RELEASE: {filename}")
                filelock.release(force=True)
        wait_for_lock = False
