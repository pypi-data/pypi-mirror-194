...
troposphere.autoscaling.LaunchConfiguration.props['SecurityGroups'] = (list, False)
troposphere.autoscaling.AutoScalingGroup.props['VPCZoneIdentifier'] = (list, False)
...
class ASG(StackTemplate):
    def __init__(
        ...
        if asg_config.cfn_init and asg_config.is_enabled():
            # launch_config_dict['MetadataOptions'] = troposphere.autoscaling.MetadataOptions(
        ...
        asg_dict = {
            ...
            'Cooldown': str(asg_config.cooldown_secs),
        ...
        if asg_config.ecs != None and asg_config.ecs.is_enabled():
        ...
        if asg_config.scaling_policies != None:
            for scaling_policy_name in asg_config.scaling_policies.keys():
                ...
                scaling_policy_res = troposphere.autoscaling.ScalingPolicy(
                    ...
                    Cooldown=str(scaling_policy.cooldown)
                )
                alarm_idx = 0
        ...
        for ecr_deploy_name in ecr_deploy_group.keys():
            ...
            if ecr_deploy and len(ecr_deploy.release_phase.ecs) > 0:
                ...
                for repository in ecr_deploy.repositories:
                    ...
                    pull_repo_arn = f'arn:aws:ecr:{source_env.region}:{source_account_id}:repository/{source_ecr_obj.full_repository_name}'
                    push_repo_arn = f'arn:aws:ecr:{dest_env.region}:{dest_account_id}:repository/{dest_ecr_obj.full_repository_name}'
