        ...
        for resource in permission_config.resources:
            ...
            if schemas.IResourceGroup.providedBy(resource_obj):
                ...
                statement_list.append(
                    Statement(
                        Sid=self.create_cfn_logical_id_join(f'SSMStartSession{resource_obj.name}'),
                        Effect=Allow,
                        Action=[
                ...
                statement_list.append(
                    Statement(
                        Sid=self.create_cfn_logical_id_join(f'SSMEC2{resource_obj.name}'),
                        Effect=Allow,
                        Action=[
    ...
    def deployment_pipeline_codepipeline_permissions(self, pipeline_list, assume_role_res):
        ...
        if len(retrystages_arn_list) > 0:
            ...
            while idx < len(resource_chunks):
                ...
                managed_policy_res = troposphere.iam.ManagedPolicy(
                    title=self.create_cfn_logical_id(f"CodePipelineRetryStagesPolicy{idx}"),
                    PolicyDocument=PolicyDocument(
                        Version="2012-10-17",
                        Statement=statement_list
                    ),
