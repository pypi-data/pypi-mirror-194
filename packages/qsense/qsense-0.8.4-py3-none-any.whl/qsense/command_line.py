...
class Qsense:
    ...
    def old_reloadtasks(self, host, certificate, lastreload_days, status=None, id="full"):
        """Reloadtask status. Max_age is in seconds"""
        qrs = qsAPI.QRS(proxy=host, certificate=certificate)
        return qsense.reloadtask.old_reloadtasks(qrs, int(lastreload_days), status, id)
