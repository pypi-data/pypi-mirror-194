...
class NoticeServices():
    ...
    def notice_type_list(params: dict = None, need_pagination: int = 0):
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=["id", "name", "notice_type_id"],
            alias_dict={"notice_type_id": "id"}
        )

        notice_type_obj = NoticeType.objects.filter(**params).values("id", "name", "description")
        # 不许分页直接返回
    ...
    def notice_list(params: dict = None, need_pagination: bool = True):
        ...
        notice_obj = Notice.objects.filter(**params).annotate(notice_type_name=F("notice_type__name")).values()
        # 不许分页直接返回
        if not need_pagination:
            if not params:
    ...
    def notice_edit(params: dict = None, notice_id: int = None, search_params: dict = None):
        ...
        filter_params = format_params_handle(
            param_dict=params,
            filter_filed_list=[
                "notice_type", "title", "content", "link_format", "link_params", "files", "status",
    ...
    @staticmethod
    def notice_add(params: dict = None):
        """
        提醒修改
        :param params: 要修改的参数
        :return: data,err
        ...
        filter_params = format_params_handle(
            param_dict=params,
            filter_filed_list=[
                "notice_type", "notice_type_id", "title", "content", "link_format", "link_params", "files", "status",
                "created_time", "read_time", "expire_time", "thread_id", "from_user_id",
                "to_user_id", "to_role_id",
            ],
