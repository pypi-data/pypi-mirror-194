...
class linear_fit():
   ...
   # Style des Diagramms
    ...
    def make_fit(self,x_vals,y_vals ,r = 2, hide='', name = 'generic name'):
    ...
    def plot(self, title = 'title', xlabel = 'x_achse', ylabel = 'y_achse', err=True, legend = True, infos = False ,name='',hide = False, scale = 0.7):
        ...
        if(err==True and hide == False):
            ax.plot(x_axis, self.__f(x_axis)+self._s(x_axis),'--',color=self.colors[2],zorder=0,alpha=0.8,lw=0.9,label='Fehlerkurve')
            ax.plot(x_axis, self.__f(x_axis)-self._s(x_axis),'--',color=self.colors[2],zorder=0,alpha=0.8,lw=0.9)
