...
from mitos import CONFIG, mitos
...
def check_clients(stad):
    ...
    if ((stad["time_client"] is None and time() - stad["time_server"] > ONEDAY)
       or (stad["time_client"] is not None and time() - stad["time_client"] > ONEDAY)):
...
def count_clients(stad, status=None):
    ...
    for k in list(stad.keys()):
        ...
        if status is None or stad[k]["status"] == status:
            count += 1
...
def client(keep, sleept):
    ...
    while loop:
        ...
        try:
            ...
            mitos(fastafile, job["code"], cutoff=job["cutoff"],
