...
from jhdata.dates import current_datetime
...
def add_missing_columns(df: pd.DataFrame, schema: Schema):
    #print("Sunny was here :D")
    column_list = [column for column in df.columns]
    columns_to_add = [column_name for column_name in schema.column_names if column_name not in column_list]
    print(f"Adding columns {columns_to_add}")
    for column_name in columns_to_add:
        df[column_name] = None

    df = schema.reorder_columns(df)
    #print("Sunny RULEZZZZZ")
    return df
