...
def get_spark(key_mount="datafabric", key_path="gcp/emart-datafabric/datafabric", scale=0, queue=None, extra_jars=None):
    import os
    import uuid
    import tempfile
    ...
    key = get_secrets(key_mount, key_path)["config"]
...
def send_email(subject, text, send_from, send_to, attachment=None, file_format=None):
    ...
    if attachment:
        ...
        part.add_header("Content-Disposition", f"attachment; filename={subject}.{file_format}")
        msg.attach(part)
