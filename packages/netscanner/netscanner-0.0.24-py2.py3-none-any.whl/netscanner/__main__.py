...
Modes.add_argument('-nP', action = 'store_true', help = 'This flag will execute Mode 2, NO PORT SCAN, which will execute the Host Discovery and 802.11 WLAN Discovery processes.')
Modes.add_argument('-w', action = 'store_true', help = 'This flag will execute Mode 3, WIRELESS ONLY, which will execute the 802.11 WLAN Discovery process exclusively. ')
Modes.add_argument('-l', action = 'store_true', help = 'This flag will execute Mode 4, LOCAL SCAN ONLY, which will execute the Host Discovery and Port Scan processes')
Modes.add_argument('-hD', action = 'store_true', help = 'This flag will execute Mode 5, HOST DISCOVERY ONLY, which will execute the Host Discovery Process exclusively.')
Options.add_argument('--wP', action ='store', dest = 'WLANScanPeriod', default = 60, type = int, metavar = '<integer value in seconds>', help = 'This option allows you to specify a scan period for the 802.11 WLAN Discovery process. The default is 60. This value is ignored if the mode of operation is not Mode 1, 2 or 3. Large values will result in longer scan times but greater verbosity.')
Options.add_argument('--pP', action = 'store', dest = 'PortScanPeriod', default = 60, type = int, metavar = '<integer value in seconds>', help = 'This option allows you to specify a scan period for the Port Scan process. The default is 60. This value is ignored if the mode of operation is not Mode 1 or 4. Large values will result in longer scan times but greater verbosity.')
Options.add_argument('--pR', action = 'store', dest = 'PortRange', default = '-F', type = str, metavar = '<first port-last port>', help = 'This option allows you to specify a port range for the Port Scan process. The default is the 100 most common ports determined by Nmap (-F). Large values will result in longer scan times but greater verbosity. It is useful to combine this option with the --pP option to avoid scan timeouts when scanning large ranges.')
args = parser.parse_args()

# Add -p flag for custom port range
PortRange = ''
if args.PortRange != '-F':
    if bool(re.match('^([1-9][0-9]{0,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])[-]([1-9][0-9]{0,3}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])$', args.PortRange)):
...
# Get most active interface
    ...
    # Run ifconfig
    ifconoutput = (subprocess.check_output('ifconfig').decode('utf-8'))

    # Run ifconfig to gather interface names
    interoutput = (subprocess.check_output(r"ifconfig | sed 's/[ \t].*//;/^$/d'",
    ...
    for line in interoutput.splitlines():
        Interfaces.append(line)

    # Stores RX packet values into list
    for line in ifconoutput.splitlines():
...
# Get local interface info + append to list
    ...
    if SubMask == '128.0.0.0':
        ...
        print("\nError: This program requires a valid network connection to operate. Please check your settings and try again")
        exit()
    ...
    # Get WLAN info
    try:
        iwconoutput = (subprocess.check_output(["iwconfig " + MAI],
                                           shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        # Get 802.11 protocol
        ...
        Channel = (
            subprocess.check_output(["iwlist " + MAI + " channel | grep 'Current Frequency' | awk '{printf $5}'"],
                                    shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        ...
        # Get link quality
        LinkQual = (subprocess.check_output(
            ["iwconfig " + MAI + " | grep 'Signal level=' | awk '{print $4}' |  sed -e 's/level=//'"],
            shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
...
# Execute host discovery techniques
    ...
    subprocess.run(["sudo -S nmap -PR -sn -T4 -n -oN /home/" + username + "/Documents/NetScanner/Temp/ARPScan.txt " + LANInfo[6] + LANInfo[5]],
                   shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    # Execute nmap rDNS lookup scan
    subprocess.run(["sudo -S nmap -R -PR -sn -T5 -oN /home/" + username + "/Documents/NetScanner/Temp/rDNS.txt " + LANInfo[6] + LANInfo[5]],
                   shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
# Retrieve required data from output
    ...
    ActiveIPs = []
    TotalUpHosts = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/ARPScan.txt " + "| grep 'Nmap done at' | awk '{print $14}'"],
                                            shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    for i in range(1, Range):
        IPAddrs = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/ARPScan.txt " + "| grep 'Nmap scan report for' | awk 'NR==" + str(i) + "{print $5}'"],
                                           shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    Range = int(TotalUpHosts) + 1
    for i in range(1, Range):
        Hostname = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/rDNS.txt " + "| grep 'Nmap scan report for' | awk 'NR==" + str(i) + "{print $5}'"],
                                         shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    Range = int(TotalUpHosts) + 1
    for i in range(1, Range):
        Latency = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/ARPScan.txt " + "| grep 'Host is up' | awk 'NR==" + str(i) + "{print $4}'"],
                                           shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    Range = int(TotalUpHosts) + 1
    for i in range(1, Range):
        MACAddrs = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/ARPScan.txt " + "| grep 'MAC Address: ' | awk 'NR==" + str(i) + "{print $3}'"],
                                           shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
...
# Execute port scans
    ...
    for i in range(0, Range, 9):
        try:
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i] + ".txt " + ActiveIPs[i] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+1] + ".txt " + ActiveIPs[i+1] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+2] + ".txt " + ActiveIPs[i+2] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+3] + ".txt " + ActiveIPs[i+3] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+4] + ".txt " + ActiveIPs[i+4] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+5] + ".txt " + ActiveIPs[i+5] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+6] + ".txt " + ActiveIPs[i+6] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+7] + ".txt " + ActiveIPs[i+7] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+8] + ".txt " + ActiveIPs[i+8] + "/32")
            PortScanCmds.append("sudo -S nmap -sS -sU -T4 -n -Pn " + PortRange + " -oN /home/" + username + "/Documents/NetScanner/Temp/PortScan-" + ActiveIPs[i+9] + ".txt " + ActiveIPs[i+9] + "/32")
        except IndexError:
            pass
    ...
    airodump = subprocess.Popen(["sudo -S airodump-ng " + MAI + "mon --band abg -w /home/" + username + "/Documents/NetScanner/Temp/airodump-ng --output-format csv"],
                                shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    TotalAPs = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | wc -l"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    TotalAPs = int(TotalAPs)

    # Get ESSIDs
    ESSIDs = []
    for i in range(1, TotalAPs):
        ESSID = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $18,$19}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        ESSID = ESSID.replace(',', '')
        ESSID = ESSID.replace('\n', '')
        ESSID = ESSID.replace('\r', '')
        IsESSID = any(str.isdigit(i) for i in ESSID)
        if IsESSID == True:
            ESSID = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $19,$20}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
            ESSID = ESSID.replace(',', '')
            ESSID = ESSID.replace('\n', '')
            ESSID = ESSID.replace('\r', '')
            if ESSID == ' ':
    ...
    from mac_vendor_lookup import VendorNotFoundError
    for i in range(1, TotalAPs):
        BSSID = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $1}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        BSSID = BSSID.replace(',', '')
        BSSID = BSSID.replace('\n', '')
    ...
    Channels = []
    for i in range(1, TotalAPs):
        Channel = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $6}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        Channel = Channel.replace(',', '')
        Channel = Channel.replace('\n', '')
    ...
    Throughputs = []
    for i in range(1, TotalAPs):
        Throughput = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $7}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    RSSIs = []
    for i in range(1, TotalAPs):
        RSSI = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $11}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        RSSI = RSSI.replace(',', '')
        RSSI = RSSI.replace('\n', '')
        RSSI = RSSI.replace('\r', '')
        if RSSI == '-1':
    ...
    ENCs = []
    for i in range(1, TotalAPs):
        EncP = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $8}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        EncP = EncP.replace(',', '')
        EncP = EncP.replace('\n', '')
        EncP = EncP.replace('\r', '')
        ENCs.append(EncP)
    ...
    Ciphers = []
    for i in range(1, TotalAPs):
        Cipher = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $9}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    AuthMthds = []
    for i in range(1, TotalAPs):
        Auth = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/Key/{flag=1;next}/Station/{flag=0}flag' | awk '(NR==" + str(i) + "){print $10}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
        Auth = Auth.replace(',', '')
        Auth = Auth.replace('\n', '')
        Auth = Auth.replace('\r', '')
        if Auth == '':
    ...
    TotalStations = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/ESSIDs/{flag=1;next}/'abcdefghijklmnopqrstuvwxyz'/{flag=0}flag' | wc -l"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    for i in range(1, TotalStations):
        StationMAC = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/ESSIDs/{flag=1;next}/'abcdefghijklmnopqrstuvwxyz'/{flag=0}flag' | awk '(NR==" + str(i) + "){print $1}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    StationRSSIs = []
    for i in range(1, TotalStations):
        StationRSSI = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/ESSIDs/{flag=1;next}/'abcdefghijklmnopqrstuvwxyz'/{flag=0}flag' | awk '(NR==" + str(i) + "){print $6}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
    ...
    StationAPs = []
    for i in range(1, TotalStations):
        StationAP = (subprocess.check_output(["cat /home/" + username + "/Documents/NetScanner/Temp/airodump-ng-01.csv | awk '/ESSIDs/{flag=1;next}/'abcdefghijklmnopqrstuvwxyz'/{flag=0}flag' | awk '(NR==" + str(i) + "){print $8}'"], shell=True, stderr=subprocess.DEVNULL).decode('utf-8'))
...
# Output data
    ...
    if (str(LANInfo[13])) == 'N/A':
        print("\nOperating Frequency: " + str(LANInfo[13]))
    else:
        print("\nOperating Frequency: " + str(LANInfo[13]) + "Ghz")
    if (str(LANInfo[15])) == 'N/A':
        print("\nLink Quality: " + str(LANInfo[15]))
    else:
        print("\nLink Quality: " + str(LANInfo[15]) + "dBm, " + str(LANInfo[16]))
    print("\n===============================================")
