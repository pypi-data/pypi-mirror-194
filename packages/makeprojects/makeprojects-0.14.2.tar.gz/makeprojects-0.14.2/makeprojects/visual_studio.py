...
class BuildVisualStudioFile(BuildObject):
    ...
    def build(self):
        ...
        if vstudiopath.endswith(".com"):
            ...
            # Use MSBuild
            cmd = [vstudiopath, "-t:Build", "-v:m", "-noLogo", "-m",
                "-p:Configuration={0};Platform={1}".format(
                    targettypes[0], targettypes[1]), self.file_name]
    ...
    def clean(self):
        ...
        # Locate the proper version of Visual Studio for this .sln file
        vstudiopath = where_is_visual_studio(self.vs_version, "msbuild.exe")
        if vstudiopath is None:
            vstudiopath = where_is_visual_studio(self.vs_version, "devenv.com")
        ...
        if vstudiopath.endswith(".com"):
            # Use the IDE
            # Visual Studio 2003 doesn't support setting platforms, just use
            ...
            # Use MSBuild
            cmd = [vstudiopath, "-t:Clean", "-v:m", "-noLogo", "-m",
                "-p:Configuration={0};Platform={1}".format(
                    targettypes[0], targettypes[1]), self.file_name]
        if self.verbose:
            print(" ".join(cmd))
