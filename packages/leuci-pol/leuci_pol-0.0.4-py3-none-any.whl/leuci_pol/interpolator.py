...
def create_interpolator(method, values, F, M, S, log_level=0):
    if method == "linear":
        return Linear(values, F, M, S, log_level)
    else: #nearest is default
        return Nearest(values, F, M, S, log_level)

### Abstract class ############################################################################
class Interpolator(ABC):
    def __init__(self, values, F, M, S, log_level=0):
        self._values = values  
        self._F = F
        self._M = M
        self._S = S
