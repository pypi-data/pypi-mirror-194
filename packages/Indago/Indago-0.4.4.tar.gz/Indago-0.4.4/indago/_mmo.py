...
class MMO(Optimizer):
    ...
    def _init_method(self):
        ...
        if 'ABC' in self.methods:
            ...
            """ Initialize ABC """
            # Generate a swarm
            self.ABC_cS = np.array([Bee(self) for c in range(self.params['bees'])], dtype=Bee)
            self.ABC_cS_k = np.array([Bee(self) for c in range(self.params['bees'])], dtype=Bee)

            # Generate initial trial and probability vectors

            self.trials = np.zeros([self.params['bees']],dtype=np.int_)
