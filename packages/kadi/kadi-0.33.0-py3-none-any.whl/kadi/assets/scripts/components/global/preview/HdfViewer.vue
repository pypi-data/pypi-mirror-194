    ...
    <div :class="{'mt-3 mr-4': depth === 0}">
      <ul :style="listWidth">
        <li v-for="entry in entries_" :key="entry.id">
          ...
          <div class="collapse" :id="`${entry.id}-details`">
            <div class="card card-body py-2 my-2" v-if="entry.meta">
              <strong>{{ $t('Object info') }}</strong>
              <hr class="my-1">
              <div class="row mb-2 mb-md-0" v-for="attr in metaAttrs" :key="attr">
          ...
          <div v-if="entry.is_group">
            <hdf-viewer class="collapse show" :id="entry.id" :entries="entry.children" :depth="depth + 1"></hdf-viewer>
          </div>
        </li>
      </ul>
    </div>
