...
def get_container(cfg: KubernetesContainer) -> Container:
    ...
    return Container(
        ...
        resources=ResourceRequirements(limits=cfg.resources.limits.to_dict(), requests=cfg.resources.requests.to_dict()) if cfg.resources is not None else ResourceRequirements(),
        args=cfg.args,
        env=[
            EnvVar(
                name=envvar.name,
