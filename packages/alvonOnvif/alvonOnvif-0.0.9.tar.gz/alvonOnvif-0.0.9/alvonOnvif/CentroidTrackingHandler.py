...
class CentroidTracker:
    ...
    def update(self, rects):
        ...
        if len(rects) == 0:
            ...
            bboxes = np.zeros((len(rects), 4), dtype="int")
            ...
            for (i, (startX, startY, endX, endY)) in enumerate(rects):
                ...
                bboxes[i] = (startX, startY, endX, endY)

            # if we are currently not tracking any objects take the input
            # centroids and register each of them
            if len(self.objects) == 0:
                for i in range(0, len(inputCentroids)):
                    self.register(inputCentroids[i], bboxes[i])
            ...
            # centroids
                ...
                objectCentroids = list(e[0] for e in self.objects.values())
                ...
                for (row, col) in zip(rows, cols):
                    ...
                    self.objects[objectID][1] = bboxes[col]
                ...
                # register each new input centroid as a trackable object
                    ...
                    for col in unusedCols:
                        self.register(inputCentroids[col], bboxes[col])
