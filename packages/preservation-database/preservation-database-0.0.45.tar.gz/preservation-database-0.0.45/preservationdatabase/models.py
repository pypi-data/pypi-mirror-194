...
class LockssPreservation(models.Model):
    ...
    @staticmethod
    def preservation(
        container_title: str, issn: str, volume: str, no=None, year=None
    ):
        """
...
class PKPPreservation(models.Model):
    ...
    @staticmethod
    def preservation(container_title, issn, volume, no, year=None):
        """
...
class HathiPreservation(models.Model):
    ...
    @staticmethod
    def preservation(container_title, issn, volume, no=None, year=None):
        """
        ...
        if len(pis) > 0:
            ...
            if len(pis) > 0:
                for pi in pis:
                    ...
                    unpacked_range = utils.unpack_range(matches)
                    print(f"Range unpacked to: {unpacked_range}")
...
class InternetArchivePreservation(models.Model):
    ...
    def preservation(container_title, issn, volume, no=None, year=None):
        ...
        pi = preserved_item.filter(volume=str(volume))
            ...
            with open(file, "rb") as f:
                ...
                for json_data in f:
                    ...
                    for outcome, url in list_comp:
                        # print(f"{url}, {outcome.ext_ids.doi},
                        # {outcome.ident}")
                    ...
                    if count == 1000:
                        ...
                        logging.info(f"Flushing {len(batch_objects)} items.")
                        count = 0
                        batch_objects = []
