...
class SteinerTreeEnv(gym.Env):
    ...
    def _get_mask(self) -> np.array:
        ...
        mask[self.graph.nodes.flatten()[self.graph.edge_links[:, 0]] != self.HAS_MSG] = False
    ...
    def step(self, action: Tuple[int, int]) -> Tuple[gym.spaces.GraphInstance, SupportsFloat, bool, bool, dict]:
        ...
        assert np.isclose(self.graph.nodes[u], self.HAS_MSG) == True, f"BUG IN THE LIBRARY! Node {u} is not a part of the path!"
        assert np.isclose(self.graph.nodes[v], self.HAS_MSG) == False, f"BUG IN THE LIBRARY! Node {v} is already a part of the path!"
        
        done = False
        reward = -self.graph.edges[action, 0]
        self.graph.nodes[v] = self.HAS_MSG
