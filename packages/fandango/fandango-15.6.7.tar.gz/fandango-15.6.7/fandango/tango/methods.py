...
def get_model_name(model):
    """
    NON-DEPRECATED YET!
    get_model_name is capable to extract the model from an object,
    parse_tango_model and get_fqdn_name only accepts strings
    """
    if isString(model):
...
def check_attribute(model, attr='', readable=True, timeout=0, brief=False, 
                    trace=False, expire=0):
    ...
    :param expire: checks if the value is older than expire time
    """
    try:
        if isinstance(model, DeviceProxy):
            proxy = model
        elif hasattr(model, "get_device_proxy"):
            proxy, attr = model.get_device_proxy(), model.name()
        else:
            dev, attr = (model,attr) if attr else model.rsplit("/", 1)
            proxy = get_device(dev)
            attrlist = [str(s).lower() for s in proxy.get_attribute_list()]
            
            if attr.lower() not in attrlist:
                raise AttrNotFound(attr)
        ...
        value = proxy.read_attribute(attr)
        
        if readable and value.quality == AttrQuality.ATTR_INVALID:
            return None
        elif expire and value.time.totime() < (time.time() - expire):
            return None
        else:
            if brief:
                value =  getattr(value, "value", 
                        getattr(value, "rvalue", None))
            return value
...
def read_attribute(model, attr='', timeout=0, full=False):
    """
...
def read_internal_attribute(device, attribute, logger=None):
    """
    ...
    logger = logger or (lambda *args: None)
    ...
    if aname == "state":
        ...
        logger("%s.%s = %s" % (attr.device, attr.name, attr.value))
        attr.error = ""
    else:
        if isProxy:
            logger(
                ...
                if isDyn:
                    ...
                    if not device.myClass:
                        ...
                        try:
                            ...
                            logger("%s = %s" % (attr.name, attr.value))
                            attr.error = ""
                        except:
                            attr.throw_exception()
                else:
