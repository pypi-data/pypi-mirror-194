...
def create_distributed_data_parallel_model(model, device_ids):
    # We need to set find_unused_parameters=True when freezing certain modules using the freeze_modules plugin.
    ddp_model = torch.nn.parallel.DistributedDataParallel(DDPWrapperModel(model), device_ids=device_ids, find_unused_parameters=True)
