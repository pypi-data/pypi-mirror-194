...
class UserService:
    ...
    @staticmethod
    def user_search(user_id_list, need_map=False, filter_fields=None):
        if not user_id_list:
            return [], None
        # 查询管理
        try:
            ...
            user_base_set = BaseInfo.objects.filter(id__in=user_id_list).values(*filter_fields)
    ...
    def user_edit_v2(params=None, user_id=None):
        """
        用户修改服务。
        PS：原本的代码逻辑复杂，而且不规范，为了大面积的受影响，开发二版本。
        :param params: 修改参数集合
        :param user_id: 用户ID
        :return: None,err_msg
        """
    ...
    def user_role_edit(params):
        # 修改逻辑 TODO 为什么起这个名字，这个不是用户基础信息修改吗 BaseInfo.objects.filter(id=user_id).update(**base_info)
        try:
            account = str(params.get('account', ''))
            phone = str(params.get('phone', ''))
            full_name = str(params.get('full_name', ''))
