...
class UserDefinedFeatureExtractor(object):
    ...
    def __init__(self,
        ...
        self.in_folder = os.path.join(self.project_path, Paths.OUTLIER_CORRECTED.value)
        self.out_folder = os.path.join(self.project_path, Paths.FEATURES_EXTRACTED_DIR.value)
        ...
        self.roll_windows_values = check_minimum_roll_windows(Options.ROLLING_WINDOW_DIVISORS.value, self.vid_info_df ['fps'].min())
        self.files_found = glob.glob(self.in_folder + '/*.' + self.file_type)
        check_if_filepath_list_is_empty(filepaths=self.files_found,
                                        error_msg=f'SIMBA ERROR: No files of type {self.file_type} found in {self.in_folder}')
