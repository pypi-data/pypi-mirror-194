    ...
    for auth_group in config.auth_groups:
        for env in auth_group.envs:
            ...
            if env.roles:
                ...
                old_content = re.sub(module_regex(iam_module), projects_iam, old_content, flags=re.MULTILINE)
            ...
            for bucket, bindings in igroupby(env.buckets, lambda b: b.name).items():
                ...
                old_content = re.sub(module_regex(module_name), bucket_iam, old_content, flags=re.MULTILINE)
