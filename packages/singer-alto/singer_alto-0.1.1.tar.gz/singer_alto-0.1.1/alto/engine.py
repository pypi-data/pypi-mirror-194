...
class AltoTaskEngine(DoitEngine):
    ...
    def load_doit_config(self) -> t.Dict[str, t.Any]:
        ...
        if bool(os.getenv("ALTO_RICH_UI")):
            # If the user has opted into the rich UI, we will try to import it.
            # If it is not available, we will fall back to the console reporter.
            try:
                from alto.ui import AltoUI
            except ImportError:
                ui = ConsoleReporter(sys.stdout, {"verbosity": 2})
            else:
                ui = AltoUI(None, {"verbosity": 2})
        else:
            ui = ConsoleReporter(sys.stdout, {"verbosity": 2})
    ...
    def task_build(self) -> AltoTaskGenerator:
        ...
        def build_pex(plugin: AltoPlugin) -> None:
            ...
            # Build the pex
            subprocess.run(
                ["pex", *plugin.pip_url.split(), "-o", output, "--no-emit-warnings"],
                check=True,
            )
