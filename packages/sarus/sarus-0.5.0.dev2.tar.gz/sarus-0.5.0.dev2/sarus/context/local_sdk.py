...
class LocalSDKContext(PublicContext):
    ...
    def __init__(self, client: Client) -> None:
        ...
        self.factory().register(
            ...
            lambda protobuf: Schema(cast(sp.Schema, protobuf)),
        )
        self.factory().register(
            sp.type_name(sp.Attribute),
            lambda protobuf: Attribute(cast(sp.Attribute, protobuf)),
        )
        self.factory().register(
            sp.type_name(sp.VariantConstraint),
            lambda protobuf: VariantConstraint(
                cast(sp.VariantConstraint, protobuf)
            ),
        )
