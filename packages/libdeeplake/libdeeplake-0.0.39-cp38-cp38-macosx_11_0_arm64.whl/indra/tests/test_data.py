...
def test_chunk_compressed_chunk_shape_encoder_bps_encoder_matches(tmp_datasets_dir):
    ...
    for i in range(1000):
        assert cpp_ds.tensors[0][i] == ds.tokens[i].numpy()
...
def test_samples_compressed_chunk_shape_encoder_bps_encoder_matches(tmp_datasets_dir):
    ...
    assert len(cpp_ds) == 2
    assert cpp_ds.tensors[0][0].shape == first.shape == (390,)
    assert cpp_ds.tensors[0][1].shape == second.shape == (374,)
