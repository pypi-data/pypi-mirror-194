...
def ingest(log):
    ...
    for item in log.parsed_output:
        ...
        mac_address = (
            utils.normalize_mac_address(item.get("hw_address"))
            if not utils.incomplete_mac(item.get("hw_address"))
            else None
        )
        mode = utils.normalize_interface_mode(item.get("port_link_type"))
        int_type = utils.normalize_interface_type(item.get("intf"))
        enabled = utils.normalize_interface_status(item.get("line_status"))
        ...
        tagged_vlans = [native_vlan] + [
            utils.normalize_vlan_list(vlan) for vlan in item.get("vlan_permitted")
        ]
