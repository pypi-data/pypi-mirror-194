            ...
            for point in data["draw_point"]:
                ...
                if self.__type == "polygon":
                    try:
                        ...
                        draw.line(
                            tuple_poly + tuple_poly[0], fill="red", width=3)
                    except:
                        print(
                            f"이미지에 그림 그려질 Polygon 좌표가 없는 경우의 파일 경로: {self.__img_path}")  # print 함수로 로그하는 방법을 logging모듈을 활용할 것 다음 업데이트!!
                elif self.__type == "bbox":
                    try:
                        draw.rectangle(
                            self.__data, outline=(0, 0, 255), width=3)
                    except:
                        print(
                            f"이미지에 그림 그려질 bbox 좌표가 없는 경우의 파일 경로: {self.__img_path}")  # print 함수로 로그하는 방법을 logging모듈을 활용할 것 다음 업데이트!!
                else:
                    try:
                        tuple_poly = tuple(tuple(x) for x in self.__data)
                        ...
                        print(
                            f"이미지에 그림 그려질 kepoint, polyline 좌표가 없는 경우의 파일 경로: {self.__img_path}")  # print 함수로 로그하는 방법을 logging모듈을 활용할 것 다음 업데이트!!
