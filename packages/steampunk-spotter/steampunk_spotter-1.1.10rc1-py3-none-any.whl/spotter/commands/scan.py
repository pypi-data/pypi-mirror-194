...
class ScanResult:
    ...
    @classmethod
    def from_api_response(cls, response_json: Dict[str, Any], input_tasks: List[Dict[str, Any]],
        ...
        scan_result.parse_check_results(response_json, input_tasks)
        return scan_result

    def parse_check_results(self, response_json: Dict[str, Any], input_tasks: List[Dict[str, Any]]) -> None:
        """
        ...
        for element in response_json.get("elements", []):
            ...
            if task_meta:
                ...
                if display_level == DisplayLevel.ERROR:
                    self.summary.num_errors += 1
                elif display_level == DisplayLevel.WARNING:
                    self.summary.num_warnings += 1
                else:
                    self.summary.num_hints += 1
...
def scan(api_endpoint: str, storage_path: Path, username: Optional[str], password: Optional[str], no_colors: bool,
    ...
    if export_payload:
        ...
        scan_result = ScanResult.from_api_response(response_json, parsing_result.tasks, scan_time)
