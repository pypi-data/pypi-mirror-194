...
class ProxySuiteUnit:
    ...
    def getDepartmentSimpleList(self, depid: int=0):
        ...
        获取部门的编号及上下级信息
        """
        _url = f"https://qyapi.weixin.qq.com/cgi-bin/department/simplelist?access_token={self.getCorpAccessToken(self._corp_id, self._suite_secret)}"
        logger.debug(_url)
        if depid:
            _url += f"&id={depid}"
        return self.getRequest(_url, {})
    ...
    def UserIdListBatch(self, batchsize: int = 100):
        ...
        """
        _url = f"https://qyapi.weixin.qq.com/cgi-bin/user/list_id?access_token={self.getCorpAccessToken(self._corp_id, self._suite_secret)}"
        return self.postRequest(_url, {
            "cursor": "",
            "limit": batchsize
        })
