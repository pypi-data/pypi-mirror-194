...
class ProxySuiteUnit:
    ...
    def getCorpAccessToken(self, corp_id: str, pernament_code: str) -> str:
        ...
        if not os.path.isdir(_tmppath):
            raise Exception("template path is invalid")
        _cache_token_file = os.path.join(_tmppath, f"corp_access_token_{self._corp_id}.bin")
        if os.path.isfile(_cache_token_file):
            with open(_cache_token_file, "rb") as f:
                logger.debug("load access_token from cache file")
                _catched_data = pickle.load(f)
        ...
        if not ret_token or ret_expiretm < int(time.time()):
            ...
            logger.debug("get corp_access_token from cache file")
            logger.debug(_refresh_data)
            if isinstance(_refresh_data, dict):
    ...
    def getDepartmentSimpleList(self, corp_id: str, pernament_code: str, depid: int=0):
        """
        获取部门的编号及上下级信息
        """
        _url = f"https://qyapi.weixin.qq.com/cgi-bin/department/simplelist?access_token={self.getCorpAccessToken(corp_id, pernament_code)}"
        logger.debug(_url)
        if depid:
            _url += f"&id={depid}"
        return self.getRequest(_url, {})
    ...
    def UserIdListBatch(self, corp_id: str, pernament_code: str, batchsize: int = 100):
        """
        批量获取用户编号
        :param batchsize int 批次大小
        """
        _url = f"https://qyapi.weixin.qq.com/cgi-bin/user/list_id?access_token={self.getCorpAccessToken(corp_id, pernament_code)}"
        return self.postRequest(_url, {
            "cursor": "",
            "limit": batchsize
        })
