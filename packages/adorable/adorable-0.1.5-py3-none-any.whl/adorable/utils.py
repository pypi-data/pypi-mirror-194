...
def _copydoc(
    source: Any,
    replace: Optional[
        Mapping[re.Pattern[str], Union[str, Callable[[re.Match[str]], str]]]
    ] = None,
) -> Callable[[T], T]:
    """
    ...
    def decorator(fn: T) -> T:
        ...
        if replace is not None:
            for pattern, replace_with in replace.items():
                doc = pattern.sub(replace_with, doc)
...
def _get_closest_color(color: RGB, colors: Iterable[tuple[T, RGB]]) -> T:
    """
