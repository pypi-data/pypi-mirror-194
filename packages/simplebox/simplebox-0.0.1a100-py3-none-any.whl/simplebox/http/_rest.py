...
class Body(UserDict):
    """
    requests module's HTTP request-related options.
    Usage:
        Options(params={}, data={})
    """
...
class Rest(object):
    ...
    def request(self, api_name: str = None, server_name: str = None, host: str = None, api: str = None,
        ...
        usage:
            ...
            user.get_info(params={}, data={}) equivalent to user.get_info(body=Body(params={}, data={}))
            the Options object is recommended.
        """
        def __inner(func):
            @wraps(func)
            def __wrapper(*args, **kwargs):
    ...
    def __request(self, func: callable, kwargs: dict, api_name: str = None, server_name: str = None, host: str = None,
        ...
        # noinspection PyBroadException
        if "body" in kwargs:
            options = kwargs.get("body")
            if options and isinstance(options, Body):
    ...
    def __build_optional_args(func: Callable, all_args: dict, api_info: dict) -> dict:
        ...
        for k in list(all_args.keys())[:]:
            if spec.args and k not in spec.args and spec.kwonlyargs and k not in spec.kwonlyargs and k != "body":
                del all_args[k]
        if optional_args:
