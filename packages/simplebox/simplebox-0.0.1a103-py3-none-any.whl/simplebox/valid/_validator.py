    ...
    def validate(self, value):
        for operate, obj in self.__operators.items():
            ...
            if not hasattr(obj, operate_) or not hasattr(value, operate_):
                raise ValidatorException(f"Valid error: '{obj}' have not '{operate}' implemented.")
            ...
            if not result:
                raise ValidatorException(f"Valid error: CompareValidator fail: excepted '{value}' {symbol} '{obj}', "
...
class IdentityValidator(Validator):
    ...
    def validate(self, value):
        ...
        for operate, obj in self.__operators.items():
            result = getattr(operator, operate.name)(value, obj)
            if not result:
                raise ValidatorException(f"Valid error: Excepted '{value}' {operate.value} '{obj}', but check fail.")
...
class MemberValidator(Validator):
    ...
    def validate(self, value):
        if _Symbols.in_ in self.__operators and _Symbols.not_in in self.__operators:
            raise ValidatorException(f"Valid error: '{_Symbols.in_.value}' and '{_Symbols.not_in.value}'"
...
class StringValidator(Validator):
    ...
    def validate(self, value):
        if not isinstance(value, str):
            raise TypeError(f'Valid error: Expected {value!r} to be an str')
        ...
        if self.__prefix is not None and not value.startswith(self.__prefix):
            raise ValidatorException(f"Valid error: Expected '{value}' prefix is {self.__prefix}, but check fail.")
        if self.__suffix is not None and not value.endswith(self.__suffix):
            raise ValidatorException(f"Valid error: Expected '{value}' suffix is {self.__suffix}, but check fail.")
        ...
        if self.__black and StringUtils.is_not_black(value):
            raise ValidatorException(f"Valid error: Excepted black, but got a '{value}'")
        if self.__empty and StringUtils.is_not_empty(value):
            raise ValidatorException(f"Valid error: Excepted empty, but got a '{value}'")
...
class TypeValidator(Validator):
    ...
    def validate(self, value):
        ...
        if not issubclass(value_type, tuple(self.__types)) or (self.__can_none and value is not None):
            raise ValidatorException(f"Valid error: Expected \"{self.__type_names}\", got a '{value_type.__name__}'")
...
class BoolValidator(Validator):
    ...
    def validate(self, value):
        if self.__true:
            if not value:
                raise ValidatorException(f"Valid error: Excepted true, but false.")
        else:
            if value:
                raise ValidatorException(f"Valid error: Excepted false, but true.")
    ...
    def validate(self, value):
        ...
        if isinstance(value, datetime):
            ...
            if not self.__format:
                raise ValidatorException(f"Valid error: datetime format is must required.")
            ...
            difference = 0
        if self.__future and difference < 0:
            raise ValidatorException(f"Valid error: Excepted datetime is after, got a before datetime: {value}")
        if self.__past and difference > 0:
            raise ValidatorException(f"Valid error: Excepted datetime is before, got a after datetime: {value}")
...
class IterableValidator(Validator):
    ...
    def validate(self, value):
        ...
        if self.__ne is not None and self.__eq is not None:
            raise ValidatorException(f"Cannot be both equal and unequal.")
        if self.__has is not None and self.__has_no is not None:
            raise ValidatorException(f"Cannot be both have and no.")

        if self.__eq is not None and not self.__eq == value:
            raise ValidatorException(f"Check '{value}' == '{self.__eq}' fail.")
        if self.__ne is not None and not self.__ne != value:
            raise ValidatorException(f"Check '{value}' != '{self.__eq}' fail.")
        if self.__has is not None and self.__has not in value:
            raise ValidatorException(f"Check '{self.__has}' in '{value}' fail.")
        if self.__has_no is not None and self.__has_no in value:
            raise ValidatorException(f"Check '{value}' not in '{self.__has_no}' fail.")
