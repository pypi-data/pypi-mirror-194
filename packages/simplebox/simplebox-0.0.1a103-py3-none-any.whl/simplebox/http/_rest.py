...
class RestFast(object):
    ...
    def send(self, method: Method) -> 'RestFast':
        ...
        """
        self.__method = method.value if method else Method.GET.value
        return self

    def response(self, only_body) -> Response or Dict:
        """
        send request and get response.
        """
        resp: Response = getattr(requests, self.__method)(url=f"{self.__host}{self.__api}", **self.__opts.data)
        if only_body:
            return resp.json()
        else:
            return resp
