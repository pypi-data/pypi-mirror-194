...
from tinybird.ch import (
    ...
    ch_flush_logs_on_all_replicas_sync, ch_attach_partitions, ch_query_table_partitions)
...
class TestPartitions(unittest.TestCase):
    ...
    def test_ch_attach_string_partitions(self):
        ...
        partitions = [p.as_sql() for p in partitions if p.table == 'test_ch_partitions_substring']
    ...
    def test_ch_attach_compound_partitions(self):
        ...
        partitions = async_to_sync(ch_query_table_partitions)(self.workspace.database_server, [self.workspace.database])
        partitions = [p.as_sql() for p in partitions if p.table == 'test_ch_partitions_compound']
...
class TestSqlAnalysisAndValidations(unittest.IsolatedAsyncioTestCase):
    ...
    def test_nested_aggregation_function(self):
        ...
        self.assertEqual(context.exception.args[0], "[Error] You cannot use the aggregate function 'avg(number)' or its alias inside another aggregate function. Contact us at support@tinybird.co for help or read this SQL tip:  https://docs.tinybird.co/tips/data-source-tips.html#choosing-the-engine-partition-key")
    ...
    def test_nested_aggregation_function_with_alias(self):
        ...
        self.assertEqual(CHErrors.ILLEGAL_AGGREGATION, context.exception.code)
        self.assertEqual(context.exception.args[0], "[Error] You cannot use the aggregate function 'max(number)' or its alias inside another aggregate function. Contact us at support@tinybird.co for help or read this SQL tip:  https://docs.tinybird.co/tips/data-source-tips.html#choosing-the-engine-partition-key")
