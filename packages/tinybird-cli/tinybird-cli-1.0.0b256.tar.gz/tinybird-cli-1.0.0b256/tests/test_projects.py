...
from tinybird.tb_cli_modules.common import folder_pull
...
class TestProjects(BaseTest):
    ...
    async def test_tinyb_422(self):
        tb_client = TinyB(self.admin_token, self.get_host())
        with self.assertRaisesRegex(Exception, r"Unable to connect to Kafka server due to error: .*\. Check bootstrap server address, auth settings and firewalls"):
            await tb_client._req('/v0/connector/preview')
    ...
    async def test_join_tables(self):
        ...
        expected_min_number_of_replicas = 2 if is_ci else 1
        self.assertTrue(len(replicas) >= expected_min_number_of_replicas)
    ...
    @tornado.testing.gen_test
    async def test_complete_project(self):
        await self.__push_folder(os.path.dirname(__file__) + '/projects/ecommerce', tag='t0', upload_fixtures=True, push_deps=True)

        params = {
            'token': self.admin_token
        }

        for ds in ['t0__products', 't0__events']:
            response = await self._fetch(f'/v0/datasources/{ds}?{urlencode(params)}')
            self.assertEqual(response.status_code, 200, response)

        for pipe in ['t0__sales', 't0__top_product_per_day', 't0__sales', 't0__top_products', 't0__articles_join_by_id_color__v0']:
            response = await self._fetch(f'/v0/pipes/{pipe}?{urlencode(params)}')
            self.assertEqual(response.status_code, 200, f"{pipe} does not exist")

        # check mat node is using the right version of the target datasource
        response = await self._fetch(f'/v0/pipes/t0__articles_join_by_id_color__v0?{urlencode(params)}')

        ds_id = Users.get_datasource(self.u, 't0__products_by_id_color__v0').id
        self.assertEqual(response.json()['nodes'][0]['materialized'], ds_id)
