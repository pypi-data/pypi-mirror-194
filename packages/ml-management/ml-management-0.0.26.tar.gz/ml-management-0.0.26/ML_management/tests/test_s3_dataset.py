...
from ML_management.dataset.s3_dataset.S3Dataset import (
    S3BucketNotFound,
    S3FolderNotFound,
    S3ObjectNotFound,
)
...
class TestS3Dataset(unittest.TestCase):
    ...
    def test_download_files(self):
        ...
        assert (
            mock_client_download_successful
            is ML_management.dataset.s3_dataset.S3Dataset.client
        )
        ...
        with open(
            os.path.join(local_path, "sample_data/sample_folder/3.txt"), "r"
        ) as f:
            text_one = f.read()
    ...
    def test_download_folder(self):
        s3_dataset = ML_management.dataset.s3_dataset.S3Dataset.S3Dataset()
        assert (
            mock_client_download_folder_successful
            is ML_management.dataset.s3_dataset.S3Dataset.client
        )
        endpoint_url = "some_mock_url"
    ...
    def test_download_bad_object(self):
        s3_dataset = ML_management.dataset.s3_dataset.S3Dataset.S3Dataset()
        assert (
            mock_client_bad_object is ML_management.dataset.s3_dataset.S3Dataset.client
        )
        endpoint_url = "some_mock_url"
    ...
    def test_download_bad_folder(self):
        s3_dataset = ML_management.dataset.s3_dataset.S3Dataset.S3Dataset()
        assert (
            mock_client_bad_folder is ML_management.dataset.s3_dataset.S3Dataset.client
        )
        endpoint_url = "some_mock_url"
