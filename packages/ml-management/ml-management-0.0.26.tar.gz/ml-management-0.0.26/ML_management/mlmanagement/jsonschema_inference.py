...
def __get_or_raise(type_name):
    if type_name in type_map:
        ...
        raise UnsupportedType(
            annotation=type_name, supported_types=list(type_map.keys())
        )
...
def __is_optional(annotation):
    ...
    if repr_annotation.startswith("typing.Optional"):
        return True
    elif repr_annotation.startswith("typing.Union") and repr_annotation.strip(
        "]"
    ).endswith("NoneType"):
...
def __get_json_schema_from_annotation(annotation):
    if annotation.__module__ == "typing":
        if __is_optional(annotation):
            ...
            annotation_name = formatted_annotation.partition("[")[
                0
            ...
            if annotation_name == "List":
                ...
                return {
                    "type": "object",
                    "additionalProperties": __get_json_schema_from_annotation(
                ...
                return {
                    "anyOf": [
                        __get_json_schema_from_annotation(ann)
...
def infer_jsonschema(func):
    ...
    for arg in all_args:
        ...
        schema["properties"][arg] = __get_json_schema_from_annotation(
