...
def login(un, k):
  ...
  print("Logging in...")
  credentials = {'username':un, 'key':k, 'task':'login'}
...
def node(node_name = "", filename = "", lineno = 0, node_description="", library_function = 0):
  ...
  if prev_node == "-999":
    data = {'current_node_name': node_name, 'node_description': node_description,
            'username': username, 'key': key, 'run_id': os.environ['run_id'],
...
def node_log(variables, main_function = False):
  ...
  if main_function == True:
      data = {'run_id':os.environ['run_id'], '_id': prev_node, 'type':'node', 'variables': str(variables), 'username': username, 'key': key, 'main_function':1}
  else:
