...
class FargateRunnerProvider(
    ...
    def __init__(
        ...
        *,
        ...
        :param image_builder: (experimental) Provider running an image to run inside CodeBuild with GitHub runner pre-configured. A user named ``runner`` is expected to exist. The image builder determines the OS and architecture of the runner. Default: image builder with ``FargateRunner.LINUX_X64_DOCKERFILE_PATH`` as Dockerfile
        :param label: (deprecated) GitHub Actions label used for this provider. Default: undefined
