...
class DateRangeCreator:
    ...
    def create_date_range(
            self,
            split=False
    ) -> Union[pd.DatetimeIndex, Tuple[pd.DatetimeIndex, pd.DatetimeIndex]]:
        ...
        if split:
            return encoder_dates, decoder_dates
        return encoder_dates.union(decoder_dates)

    def make_encoder_dates(self, start):
        return pd.date_range(
            end=start, periods=self.encoder_length + 1, inclusive='left',
            freq=self.freq)
    ...
    def map_to_freq(self, date):
        return map_to_freq(date, self.freq)


def map_to_freq(d, freq='W'):
