...
class PytorchForecastingCollateFn:
    ...
    def __init__(self):
        self._collate_fn = pytorch_forecasting.TimeSeriesDataSet._collate_fn

    def __call__(self, batch):
        X, y = self._collate_fn(batch)
...
class PytorchForecastingNeuralNet(base.TimeseriesNeuralNet):
    ...
    def __init__(
            ...
            batch_size=64, callbacks=None, collate_fn=None, verbose=1,
            device='cpu', **kwargs
    ):
        super().__init__(
            ...
            batch_size=batch_size, callbacks=callbacks, collate_fn=collate_fn,
    ...
    def interpret_output(self, X):
        ...
        interpretation = self.nn_.module_.interpret_output(
            raw_predictions, reduction="sum")
        self.nn_.module_.plot_interpretation(interpretation)
    ...
    def plot_prediction(self, X, idx, add_loss_to_title=False, ax=None):
        ...
        self.nn_.module_.plot_prediction(
    ...
    def _raw_predict(self, X, return_x=False):
        ...
        if hasattr(self.nn_.module, '_output_class'):
            module = True
            delattr(self.nn_.module, '_output_class')
        else:
            module = False
        if hasattr(self.nn_.module_, '_output_class'):
            module_ = True
            delattr(self.nn_.module_, '_output_class')
        else:
            module_ = False

        # Generate raw predictions
        ds = self.get_dataset(X)
        raw_predictions = self.nn_.module_.predict(
            ds, mode='raw', batch_size=len(ds), return_x=return_x
        )
        # Recover _output_class attribute
        if module:
            self.nn_.module._output_class = output_class
        if module_:
            self.nn_.module_._output_class = output_class
        return raw_predictions


class TemporalFusionTransformer(PytorchForecastingNeuralNet):
    ...
    def __init__(
            ...
            min_encoder_length=None, criterion=None, optimizer=None, lr=1e-5,
            max_epochs=10, batch_size=64, callbacks=None, emb_dim=10,
            hidden_size=16, hidden_continuous_size=8, lstm_layers=2,
            dropout=0.1, output_size=1, verbose=1, device='cpu', **kwargs
    ):
        super().__init__(
            module=pytorch_forecasting.TemporalFusionTransformer,
            dataset=datasets.TimeseriesDataset, group_ids=group_ids,
            ...
            collate_fn=PytorchForecastingCollateFn(), **kwargs
        )
