...
class ModelParser:
    ...
    def get_keras_model_and_model_graph(
        ...
        try:
            ...
            graph, seen_inputs = model_generator.generate_graph(model_schema, layer_name_to_inbound_nodes)
            return graph, seen_inputs, keras_model
    ...
    def generate_model_graph(self, model_path: Path, model_type: ImportModelTypeEnum) -> Tuple[Dict[str, NodeResponse], List[InputInfo]]:
        model_graph, seen_inputs, _ = self.get_keras_model_and_model_graph(model_path, model_type)
        return model_graph, seen_inputs
