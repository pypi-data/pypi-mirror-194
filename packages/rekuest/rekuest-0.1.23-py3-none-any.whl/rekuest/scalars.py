...
class SearchQuery(str):
    ...
    def validate(cls, v):
        ...
        assert len(definition.variable_definitions) >= 2, "At least two arguments should be provided ($search: String, $values: [ID]))"
        ...
        if (
            len(definition.variable_definitions) > 0
            and definition.variable_definitions[1].variable.name.value != "values"
        ):
            raise ValueError(
                "Seconrd parameter of search function should be '$values: [ID]' if you provide arguments for your options. This parameter will be filled with the default values"
            )
