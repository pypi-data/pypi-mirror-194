...
class PTRaw:
    ...
    def get_file_meta(self, file):
        ...
        file_regex = r"((\d{4}-\d{2}(?:-\d{2})?)\_(.*)\_" + \
                     r"(in-network-rates|allowed-amounts|prescription-drugs|provider-reference|table-of-contents)" + \
                     r"\_?([^\.]*)\.([^(?:\?|$)]*))(?:\??[^$]*)"
        ...
        return {'name': file_name, 'dte': dte, 'mth': mth, 'ext': ext,
                'indexed': index != '', 'index': int(index.split('_')[-1]), 'schema': schema,
                'payer_plan': payer_plan, 'raw_tgt_folder': raw_tgt_folder}
