...
class GetTopoJsonRequest(betterproto.Message):
    ...
    """
    Which map_layer to search - only allowing one; too complex to derive and
    make separations in the response, and no real usecase for combined
    map_layers
    """

    feature_id_filter: List[str] = betterproto.string_field(5)
    """
    List of features by ID to limit the search to - if provided, only these
    ...
    """
    only features with geometriess intersecting with the area defined by the
    bound is returned Passing an empty polygon (= all values set to 0 as
    `optional` is not supported), means no filter on geom
    """

    zoom_level: int = betterproto.int32_field(9)
    """
    This value indicates the zoom level of the map the features will be drawn
    on. Feature-geometries will be decimated using the DouglasPeucker-algorithm
    to reduce fidelity of geometries returned. Follows the zoom-level
    definition of MapBox (https://docs.mapbox.com/help/glossary/zoom-level/) If
    not set, API assumes a value of 6.
    """
...
class GetTopoJsonResponse(betterproto.Message):
    ...
    """
    The resulting TopoJSON (https://github.com/topojson/topojson) as a string-
    blob The ID of the GeoJSON.Feature corresponds to the FeatureID of the
    Feature in the MapLayer. Also includes DisplayName in the attributes of the
    GeoJSON.Feature.
    """
...
class GeoServiceBase(ServiceBase):
    async def get_topo_json(self) -> "GetTopoJsonResponse":
