...
from fortuna.output_calibrator.output_calibration import CalibState
...
class RandomModel(abc.ABC):
    ...
    def train(
        ...
        val_data_loader : DataLoader
            A validation data loader. This is used to validate both posterior fitting and output_calibration.
        calib_data_loader : DataLoader
            A output_calibration data loader. If this is not passed, no output_calibration is performed.
        fit_config : FitConfig
            An object to configure the posterior distribution fitting.
        calib_config : CalibConfig
            An object to configure the output_calibration.
        map_fit_config : Optional[FitConfig] = None
        ...
        -------
        Dict[str, Status]
            Status objects for both posterior fitting and output_calibration.
    ...
    def _calibrate(
        ...
        if self.output_calib_manager is None:
            ...
            if self.posterior.state is None:
                raise ValueError(
                    """Before output_calibration, you must either train the probabilistic model (see 
            ...
            if calib_config.monitor.verbose:
                logging.info(
                    "Pre-compute ensemble of outputs on the output_calibration data loader."
                )
            ...
            if calib_config.monitor.verbose:
                logging.info("Start output_calibration.")
