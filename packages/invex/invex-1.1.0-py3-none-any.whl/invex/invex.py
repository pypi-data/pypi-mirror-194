    ...
    while choice !=7:
        ...
        if choice == 1:
            ...
            for item in itemList:
                if newItemID in item:
                    print("Another item has", newItemID, "as its barcode.")
                    print("Updating quantity of", item[0], "...")
                    newItemQ = item[2]+1
                elif newItemN in item:
                    print("Another item has", newItemN, "as its name.")
                    print("Updating quantity of", item[0], "...")
                    newItemQ = item[2]+1
                else:
                    newItemQ = 1
            testIf0 = len(itemList)
            if testIf0 == 0:
                newItemQ = 1
            if newItemQ != 1:
                for item in itemList:
                    item[2] = newItemQ
                    print("Updated quantity of",item[0])
                    print("")
                    pickle.dump(itemList, open("invex.dat", "wb"))
            else:
                newItemCat = input("Item Category (e.g. Laptop, Cable, etc.): ")
                newItemInOut = ("Signed In")
                itemList.append([newItemN, newItemID, newItemQ, newItemCat, newItemInOut])
            ...
            for item in itemList:
                if modQ in item:
                    ...
                    item[2] = input("Item Quantity: ")
                    item[3] = input("New Item Category (e.g. Laptop, Cable, etc.): ")
            ...
            if searchQ == "*":
                ...
                print("['Name of Item', 'Barcode', 'Quantity', 'Category', 'Item Status (In/Out)']")
                print("")
