...
from starfish.constants import HTTP_TIMEOUT_SECONDS
...
class RemoteAgentAdapter():
    ...
    def get_metadata_list(self, url, authorization_token=None):
        ...
        response = self.request_get(url, headers=headers, timeout=self._timeout)
    ...
    def save_metadata(self, metadata, url, authorization_token=None):
        ...
        response = self.request_post(url, data=metadata, headers=headers, timeout=self._timeout)
        if response and response.status_code == requests.codes.ok:
            data = ResponseWrapper(response).json
            return data
        else:
    ...
    def create_listing(self, listing_data, asset_id, url, authorization_token=None):
        ...
        headers = RemoteAgentAdapter.create_headers('application/json', authorization_token)
        response = self.request_post(url, json=data, headers=headers, timeout=self._timeout)
        if response and response.status_code == requests.codes.ok:
            data = ResponseWrapper(response).json
    ...
    def update_listing(self, listing_id, data, url, authorization_token=None):
        ...
        headers = RemoteAgentAdapter.create_headers('application/json', authorization_token)
        response = self.request_put(url, json=data, headers=headers, timeout=self._timeout)
        if response and response.status_code == requests.codes.ok:
            return True
        else:
    ...
    def upload_asset_data(self, asset_id, data, url, authorization_token=None):
        ...
        response = self.request_post(url, files=files, headers=headers, timeout=self._timeout)
    ...
    def purchase_asset(self, purchase, url, authorization_token=None):
        ...
        headers = RemoteAgentAdapter.create_headers('application/json', authorization_token)
        response = self.request_post(url, json=purchase, headers=headers, timeout=self._timeout)
        if response and response.status_code == requests.codes.ok:
            data = ResponseWrapper(response).json
    ...
    def invoke(self, asset_id, inputs, url, authorization_token=None):
        ...
        url = urljoin(f'{url}/', asset_id)
        headers = RemoteAgentAdapter.create_headers('application/json', authorization_token)
        response = self.request_post(url, json=inputs, headers=headers, timeout=self._timeout)
    ...
    def get_authorization_token(self, username, password, url):
        ...
        if response.status_code == 200:
            ...
            if len(tokens) > 0:
                ...
                response = self.request_post(url, auth=(username, password), timeout=self._timeout)
                if response.status_code == 200:
    ...
    def remove_collection_items(self, url, collection_name, asset_list, authorization_token=None):
        ...
        headers = RemoteAgentAdapter.create_headers('application/json', authorization_token)
        response = self.request_post(url, json=asset_list, headers=headers, timeout=self._timeout)
        if response and response.status_code == requests.codes.ok:
            data = ResponseWrapper(response).json
            return data
        else:
