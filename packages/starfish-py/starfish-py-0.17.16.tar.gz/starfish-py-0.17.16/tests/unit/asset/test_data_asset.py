...
def test_create_empty_data():
    ...
    asset = DataAsset.create(asset_name)
    assert(asset)
    assert(isinstance(asset, DataAsset))
...
def test_create_from_json_text():
    ...
    asset = DataAsset.create(asset_name, json=values)
    assert(asset)
    assert(isinstance(asset, DataAsset))
    assert(asset.name == asset_name)
    assert(asset.type_name == 'dataset')
...
def test_create_from_csv():
    ...
    asset = DataAsset.create(asset_name, csv_rows=rows, csv_header=rows[0].keys())
    assert(asset)
    assert(isinstance(asset, DataAsset))
    assert(asset.name == asset_name)
    assert(asset.type_name == 'dataset')
    assert(asset.metadata['contentType'] ==  'text/csv')
    assert(asset.is_csv)
    assert(asset.csv())
