...
class AgentManager:
    ...
    def register_agents(self, agents, http_client=None):
        ...
        for name, agent in agents.items():
            authentication = None
            auth_data = agent
            if 'authentication' in agent:
                auth_data = agent.get('authentication', {})
            if 'username' in auth_data:
                authentication = AuthenticationBasic(auth_data.get('username'), auth_data.get('password', None))
            elif 'token' in auth_data:
                authentication = AuthenticationToken(auth_data.get('token'))
    ...
    def register_agent(
        ...
        :param dict authentication: Authentication  object that gives access to the agent
