...
from convex_api.utils import remove_0x_prefix

from starfish.agent.agent_base import AgentBase
from starfish.agent.authentication import (
    Authentication,
    AuthenticationBasic,
    AuthenticationToken
)
...
from starfish.constants import JOB_TIMEOUT_SECONDS
...
class RemoteAgent(AgentBase, Generic[TRemoteAgent]):
    ...
    :param dict authentication: authentication is a authentication object with a username/password or a token
        a remote agent.
    ...
    def __init__(self, ddo: DDO, authentication: Authentication = None, http_client: Any = None) -> None:
        ...
        if authentication and not isinstance(authentication, Authentication):
            raise TypeError('authentication parameter must be a child of Authentication class')
    ...
    def invoke(self, asset_or_name: [str, TAsset], inputs: Any = None, is_async: bool = False) -> Any:
        """
        ...
        """
        asset = None
        if isinstance(asset_or_name, str):
            if is_asset_id(asset_or_name) or is_did(asset_or_name):
                asset = self.get_asset(asset_or_name)
            else:
                asset_filter = {
                    'name': asset_or_name,
                    'type': 'operation'
                }
                asset_list = self.find_asset(asset_filter)
                if asset_list:
                    asset = self.get_asset(asset_list[0])
        elif isinstance(asset_or_name, OperationAsset):
            asset = asset_or_name
        else:
            raise ValueError('Asset is not a OperationAsset or string')

        if asset is None:
            raise StarfishAssetNotFound(f'Cannot find invoke asset {asset_or_name}')
    ...
    def get_authorization_token(self) -> str:
        ...
        if self._authentication and isinstance(self._authentication, AuthenticationToken):
        ...
        try:
            ...
            url = None
        token = None
        if url and self._authentication and isinstance(self._authentication, AuthenticationBasic):
    ...
    def find_asset(self, filter_values: dict) -> List[str]:
        """
        Search the metadata list for assets that match the filter dict provided.

        :param dict filter Metadata filter to match for any found assets.
        :return a list of asset dids that match the filter
        ...
        if asset_list:
            for asset_id, metadata in asset_list.items():
                if RemoteAgent.is_metadata_match(filter_values, metadata):
    ...
    def resolve_url(url: str, authentication: Authentication = None, http_client: Any = None) -> DDO:
        ...
        if url:
            ...
            token = None
            # try to get a token from the agent using the username/password authentication
            if authentication:
                if isinstance(authentication, AuthenticationBasic):
                ...
                elif isinstance(authentication, AuthenticationToken):
    ...
    @staticmethod
    def is_metadata_match(filter_values: dict, metadata: dict) -> bool:
        is_found = True
        for name, value in filter_values.items():
            if value and metadata.get(name, None) != value:
                is_found = False
                break
        return is_found
