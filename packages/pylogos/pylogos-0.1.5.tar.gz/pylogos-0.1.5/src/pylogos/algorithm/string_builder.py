...
class SStrChar:
    ...
    :type str: _type_
    """
    def __init__(self, text: str, table_node: str, column_node: Optional[str]=None, op_type: Optional[OperationType]=None):
        self.ori_label = text
        self.text: str = text
    ...
    def __eq__(self, other):
        if type(other) == SStrChar:
            return self.text == other.text and self.table_node == other.table_node and self.column_node == other.column_node
        return False
    ...
    def is_same_root(self, other):
        return self.ori_label == other.ori_label and \
            remove_number_in_string(self.table_node) == remove_number_in_string(other.table_node) and \
            remove_number_in_string(self.column_node) == remove_number_in_string(other.column_node)
...
class StringBuilder:
                ...
                for reference_point, rel1, edge, rel2, has_incoming_edge in self.join_conditions:
                    ...
                    if not has_incoming_edge and reference_point == target_relation:
                        ...
                        if edge or texts:
                            ...
                            correlations_for_rel2 = [sel for sel in self.selection if sel[0] != sel[1] and sel[0] == target_relation and sel[1].is_same_root(rel2) and rel2 == sel[5]]
                            ...
                            if correlations_for_rel2:
                                ...
                                texts.add_suffix(f" of the same")
                                texts += selected_correlation[2]
            ...
            if self.selection:
                ...
                for rp, rel, att, op, val, val_parent in self.selection:
                    if rp.is_same_root(target_relation):
                        ...
                        if val_parent:
                            val_desc.add_suffix(f" of those")
                            val_desc += val_parent
    ...
    def add_projection(self, relation: Node, attribute: Node, agg_func: Optional[str]) -> None:
        # Create SStr objects
        rel_sstr = SStrChar(relation.label, relation.name, op_type=OperationType.Projection)
        att_sstr = SStrChar(attribute.label, relation.name, attribute.name, op_type=OperationType.Projection)
        agg_sstr = SStrChar(agg_func, relation.name, attribute.name, op_type=OperationType.Projection) if agg_func else None
        self.projection.append((rel_sstr, att_sstr, agg_sstr))

    def add_selection(self, reference_point: Node, relation: Node, attribute: Node, op: Predicate, operand: str, operand_parent: Optional[Node] = None) -> None:
        # Create SStr objects
        ref_sstr = SStrChar(reference_point.label, reference_point.name, op_type=OperationType.Selection)
        rel_sstr = SStrChar(relation.label, relation.name, op_type=OperationType.Selection)
        ...
        if type(operand) == SStrSen:
            ...
            operand_parent_sstr = SStrChar(operand_parent.label, operand_parent.name, op_type=OperationType.Selection) if operand_parent else None
        self.selection.append((ref_sstr, rel_sstr, att_sstr, op_sstr, operand_sstr, operand_parent_sstr))

    def add_join_conditions(self, reference_point: Node, relation1: Node, edge: str, relation2: Node, has_incoming_edge: bool) -> None:
        # Create SStr objects
        ref_sstr = SStrChar(reference_point.label, reference_point.name, op_type=OperationType.Selection)
        rel1_sstr = SStrChar(relation1.label, relation1.name, op_type=OperationType.Selection)
        edge_sstr = SStrChar(edge, relation1.name, relation2.name, op_type=OperationType.Selection)
        rel2_sstr = SStrChar(relation2.label, relation2.name, op_type=OperationType.Selection)
    ...
    def add_grouping(self, reference_point: Node, relation: Node, attribute: Node) -> None:
        # Create SStr objects
        ref_sstr = SStrChar(reference_point.label, reference_point.name, op_type=OperationType.Grouping)
        rel_sstr = SStrChar(relation.label, relation.name, op_type=OperationType.Grouping)
    ...
    def add_having(self, reference_point: Node, relation: Node, attribute: Node, function: Node, edge: Predicate, value: Node) -> None:
        # Create SStr objects
        ref_sstr = SStrChar(reference_point.label, reference_point.name, op_type=OperationType.Selection)
        rel_sstr = SStrChar(relation.label, relation.name, op_type=OperationType.Selection)
        att_sstr = SStrChar(attribute.label, relation.name, attribute.name, op_type=OperationType.Selection)
