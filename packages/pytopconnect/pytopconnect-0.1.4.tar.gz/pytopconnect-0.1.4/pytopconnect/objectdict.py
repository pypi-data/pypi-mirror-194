							...
							for dic in result:
								...
								for key in cond[i][1]:
									try:
										...
										pass
								res = self.array_value_to_str(res)
								d = [len(i) for i in res]
								k = sum(d)/len(d) if len(d)>0 else None
								if k:
									if cond[i][2]:
										if all([SequenceMatcher(lambda x: x in [' ','\t','\n','\r'],a,b.upper()).ratio() >= (len(a)/k)+cond[i][3] for b in res]):
											arr.append(dic)
									else:
