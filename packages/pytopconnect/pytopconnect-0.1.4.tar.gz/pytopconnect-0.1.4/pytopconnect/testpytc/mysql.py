...
class queryPY(object):
	...
	try:
		...
		def show_field_f(self,cursor,con,q,r):
			ex = list(filter(None, q.split(";")))
			show = "SELECT `COLUMN_NAME`,`DATA_TYPE`,`IS_NULLABLE`,`COLUMN_KEY`,`COLUMN_DEFAULT`,`EXTRA`,`CHARACTER_MAXIMUM_LENGTH` FROM INFORMATION_SCHEMA.COLUMNS WHERE `TABLE_NAME` = '"+ex[0]+"'"
			cursor.execute(show)
