...
class QueryRead(do):
	...
	def valid(self,k):
		n = ["database","dbFile","excelFile"]
		i = 0
		key = None
		while len(n) > i:
			try:
				if n[i] in ["dbFile","excelFile"]:
	...
	def enjoy_replace(self,table,index):
		def is_atr(obj,atr):
			if table:
				try:
					...
					return False
			return False
		names = [is_atr(self,self.valid(n)) for n in self.DATA[self.METHOD]]
		if all(names):
			for name in names:
				if name.INDEX == index:
					result = {}
					kol = name.QR.DB_NAME
					app = name.QR.APPLICATION
					obj = is_atr(name,table)
					if obj:
						c = self.all_columns(table,index)[kol]
						v = self.all_values(table,index)[kol]
						val = self.values_res(v,c)
						fet = self.fetchall_res(v,c)
						result[table] = {
							"LENGTH":len(val),
							"TYPE":self.fields(table,index)[kol][0],
							"COLUMNS":list(c[0]),
							"VALUES": val,
							"FETCHALL":fet,
							"NAME":table,
						...
						self.startDO(kol,{kol:result},self.METHOD,self,name.ALL_TABLES)
						break
