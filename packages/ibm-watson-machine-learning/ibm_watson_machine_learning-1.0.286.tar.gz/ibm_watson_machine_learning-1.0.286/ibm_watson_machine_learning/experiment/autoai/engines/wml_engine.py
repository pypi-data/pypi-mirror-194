...
class WMLEngine(BaseEngine):
    ...
    def summary(self, scoring: str = None, sort_by_holdout_score: bool = True) -> 'DataFrame':
        ...
        details = self._wml_client.training.get_details(training_uid=self._current_run_id, _internal=True)
        optimized_scoring = self._auto_pipelines_parameters['scoring']

        if scoring is None:
            scoring = optimized_scoring

        return create_summary(details=details, scoring=scoring, sort_by_holdout_score=sort_by_holdout_score).rename(
                    {
                        f"training_{optimized_scoring}":
                            f"training_{optimized_scoring}_(optimized)",
                        f"holdout_{optimized_scoring}":
                            f"holdout_{optimized_scoring}_(optimized)"
                    }, axis='columns')
