...
class Fk_viewer(object):
    ...
    def setting_file_name(self):
        ...
        vessel_name=self.config[self.env]['VESSEL_NAME']
        file_name= f"f{d.strftime('%Y%m%d%H%M%S')}{vessel_name}.mp4"
        file_name_s= f"fc{d.strftime('%Y%m%d%H%M%S')}{vessel_name}.mp4"
