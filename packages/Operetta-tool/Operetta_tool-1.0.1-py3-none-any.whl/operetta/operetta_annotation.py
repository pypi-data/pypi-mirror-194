...
def detect_outlires(xml_file, list_of_out:list = []):
...
def image_concatenate(path_to_images, imgs, img_length, img_width, scale_factor, chanels, n_thread):
...
def z_projection(path_to_tiff, color):
...
def merge_images(image_list, intensity_factors):

    result = None
    
    for i, image in enumerate(image_list):
        if result is None:
            result = image.astype(float) * alpha_list[i]
        else:
            result = cv2.addWeighted(result, 1, image.astype(float) * alpha_list[i], 1, 0)
