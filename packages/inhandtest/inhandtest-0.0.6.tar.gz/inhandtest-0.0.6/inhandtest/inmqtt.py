...
from inhandtest.file import check_file, del_file
...
class MqttClient:
    ...
    def sub(self, topic, qos=0, clear_recv_datas=False):
        """订阅主题
        :param topic:
    ...
    def un_sub(self, topic: str or list):
        """取消主题订阅
    ...
    def pub(self, topic: str, payload: str or dict, qos=0, clear_recv_datas=False):
        """发布主题
    ...
    def expect_data(self, expect_recv_rules: dict, timeout=10):
...
class MosquittoBrokerWindows:
    ...
    def __init__(self, mosquitto_path='D:\Program Files\mosquitto', port=1883):
        ...
        :param port:
        """
        self.__mosquitto_broker_p = None
        ...
        [del_file(file_) for file_ in (self.broker_log_file, self.sub_log_file)]

    def start_mosquitto_broker(self, config='mosquitto.conf'):
        """
        ...
        self.__mosquitto_broker_p = subprocess.Popen(f'"{self.mosquitto_exe}" -c "{cnf_file}" -p {self.port} -v',
                                                     stdout=log, stderr=log)
        logging.info(f"start mosquitto broker server success the pid is {self.__mosquitto_broker_p.pid}")
        logging.info(f"mosquitto broker log is {self.broker_log_file}")

    def broker_user(self, username, password, filename='pwfile.example', type_='add'):
        """ 添加删除更新服务的用户

        :param username: 用户名
        :param password: 密码
        :param filename: 存储文件密码文件
        :param type_: 'add'|'delete'|'update'
        :return:
        """
    ...
    def sub(self, param: str) -> subprocess.Popen:
        """ 订阅Topic
    ...
    def close(self, p=None):
        """ 删除服务

        :param p: 默认删除mosquitto server， 传入p 删除指定的
        :return:
        """
        p = self.__mosquitto_broker_p if p is None else p
        if p.pid is None:
            pass
        else:
            logging.info(f"stop windows server pid {p.pid}")
