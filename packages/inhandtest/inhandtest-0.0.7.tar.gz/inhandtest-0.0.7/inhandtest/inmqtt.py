...
class MosquittoBrokerWindows:
    ...
    def start_mosquitto_broker(self, config='mosquitto.conf'):
        ...
        :return:
        """
        if not check_windows_process('mosquitto.exe'):
    ...
    @staticmethod
    def close(server='mosquitto'):
        """ 删除服务

        :param server: 'mosquitto'|'mosquitto_sub'
        :return:
        """
        if server == 'mosquitto':
            subprocess.Popen(f'taskkill /F /IM mosquitto.exe')
            logging.info("close mosquitto broker server")
        elif server == 'mosquitto_sub':
            subprocess.Popen(f'taskkill /F /IM mosquitto_sub.exe')
            logging.info("close mosquitto_sub server")
        else:
