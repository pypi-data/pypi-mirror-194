...
def check_windows_process(process_name: str or list) -> bool or list:
    """ 检测进程是否运行
    :param process_name: str|list, 进程名称(不一定是应用程序名称！), eg:'3CDaemon'|'mosquitto'
    :return: boolean
    """

    def _search(process_name_: str):
        if process_name_ == '':
            return False
        else:
            command = ''.join(['tasklist |findstr ', process_name_])
            logging.info(command)
            process_ = len(os.popen(command).readlines())
            logging.info(f'number of {process_name_} process is: {process_}')
            if process_ >= 1:
                return True
            else:
                return False
    ...
    if isinstance(process_name, list):
        return [_search(name_) for name_ in process_name]
    else:
        return _search(process_name)
...
if __name__ == '__main__':
    ...
    print(check_windows_process('mosquitto.exe'))
