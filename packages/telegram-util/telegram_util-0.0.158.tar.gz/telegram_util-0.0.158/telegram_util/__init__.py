...
def send_message(chat, message, parse_mode='markdown'):
    try:
        wait_when_need(lambda: chat.send_message(message, disable_web_page_preview=True, parse_mode=parse_mode))
    except Exception as e:
        wait_when_need(lambda: chat.send_message(message, disable_web_page_preview=True))