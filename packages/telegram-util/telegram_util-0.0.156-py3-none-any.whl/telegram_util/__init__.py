...
def send_message(chat, message, parse_mode='Markdown'):
    try:
        chat.send_message(message, disable_web_page_preview=True, parse_mode='markdown')
    except Exception as e:
        ...
        if str(e).endswith('seconds'):
        	...
        	time.sleep(wait + 5)
        	send_message(chat, message, parse_mode=parse_mode)
        	return
        chat.send_message(message, disable_web_page_preview=True)